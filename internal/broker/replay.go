...
type PayloadStreamParser func(data []byte, fn func(evt *domain.Event)) error

type Replayer struct {
	cfg   ReplayConfig
	store domain.EventStore
	parse PayloadStreamParser
}

func NewReplayer(cfg ReplayConfig, store domain.EventStore, parse PayloadStreamParser) *Replayer {
	if cfg.BatchSize <= 0 {
		cfg.BatchSize = 50000
	}
	return &Replayer{
	...
	for _, partDir := range partDirs {
		...
		for {
			...
			for rawOffset+12 <= len(payload) {
				...
				parseErr := r.parse(userPayload, func(evt *domain.Event) {
					res.EventsRead++
