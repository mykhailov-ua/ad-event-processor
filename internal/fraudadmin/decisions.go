...
func getCampaignFraudForCustomer(ctx context.Context, pool interface {
	...
	if err != nil {
		return out, mapCampaignNotFound(err)
	}
	out.pass = uint8(pass)
...
func queryFraudExplainCH(ctx context.Context, host DecisionsHost, ipHash string, hours int, campaignID uuid.UUID) (fraudExplainCHRow, bool, error) {
	var out fraudExplainCHRow
	clickhouse := host.DecisionsClickHouse()
	if host == nil || clickhouse == nil {
		return out, false, fmt.Errorf("clickhouse not configured")
	}

	ipBytes, err := hex.DecodeString(ipHash)
	if err != nil || len(ipBytes) != 16 {
		return out, false, ValidationError("ip_hash must be 32 hex characters")
	}
...
func explainLiveScore(ctx context.Context, host DecisionsHost, row fraud.FeatureRow) (float64, error) {
	scorer, err := host.FraudExplainScorer(ctx)
	if err != nil {
		return 0, err
	}
...
func ExplainFraudDecision(ctx context.Context, host DecisionsHost, customerID uuid.UUID, ipHash string, campaignID *uuid.UUID, hours int) (FraudDecisionDTO, error) {
	if customerID == uuid.Nil {
		return FraudDecisionDTO{}, ValidationError("customer_id is required")
	}
	...
	if campaignID != nil && *campaignID != uuid.Nil {
		...
		if _, err := getCampaignFraudForCustomer(ctx, host.DecisionsPool(), customerID, filterCampaign); err != nil {
	...
	chRow, found, err := queryFraudExplainCH(clickhouseCtx, host, ipHash, hours, filterCampaign)
	if err != nil {
		return FraudDecisionDTO{}, err
	}
	if !found {
	...
	thresholds, err := getCampaignFraudForCustomer(ctx, host.DecisionsPool(), customerID, resolvedCampaignID)
	if err != nil {
		if errors.Is(err, campaign.ErrCampaignNotFound) {
			return FraudDecisionDTO{}, ErrFraudDecisionNotFound
		}
	...
	if chRow.hasShadowScore {
		...
		modelScorePtr = &score
	} else if host.FraudExplainLiveScoreEnabled() {
		liveScore, liveErr := explainLiveScore(clickhouseCtx, host, featureRow)
		if liveErr == nil {
