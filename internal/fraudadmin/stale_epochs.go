...
func CheckAndHandleStaleEpochs(ctx context.Context, host StaleEpochsHost) error {
	if host == nil || len(host.StaleEpochsRedisShards()) == 0 {
		return nil
	}
	...
	for _, redisClient := range host.StaleEpochsRedisShards() {
		if redisClient == nil {
			continue
		}
	...
	err := host.StaleEpochsPool().QueryRow(ctx, "SELECT value FROM system_settings WHERE key = 'fraud_rl_suspect_pct'").Scan(&currentPctStr)
	if err != nil {
	...
	if err := host.StaleEpochsUpdateSettings(ctx, map[string]string{
