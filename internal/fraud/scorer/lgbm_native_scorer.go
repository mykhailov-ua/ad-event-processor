...
func (s *NativeScorer) Dims() int {
	if s == nil || s.model == nil {
		return features.FeatureVectorDims
	}
	if s.model.dims > 0 {
		return s.model.dims
	}
...
func (s *NativeScorer) ScoreBatch(ctx context.Context, rows []features.FeatureRow) ([]float64, error) {
	if len(rows) == 0 {
		return nil, nil
	}
...
func ScoreBatchSoA(ctx context.Context, model *NativeModel, rows []features.FeatureRow, out []float64) error {
	...
	var features [features.FeatureVectorDims]float64
	for i := range rows {
		features[0] = events[i]
		features[1] = clicks[i]
		features[2] = ctr[i]
