...
func bookAndClaimLease(ctx context.Context, t *testing.T, worker *OperationLeaseWorker, seq int64) (uuid.UUID, error) {
	t.Helper()
	opID := uuid.New()
	scope := dedup.NewAdapter(worker.host.Pool(), 1, 1).RegionScope(dedupkey.RelaySourceID(1), seq, seq)
	_, err := worker.Book(ctx, OperationLeaseBookRequest{
		OpID:         opID,
		RegionCode:   1,
		Role:         "management",
	...
	rows, err := db.New(worker.host.Pool()).OperationLeaseClaimExecuting(ctx, db.OperationLeaseClaimExecutingParams{
