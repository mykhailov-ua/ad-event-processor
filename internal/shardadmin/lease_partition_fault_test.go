...
func TestFault_OperationLease_PGPartition(t *testing.T) {
	...
	host := newLeaseTestHost(t, pool, nil, cfg)
	worker := NewOperationLeaseWorker(host)
...
func TestFault_OperationLease_PGStopDuringExecuting(t *testing.T) {
	...
	host := newLeaseTestHost(t, infra.Pool, nil, cfg)
	worker := NewOperationLeaseWorker(host)
	...
	host.(*leaseTestHost).SetPool(infra.Pool)
...
func TestFault_OperationLease_GhostExecutor(t *testing.T) {
	...
	host := newLeaseTestHost(t, pool, []redis.UniversalClient{redisClient}, cfg)
	worker := NewOperationLeaseWorker(host)
	...
	standbyWorker := NewOperationLeaseWorker(host)
...
func TestFault_OperationLease_DualCAS(t *testing.T) {
	...
	host := newLeaseTestHost(t, pool, nil, cfg)
	coordinator := NewOperationLeaseWorker(host)
	...
	for i := range contenders {
		...
		go func(i int) {
			defer wg.Done()
			nodeWorker := NewOperationLeaseWorker(host)
