...
// Generate builds the full admin traffic-source template list from bundled schemas and sidecar overrides.
...
// LoadSidecar reads curated template overrides from deploy/vendor/traffic_source_ui.yaml.
...
// WriteJSON emits internal/traffictemplates/generated_templates.json for API and future admin UI.
...
// WriteTypeScript emits TypeScript for admin UI consumers (optional; not used while web/ is absent).
func WriteTypeScript(path string, templates []Template) error {
	return os.WriteFile(path, RenderTypeScript(templates), 0o644)
}

// RenderTypeScript returns generated TypeScript source bytes.
...
// CountBundledTrafficSchemas returns traffic_source rows in the integration catalog.
