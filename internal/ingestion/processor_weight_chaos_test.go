...
// TestChaos_ProcessorWeightDrain proves weighted consume skew between fast and slow replicas.
func TestChaos_ProcessorWeightDrain(t *testing.T) {
	if testing.Short() {
	...
	t.Logf("chaos_proof fault=processor_weight_drain lag_ratio=%.2f fast_reads=%d slow_reads=%d drain_weight=%.2f baseline_ok=true",
