...
func TestFault_ShardLoadSpike(t *testing.T) {
	...
	stack := startAdsIngestStackOpts(t, infra, "ads-fault-shard-load-spike", adsIngestStackOpts{
	...
	baseline := measureFaultTrackLatencies(t, stack.Handler, controlID, 1, shardLoadSpikeBaselineN)
	...
	for w := range shardLoadSpikeWorkers {
		...
		go func() {
			...
			for range shardLoadSpikePerWorker {
				...
				start := time.Now()
				status := postFaultTrackWait(t, stack.Handler, controlID, "impression", userID, clickID)
	...
	faultproof.Log(t, "shard_load_spike", map[string]string{
...
func postFaultTrackWait(t *testing.T, h *AdsPacketHandler, campaignID uuid.UUID, evtType, userID, clickID string) int {
	t.Helper()
...
func measureFaultTrackLatencies(t *testing.T, h *AdsPacketHandler, campaignID uuid.UUID, workers, total int) []time.Duration {
	t.Helper()
	if workers < 1 {
		workers = 1
	}
		...
		go func() {
			...
			for i := 0; i < n; i++ {
				...
				payload := map[string]any{
					...
					"payload":     map[string]string{"fault": "spike-baseline"},
				}
				body, err := json.Marshal(payload)
				require.NoError(t, err)
				start := time.Now()
