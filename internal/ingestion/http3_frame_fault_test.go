...
func http3FaultMalformedCases() []http3FaultCase {
...
func TestFault_HTTP3_MalformedCorpus(t *testing.T) {
	var (
		okCount   int
		errCounts = map[string]int{}
	)
	for _, tc := range http3FaultMalformedCases() {
		tc := tc
	...
	faultproof.Log(t, "http3_malformed_corpus", map[string]string{
		"cases":      fmt.Sprintf("%d", len(http3FaultMalformedCases())),
...
func TestFault_HTTP3_ConcurrentParse(t *testing.T) {
	const (
		workers    = 24
		iterations = 200
	)
	cases := http3FaultMalformedCases()
	...
	faultproof.Log(t, "http3_concurrent_parse", map[string]string{
