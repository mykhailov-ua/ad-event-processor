...
func TestEmergencyCircuitBreaker_Filter(t *testing.T) {
	...
	require.NoError(t, redisClient.HSet(ctx, "config:values", "emergency_breaker", "true").Err())
	...
	require.NoError(t, redisClient.HSet(ctx, "config:values", "emergency_breaker", "false").Err())
