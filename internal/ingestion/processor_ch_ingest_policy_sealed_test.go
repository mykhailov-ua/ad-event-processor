...
func TestProcessorCHIngestPolicy_devModeUsesEmbed(t *testing.T) {
	t.Setenv("AD_EVENT_PROCESSOR_LICENSE_MODE", "dev")
	raw, err := resolveProcessorCHIngestPolicyBytes()
	require.NoError(t, err)
	require.Equal(t, processorCHIngestPolicyEmbed, raw)
}

func TestProcessorCHIngestPolicy_invalidMCK(t *testing.T) {
	var mck [32]byte
	mck[0] = 1
	sealed, err := licensing.SealAsset(sealedProcessorCHIngestAssetLabel, processorCHIngestPolicyEmbed, mck)
	require.NoError(t, err)
	...
	_, err = resolveProcessorCHIngestPolicyBytes()
	require.Error(t, err)
}

func TestProcessorCHIngestPolicy_validMCK(t *testing.T) {
	...
	sealed, err := licensing.SealAsset(sealedProcessorCHIngestAssetLabel, wantPolicy, mck)
	require.NoError(t, err)
	...
	require.NoError(t, InitProcessorCHIngestPolicy())
