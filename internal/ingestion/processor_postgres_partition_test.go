...
func (pp *adsProcessorPartitionInfra) refreshPGPoolViaProxy(t *testing.T) {
	t.Helper()
	ctx := context.Background()
	pp.Pool.Close()
	pool, err := pgxpool.New(ctx, pp.pgProxyConnStr)
	require.NoError(t, err)
	pp.Pool = pool
	pp.Queries = db.New(pool)
	waitAdsPGReady(t, pp.Pool)
}
