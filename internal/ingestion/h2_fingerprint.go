...
func (fp *h2ConnFingerprint) copyTo(req *parsedHTTPRequest) {
	if fp == nil || req == nil {
		return
	}
	if fp.settingsCRC != 0 {
...
func recordH2PseudoHeader(req *parsedHTTPRequest, id uint8) {
...
func markH2DowngradeArtifact(req *parsedHTTPRequest) {
	if req != nil {
...
func h2SettingsAnomaly(ua string, flags uint8, enablePush uint8, initialWindow, windowInc uint32) bool {
	...
	if flags&h2WireFlagSettings == 0 || !uaClaimsChromeNotChromium(ua) {
		return false
	}
...
func h2PseudoOrderMismatch(ua string, order uint16, count uint8) bool {
	if count != 4 || !uaClaimsChromeNotChromium(ua) {
		return false
	}
