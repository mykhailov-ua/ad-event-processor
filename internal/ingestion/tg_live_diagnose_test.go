...
func TestTgClickLiveDiagnose(t *testing.T) {
	...
	t.Run("net/http keep-alive", func(t *testing.T) {
		...
		t.Log(probeTgClick(client, base, campaign, token, 40))
	})
...
func probeTgClick(client *http.Client, base, campaign, token string, n int) map[string]int {
	...
	for i := 0; i < n; i++ {
		...
		_ = resp.Body.Close()
		kind := classifyTgLiveResponse(resp.StatusCode, resp.Header.Get("Content-Length"), body)
		kinds[kind]++
	}
	return kinds
}

func classifyTgLiveResponse(status int, contentLen string, body []byte) string {
	switch status {
	case 302:
		return "redirect_302"
	case 404:
