...
func (t *ModeratorIPTable) PublishEntriesForTest(entries []moderatorintel.Entry, gen uint64) {
	t.publishEntries(entries, gen)
}

func (l *moderatorIntelFeedLoader) RefreshOnceForTest(ctx context.Context) {
	l.refreshOnce(ctx)
}

const (
	ModeratorIntelFeedFileName = moderatorIntelFeedFile
	ModeratorIntelSigFileName  = moderatorIntelSigFile
)
