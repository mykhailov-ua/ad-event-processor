...
func TestInitUnifiedFilterLua_seedCouplingPrefixesScriptNonce(t *testing.T) {
	pub, priv, err := ed25519.GenerateKey(rand.Reader)
	require.NoError(t, err)

	tel := licensing.HWIDTelemetry{
		DMIUUID:  "nonce-lua-dmi",
		DiskID:   "nonce-lua-disk",
		MAC:      "aa:bb:cc:dd:ee:03",
		CPUModel: "Nonce Lua CPU",
		CPUCores: 4,
	}
	...
	wantLua := "-- sealed unified-filter nonce test\nreturn 0\n"
	...
	unifiedFilterLuaActive = ""
	unifiedFilterLuaAny = unifiedFilterLua
	unifiedFilterLuaMu.Unlock()

	require.NoError(t, InitUnifiedFilterLua())
	require.Equal(t, formatLuaWithRedisScriptNonce(wantLua, nonce), unifiedFilterLuaForScript())
}
