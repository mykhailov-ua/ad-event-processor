...
func (sw *SettingsWatcher) StoreDynamicConfigForTest(cfg *DynamicConfig) {
	if sw == nil || cfg == nil {
		return
	}
...
func (sw *SettingsWatcher) StoreFcapSnapshotForTest(snap *rtb.FcapSnapshot) {
	if sw == nil {
		return
	}
	if snap == nil {
	...
	sw.fcapSnap.Store(snap)
}

func (sw *SettingsWatcher) StoreFraudScoreBoostsForTest(snap *FraudScoreBoostSnapshot) {
	if sw == nil || snap == nil {
		return
	}
	sw.fraudScoreBoosts.Store(snap)
}

func (sw *SettingsWatcher) ApplyConfigForTest(version int64, data map[string]string) {
	if sw == nil {
		return
	}
...
func (sw *SettingsWatcher) RTBEmergencyBreaker() bool {
	if sw == nil {
		return false
	}
...
func (sw *SettingsWatcher) FraudBoosts() *rtb.FraudBoostSnapshot {
	snap := sw.GetFraudScoreBoosts()
	if snap == nil {
		return nil
	}
...
func (sw *SettingsWatcher) SyncVPPRatiosForTest(ctx context.Context) {
	sw.syncVPPRatios(ctx)
}

func (sw *SettingsWatcher) SetRedisShardsForTest(shards []redis.UniversalClient) {
	if sw == nil {
		return
	}
	sw.redisShards = shards
}

func (sw *SettingsWatcher) StoreVPPRatiosForTest(snap *VPPRatioSnapshot) {
	if sw == nil {
		return
	}
	if snap == nil {
...
func (sw *SettingsWatcher) PickHealthyShardForTest() redis.UniversalClient {
	return sw.pickHealthyShard()
}
