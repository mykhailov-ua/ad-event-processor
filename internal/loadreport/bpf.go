...
type markerStat struct {
	...
	CampaignSlot int     `json:"campaign_slot"`
	Count        int64   `json:"count"`
	AvgUs        float64 `json:"avg_us"`
	P99Us        float64 `json:"p99_us"`
	MaxUs        float64 `json:"max_us"`
...
func writeBPFReport(b *strings.Builder, bpfDir string, data *bpfSummary, timeline *bpfTimeline) {
	...
	b.WriteString("Load generator context switches and on-CPU time are tracked separately from tracker work.\n\n")
...
func writeHotSyscallsSection(b *strings.Builder, hot []syscallStat) {
	...
	sort.Slice(sorted, func(i, j int) bool {
		if sorted[i].Role == "tracker" && sorted[j].Role != "tracker" {
			return true
		}
		if sorted[i].Role != "tracker" && sorted[j].Role == "tracker" {
			return false
...
func writeMarkersSection(b *strings.Builder, markers []markerStat) {
	...
	for _, m := range sorted {
		fmt.Fprintf(b, "| %s | %s | %d | %d | %.1f | %.1f | %.1f |\n",
			m.Role, m.Marker, m.CampaignSlot, m.Count, m.AvgUs, m.P99Us, m.MaxUs)
	}
	b.WriteString("\n")
}
	...
	if len(slowUprobe) > 0 {
		...
		for _, e := range slowUprobe[:limit] {
			...
			fmt.Fprintf(b, "| %s | %s | %d | %d | %d |\n",
				e.Role, marker, e.CampaignSlot, e.DurationUs, e.PID)
		}
		b.WriteString("\n")
	}
