...
const leaseChaosContainerStopTimeout = 10 * time.Second

// TestChaos_OperationLease_PGPartition is CH-MR-06: stale lease expires via janitor.
func TestChaos_OperationLease_PGPartition(t *testing.T) {
	t.Parallel()
	if testing.Short() {
	...
	logChaosProof(t, "mr_lease_pg_partition", map[string]string{
...
// TestChaos_OperationLease_PGStopDuringExecuting simulates PG partition while a lease is executing.
func TestChaos_OperationLease_PGStopDuringExecuting(t *testing.T) {
	if testing.Short() {
		t.Skip("integration test")
	}
	...
	lease, err := db.New(infra.Pool).GetOperationLease(ctx, ingestion.ToUUID(opID))
	require.NoError(t, err)
	require.Equal(t, string(LeaseStateExpired), lease.LeaseState)
	...
	logChaosProof(t, "mr_lease_pg_partition_executing", map[string]string{
		"subsystem":    "operation_lease",
		"op_id":        opID.String(),
		"lease_state":  lease.LeaseState,
		"budget_ok":    "true",
...
// TestChaos_OperationLease_GhostExecutor is CH-MR-07: already_confirmed skips double Redis write.
func TestChaos_OperationLease_GhostExecutor(t *testing.T) {
	t.Parallel()
	if testing.Short() {
		t.Skip("integration test")
	}
...
// TestChaos_OperationLease_DualCAS is CH-MR-08: one executing row under contention (>=20 goroutines).
func TestChaos_OperationLease_DualCAS(t *testing.T) {
	t.Parallel()
	if testing.Short() {
		t.Skip("integration test")
	}
	...
	logChaosProof(t, "mr_lease_dual_cas", map[string]string{
		"subsystem":       "operation_lease",
		"op_id":           opID.String(),
		"execution_count": "1",
...
func stopLeasePGContainer(t *testing.T, infra *database.TestDBInfra) {
	t.Helper()
	timeout := leaseChaosContainerStopTimeout
