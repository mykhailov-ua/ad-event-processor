...
func (s *Service) forceRefillCampaignFromPG(ctx context.Context, campaignID uuid.UUID, currentSpend int64) error {
	...
	err := s.GetPool().QueryRow(ctx, `SELECT budget_limit FROM campaigns WHERE id = $1`, ingestion.ToUUID(campaignID)).Scan(&budgetLimit)
	if err != nil {
		return err
	}
	...
	key := ingestion.BudgetCampaignKey(campaignID)
