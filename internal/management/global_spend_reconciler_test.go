...
func setupGlobalSpendTest(t *testing.T) (context.Context, *pgxpool.Pool, redis.UniversalClient, uuid.UUID) {
	...
	_, err := pool.Exec(ctx,
		...
		ingestion.ToUUID(customerID), "Global Spend Customer", 500_000_000)
	require.NoError(t, err)
	_, err = pool.Exec(ctx,
		"INSERT INTO campaigns (id, name, status, customer_id, budget_limit) VALUES ($1, $2, $3, $4, $5)",
		ingestion.ToUUID(campaignID), "Global Spend Campaign", "ACTIVE", ingestion.ToUUID(customerID), budgetLimit)
	require.NoError(t, err)
...
func TestGlobalSpendReconciler_ApplyBatch(t *testing.T) {
	...
	require.NoError(t, pool.QueryRow(ctx,
		`SELECT current_spend FROM campaigns WHERE id = $1`, ingestion.ToUUID(campaignID),
	).Scan(&currentSpend))
