...
type CTVSettlementResult = ingestion.CTVSettlementResult
...
func (s *Service) ApplyCTVSettlement(
	...
	result, err := ingestion.ApplyCTVSettlement(ctx, s.pool, settlementID, customerID, campaignID, spendMicro)
	if err != nil {
		if errors.Is(err, ingestion.ErrCTVSettlementCampaignNotFound) {
...
func (w *OutboxWorker) handleApplyGTVSettlement(ctx context.Context, payload []byte) error {
	...
	_, err = ingestion.ApplyCTVSettlement(ctx, w.svc.GetPool(), p.SettlementID, customerID, campaignID, p.SpendMicro)
	return err
}
