...
func TestOperationLeaseClaimExecuting_SingleWinnerUnderContention(t *testing.T) {
	...
	_, err := q.InsertOperationLease(ctx, db.InsertOperationLeaseParams{
		...
		RegionCode:   1,
		Role:         "tracker",
		ReplicaSetID: ingestion.ToUUID(replicaSetID),
		Attempt:      1,
		FactorU:      ingestion.ToUUID(factorU),
		DedupScope:   []byte(`{}`),
	...
	lease, err := q.GetOperationLease(ctx, ingestion.ToUUID(opID))
	require.NoError(t, err)
...
func TestOperationLease_CompleteAndExpire(t *testing.T) {
	...
	_, err := q.InsertOperationLease(ctx, db.InsertOperationLeaseParams{
		...
		ReplicaSetID: ingestion.ToUUID(replicaSetID),
		Attempt:      1,
		FactorU:      ingestion.ToUUID(uuid.New()),
	...
	completed, err := q.CompleteOperationLease(ctx, ingestion.ToUUID(opID))
	require.NoError(t, err)
	...
	staleID := uuid.New()
	_, err = q.InsertOperationLease(ctx, db.InsertOperationLeaseParams{
		OpID:         ingestion.ToUUID(staleID),
		RegionCode:   1,
		Role:         "tracker",
		ReplicaSetID: ingestion.ToUUID(uuid.New()),
		Attempt:      1,
		FactorU:      ingestion.ToUUID(uuid.New()),
		DedupScope:   []byte(`{}`),
	...
	stale, err := q.GetOperationLease(ctx, ingestion.ToUUID(staleID))
	require.NoError(t, err)
