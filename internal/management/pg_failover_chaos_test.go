...
type pgFailoverChaosInfra struct {
...
func setupPgFailoverChaosInfra(t *testing.T) (*pgFailoverChaosInfra, func()) {
	t.Helper()
...
// TestChaos_PostgresMasterFailover is GAP-GEO-02: coordinator promotes sync standby, fences stale primary, pushes DSN to regional cells.
func TestChaos_PostgresMasterFailover(t *testing.T) {
	if testing.Short() {
		t.Skip("chaos integration test")
	}

	ctx := context.Background()
	infra, cleanup := setupPgFailoverChaosInfra(t)
	defer cleanup()
	...
	logChaosProof(t, "postgres_master_failover", map[string]string{
		"subsystem":         "global_pg",
