...
func testConditionC1AuthoritativeDeadline(t *testing.T) {
	...
	viewA, err := db.New(pool).GetOperationLease(ctx, ingestion.ToUUID(opID))
	require.NoError(t, err)
	viewB, err := db.New(pool).GetOperationLease(ctx, ingestion.ToUUID(opID))
	require.NoError(t, err)
...
func testConditionC6RenewMax(t *testing.T) {
	...
	_, err = pool.Exec(ctx, `
		...
		WHERE op_id = $1`, ingestion.ToUUID(opID))
	require.NoError(t, err)
