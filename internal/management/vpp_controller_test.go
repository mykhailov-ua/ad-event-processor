...
func TestRunVPPPacingController_writesRedisRatio(t *testing.T) {
	...
	campaignID, err := svc.CreateCampaign(ctx, CampaignCreateSpec{
		...
		PacingMode:     db.PacingModeTypeVPP,
	...
	_, err = pool.Exec(ctx, `
		...
		WHERE id = $1`, ingestion.ToUUID(campaignID))
	require.NoError(t, err)
...
func TestRunVPPPacingController_onPaceWritesFullRatio(t *testing.T) {
	...
	svc := NewService(pool, []redis.UniversalClient{rdb}, ingestion.NewStaticSlotSharder(1), cfg)
	defer svc.Close()
...
func TestRunVPPPacingController_skipsNonVPP(t *testing.T) {
	...
	svc := NewService(pool, []redis.UniversalClient{rdb}, ingestion.NewStaticSlotSharder(1), &config.Config{})
	defer svc.Close()
