...
// CH-MR-01: cold node during warmup uses neighbor median and caps weight at w_max_cold.
func TestChaos_ScorePhaseA_ColdNodeCappedWeight(t *testing.T) {
	...
	logChaosProof(t, "mr_score_cold_node", map[string]string{
...
// CH-MR-02: scrape gap after warm period falls back to neighbor without aggressive drain.
func TestChaos_ScorePhaseC_ScrapeGapNoDrain(t *testing.T) {
	cfg := DefaultScorerConfig()
	prev := 0.55
	in := NodeScoreInput{
	...
	logChaosProof(t, "mr_score_scrape_gap", map[string]string{
		"provenance":   res.Provenance,
...
// CH-MR-03: all neighbors unhealthy uses historical_daily with gentle drain only.
func TestChaos_ScorePhaseD_AllNeighborsBadHistorical(t *testing.T) {
	cfg := DefaultScorerConfig()
	prev := 0.50
	hist := 0.72
	in := NodeScoreInput{
	...
	logChaosProof(t, "mr_score_neighbors_bad", map[string]string{
		"provenance":   res.Provenance,
...
// CH-MR-11: UDP epoch loss keeps equal peer weights (no aggressive drain).
func TestChaos_Score_UDPEpochLossEqualWeights(t *testing.T) {
	t.Parallel()
	...
	logChaosProof(t, "mr_udp_epoch_loss", map[string]string{
...
// CH-MR-12: three drain epochs reduce weight; in-flight scoring state preserved.
func TestChaos_Score_DrainThreeEpochs(t *testing.T) {
	t.Parallel()
	...
	logChaosProof(t, "mr_drain_three_epochs", map[string]string{
