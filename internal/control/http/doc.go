...
//
// Role:
//   - CORS, CSRF, security headers, AdminRequestContextMiddleware composed in controlplane/serve.go around the management mux.
...
//
// Defaults and limits:
//   - AdminLongRouteTimeouts and ManagementWriteTimeout in admin_timeouts.go; composed in controlplane/serve.go.
//   - AdminRequestContextMiddleware default ceiling HTTP_WRITE_TIMEOUT_MS on /api/*; long routes use AdminLongRouteTimeouts.
...
//	go test ./internal/control/http/ -short -run 'TestAdminRequestContext|TestManagementWriteTimeout' -count=1
