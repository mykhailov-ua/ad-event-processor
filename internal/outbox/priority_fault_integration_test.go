...
func TestFault_OutboxPriorityLanes(t *testing.T) {
	if testing.Short() {
	...
	worker := newWorker(t, pool, []redis.UniversalClient{redisClient}, cfg)
	ctx := context.Background()

	const pacingBacklog = 500
	pacingPayload, err := json.Marshal(outbox.CampaignPacingPayload{
	...
	blPayload, err := json.Marshal(outbox.BlacklistPayload{
		Action: "add",
		IP:     "203.0.113.99",
		Reason: "manual",
	})
	...
	faultproof.Log(t, "outbox_priority_lanes", map[string]string{
