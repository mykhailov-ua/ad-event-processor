...
func NewRegionOutboxRelay(host RegionRelayHost) *RegionOutboxRelay {
	code := uint8(0)
	if host != nil {
		code = host.RegionRelayRegionCode()
	}
	return &RegionOutboxRelay{
		host:       host,
		regionCode: code,
		worker:     NewWorker(host),
	}
}
...
func (r *RegionOutboxRelay) Start(ctx context.Context, interval time.Duration) {
	...
	for {
		...
		case <-pollTimer.C:
			var processed int
			var err error
			err = r.host.WithPostgresHigh(ctx, func(runCtx context.Context) error {
				var innerErr error
...
func (r *RegionOutboxRelay) ProcessPendingWithCount(ctx context.Context, limit int32) (int, error) {
	...
	opCtx, cancel := workerContext(ctx, regionRelayOutboxTimeout)
	defer cancel()

	var rows []regionDeliveryRow
	err := pgx.BeginFunc(opCtx, r.host.Pool(), func(tx pgx.Tx) error {
...
func (r *RegionOutboxRelay) revertRegionDeliveriesPending(ctx context.Context, rows []regionDeliveryRow) error {
	if r == nil || r.host == nil || len(rows) == 0 {
		return nil
	}
...
func (r *RegionOutboxRelay) leasesEnabled() bool {
	return r != nil && r.host != nil && r.host.MultiRegionCell()
}

func (r *RegionOutboxRelay) relayLeaseWorker() *shardadmin.OperationLeaseWorker {
	if worker := r.host.OperationLeaseWorker(); worker != nil {
		return worker
	}
	return r.host.NewOperationLeaseWorker()
}
...
func (r *RegionOutboxRelay) applyDeliveryLeased(opCtx, ctx context.Context, row regionDeliveryRow) error {
	...
	bookReq := r.host.RelayDeliveryBookRequest(opCtx, r.regionCode, row.outboxEventID, row.eventType, row.payload, 1)
...
func (r *RegionOutboxRelay) applyDeliveryDirect(opCtx, ctx context.Context, row regionDeliveryRow) error {
	adapter := r.host.DedupAdapter(opCtx)
	...
	if adapter != nil {
		...
		shards := r.host.RedisShards()
		if len(shards) > 0 && claim.DedupKey != "" {
			redisKey := dedupkey.RedisKey(claim.DedupKey)
			ok, nxErr := r.host.SetNXOnAllShards(opCtx, redisKey, "1", 48*time.Hour)
			if nxErr != nil {
				return nxErr
			}
...
func (r *RegionOutboxRelay) applyDeliverySideEffects(opCtx, ctx context.Context, row regionDeliveryRow, claim dedup.ClaimResult) error {
	...
	shards := r.host.RedisShards()
	if len(shards) > 0 && claim.DedupKey != "" && claim.Outcome == dedup.OutcomeConfirmed {
		redisKey := dedupkey.RedisKey(claim.DedupKey)
		ok, nxErr := r.host.SetNXOnAllShards(opCtx, redisKey, "1", 48*time.Hour)
		if nxErr != nil {
			return nxErr
	...
	if err := r.worker.HandleOutboxEvent(opCtx, ctx, ev); err != nil {
		return err
	}
