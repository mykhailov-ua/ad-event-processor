...
func NewMetricsWorker(host MetricsHost) *MetricsWorker {
	nodeID, _ := os.Hostname()
	role := "management"
	region := int16(0)
	if host != nil {
		id, r, reg := host.NodeIdentity()
		if id != "" {
			nodeID = id
		}
		if r != "" {
...
func (w *MetricsWorker) NodeID() string {
	if w == nil {
		return ""
	}
	return w.nodeID
...
func (w *MetricsWorker) Record(metric string, value float64) {
	if w == nil {
		return
	}
...
func (w *MetricsWorker) Flush(ctx context.Context, now time.Time) error {
	if w == nil || w.pool == nil {
		return nil
	}
...
func (w *MetricsWorker) flushBuckets(ctx context.Context, now time.Time) error {
	samples := w.acc.Drain()
	if len(samples) == 0 {
		return nil
	}
...
func (w *MetricsWorker) expireBuckets(ctx context.Context, now time.Time) error {
