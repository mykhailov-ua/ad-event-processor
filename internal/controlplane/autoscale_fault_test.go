...
func TestFault_AutoscaleNoDoubleFreeze(t *testing.T) {
	t.Parallel()
	if testing.Short() {
	...
	for range workers {
		go func() {
			defer wg.Done()
			if err := svc.AutoscaleBudgets(ctx, []*domain.SyncWorker{syncWorker}); err != nil {
				errMu.Lock()
				if firstErr == nil {
					firstErr = err
				}
	...
	faultproof.Log(t, "autoscale_no_double_freeze", map[string]string{
