...
const (
	licenseRevokeQueueBatchSize = 32
	defaultLicenseRevokePoll    = 30 * time.Second
)

type LicenseRevokeQueueWorker struct {
	pool       *pgxpool.Pool
...
func NewLicenseRevokeQueueWorker(pool *pgxpool.Pool, interval time.Duration) *LicenseRevokeQueueWorker {
	if interval <= 0 {
		interval = defaultLicenseRevokePoll
	}
	return &LicenseRevokeQueueWorker{
		pool:       pool,
		interval:   interval,
		reload:     reloadLicense,
		currentKey: activeActivationLicenseKey,
	}
}

func (w *LicenseRevokeQueueWorker) Start(ctx context.Context) {
	if w == nil || w.pool == nil {
		return
	}
...
func (w *LicenseRevokeQueueWorker) ProcessOnce(ctx context.Context) (int, error) {
	if w == nil || w.pool == nil {
		return 0, nil
	}
	...
	err := pgx.BeginFunc(opCtx, w.pool, func(tx pgx.Tx) error {
		rows, err := tx.Query(opCtx, `
			...
			ORDER BY id
			LIMIT $1
			FOR UPDATE SKIP LOCKED`, licenseRevokeQueueBatchSize)
		if err != nil {
...
func (s *Service) StartLicenseRevokeQueueWorker(interval time.Duration) {
	...
	s.startWorker(func() {
		NewLicenseRevokeQueueWorker(s.pool, interval).Start(s.ctx)
	})
}
