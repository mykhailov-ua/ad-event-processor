...
func TestFault_DeliveryOptimizerSingleWriter(t *testing.T) {
	...
	_, err = pool.Exec(ctx, `
		...
		domain.ToUUID(lowID), domain.ToUUID(highID))
	require.NoError(t, err)

	_, err = pool.Exec(ctx, `UPDATE campaigns SET current_spend = daily_budget / 10, pacing_mode = 'EVEN' WHERE id = $1`, domain.ToUUID(lowID))
	require.NoError(t, err)
