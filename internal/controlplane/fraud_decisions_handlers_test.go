...
type fraudDecisionsStub struct {
	...
	response   controlplane.FraudDecisionDTO
	err        error
}

func (s *fraudDecisionsStub) ExplainFraudDecision(_ context.Context, customerID uuid.UUID, ipHash string, campaignID *uuid.UUID, hours int) (controlplane.FraudDecisionDTO, error) {
	s.customerID = customerID
	s.ipHash = ipHash
	s.hours = hours
	s.campaignID = campaignID
	if s.err != nil {
		return controlplane.FraudDecisionDTO{}, s.err
	}
	return s.response, nil
}

func newFraudDecisionsHandlers(stub *fraudDecisionsStub) *controlplane.FraudHTTPHandlers {
...
func TestGetFraudDecision_returnsBreakdown(t *testing.T) {
	...
	stub := &fraudDecisionsStub{
		response: controlplane.FraudDecisionDTO{
			...
			Disclaimer:          controlplane.FraudDecisionDisclaimer(),
			Features:            map[string]float64{"events": 120},
			CampaignThresholds: controlplane.FraudTierThresholdsDTO{
				Scope:      "campaign",
				PassMax:    30,
				SuspectMax: 60,
				IVTMax:     80,
	...
	var out controlplane.FraudDecisionDTO
