...
func setupM8DB(t *testing.T) (*Service, func()) {
	t.Helper()
	if testing.Short() {
		t.Skip("CPA-M8 integration: run without -short (Docker testcontainers)")
	}
...
func TestCPA_M8_ListDLQInbox_capiVsPostbackFilter(t *testing.T) {
	svc, cleanup := setupM8DB(t)
	defer cleanup()
...
func TestCPA_M8_ListConsentProofs_filtersByUser(t *testing.T) {
	svc, cleanup := setupM8DB(t)
	defer cleanup()
	ctx := context.Background()
	reader := newOpsReader(svc)
...
func TestCPA_M8_ListDomainRotation_dmrCounts(t *testing.T) {
	svc, cleanup := setupM8DB(t)
	defer cleanup()
	ctx := context.Background()
	reader := newOpsReader(svc)
...
func TestCPA_M8_GetIncidentSnapshot_staleListsCampaigns(t *testing.T) {
	svc, cleanup := setupM8DB(t)
	defer cleanup()
	ctx := context.Background()
	reader := newOpsReader(svc)
...
func TestCPA_M8_RetryDLQInbox_postbackRequeuesOutbox(t *testing.T) {
	svc, cleanup := setupM8DB(t)
	defer cleanup()
	ctx := context.Background()
	reader := newOpsReader(svc)
