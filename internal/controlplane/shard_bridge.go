// shard_bridge: Service hosts slot migration, Redis health, dedup adapter, and Postgres failover wiring.
...
func (s *Service) GetShardHealth(ctx context.Context) (shardadmin.ShardHealthReport, error) {
	// Postgres outbox lag + per-shard Redis PING and config:version vs last processed outbox id.
