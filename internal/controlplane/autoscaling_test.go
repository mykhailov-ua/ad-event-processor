...
func TestSmartBudgetAutoscaling(t *testing.T) {
	...
	require.NoError(t, pool.QueryRow(ctx, `SELECT balance FROM customers WHERE id = $1`, domain.ToUUID(customerID)).Scan(&balanceBefore))

	err = svc.AutoscaleBudgets(ctx, []*domain.SyncWorker{syncWorker})
	require.NoError(t, err)

	var spendA string
	err = pool.QueryRow(ctx, "SELECT current_spend::TEXT FROM campaigns WHERE id = $1", domain.ToUUID(campaignA)).Scan(&spendA)
	require.NoError(t, err)
	assert.Equal(t, "5000000", spendA)

	var limitA string
	err = pool.QueryRow(ctx, "SELECT budget_limit::TEXT FROM campaigns WHERE id = $1", domain.ToUUID(campaignA)).Scan(&limitA)
	require.NoError(t, err)
	assert.Equal(t, "90000000", limitA)

	var limitB string
	err = pool.QueryRow(ctx, "SELECT budget_limit::TEXT FROM campaigns WHERE id = $1", domain.ToUUID(campaignB)).Scan(&limitB)
	require.NoError(t, err)
	...
	require.NoError(t, pool.QueryRow(ctx, `SELECT balance FROM customers WHERE id = $1`, domain.ToUUID(customerID)).Scan(&balanceAfter))
