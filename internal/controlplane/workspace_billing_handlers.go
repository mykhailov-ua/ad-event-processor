...
func (billHandlers *BillingHTTPHandlers) registerUsageExportRoutes(mux *http.ServeMux) {
	if billHandlers.UsageExport == nil {
		return
	}
	...
	mux.HandleFunc("GET /api/v1/billing/usage/export", limit(exportLimit(perm("customers:read", billHandlers.exportUsageDaily))))
}

func (billHandlers *BillingHTTPHandlers) exportUsageDaily(w http.ResponseWriter, r *http.Request) {
	if r.URL.Query().Get("format") != "csv" {
		httpresponse.Error(w, http.StatusBadRequest, "BAD_REQUEST", "format must be csv")
		return
	}

	spec, err := billHandlers.parseUsageExportSpec(r)
	if err != nil {
	...
	result, err := billHandlers.UsageExport.ExportUsageDailyCSV(r.Context(), spec, &buf)
	if err != nil {
		billHandlers.writeServiceError(w, err)
		return
	}
...
func (billHandlers *BillingHTTPHandlers) parseUsageExportSpec(r *http.Request) (UsageExportSpec, error) {
	...
	if billHandlers.ResolveUsageExportCustomerFilter != nil {
		filteredCustomer, filteredCostCenter, authErr := billHandlers.ResolveUsageExportCustomerFilter(r, customerRaw, costCenter)
		if authErr != nil {
