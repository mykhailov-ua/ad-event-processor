...
func setupPgFailoverFaultInfra(t *testing.T) (*postgresFailoverFaultInfra, func()) {
	t.Helper()
	...
	infra := &postgresFailoverFaultInfra{
...
func TestFault_PostgresMasterFailover(t *testing.T) {
	...
	regionCfg := func(code uint8) *config.Config {
		return &config.Config{
			...
			PostgresStandbyDSN:          config.Secret(infra.StandbyDSN),
			PostgresFailoverRedisURL:    redisURL,
			PostgresFailoverPollMs:      200,
		}
	}
	...
	rtA := cellA.StartPostgresFailover(ctx)
	rtB := cellB.StartPostgresFailover(ctx)
	defer rtA.ClosePostgresFailover()
	defer rtB.ClosePostgresFailover()
