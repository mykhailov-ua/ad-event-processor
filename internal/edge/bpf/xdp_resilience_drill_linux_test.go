...
func TestResilienceDrill_LoopbackBlocklistDrop(t *testing.T) {
	...
	require.NoError(t, bpf.LoadEdgeObjectsLenient(&objs, nil))
	defer objs.Close()
	require.NoError(t, bpf.InitConfigWith(objs.Config, bpf.InitOptions{}))
	...
	statsBefore, err := bpf.AggregateStats(objs.Stats)
	require.NoError(t, err)
	before := statsBefore[bpf.StatDropBlocklist]
	...
	statsAfter, err := bpf.AggregateStats(objs.Stats)
	require.NoError(t, err)
	after := statsAfter[bpf.StatDropBlocklist]
