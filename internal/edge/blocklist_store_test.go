package edge

import (
	"fmt"
	"testing"

	"github.com/stretchr/testify/assert"
	"github.com/stretchr/testify/require"
)

func TestKeyFromHost_ipv4(t *testing.T) {
	key := KeyFromHost(203, 0, 113, 7)
	assert.Equal(t, uint32(32), key.PrefixLen)
	assert.Equal(t, ToBPFAddr(0xcb007107), key.Addr)
}

func TestMergeDenyIPs_dedup(t *testing.T) {
	manual := []string{"203.0.113.5", "203.0.113.5"}
	auto := []string{"203.0.113.5", "203.0.113.6"}
	fraud := []string{"203.0.113.6"}

	got := MergeDenyIPs(manual, auto, fraud)
	assert.Len(t, got, 2)
	_, ok := got[HostKey(203, 0, 113, 5).Addr]
	assert.True(t, ok)
	_, ok = got[HostKey(203, 0, 113, 6).Addr]
	assert.True(t, ok)
}

func TestMergeDenyIPs_skipsNonIPv4(t *testing.T) {
	got := MergeDenyIPs([]string{"not-an-ip", "2001:db8::1"}, nil, nil)
	assert.Empty(t, got)
}

func TestApplyDiff_reusesScratch(t *testing.T) {
	store := NewBlocklistStore()
	firstPtr := fmt.Sprintf("%p", store.scratchHosts)

	_, _, err := store.ApplyDiff(BlocklistMaps{}, []string{"1.2.3.4"}, nil, nil)
	require.Error(t, err)

	_, _, err = store.ApplyDiff(BlocklistMaps{}, []string{"1.2.3.5"}, nil, nil)
	require.Error(t, err)
	assert.Equal(t, firstPtr, fmt.Sprintf("%p", store.scratchHosts))
}
