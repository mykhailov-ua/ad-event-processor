...
func TestReconcileBudgetSnapshot_detectsDriftAndEnqueuesAdjust(t *testing.T) {
	...
	worker := NewReconWorker(host, time.Hour)
...
func TestReconcileBudgetSnapshot_skipsMigrationFence(t *testing.T) {
	...
	host := newTestHost(t, pool, []redis.UniversalClient{redisClient}, &config.Config{})
	NewReconWorker(host, time.Hour).ReconcileBudgetSnapshot(ctx)
