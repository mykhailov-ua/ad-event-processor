...
func TestApplyReconciliationAdjust_pgFailurePreservesRedis(t *testing.T) {
	...
	host := newTestHost(t, pool, []redis.UniversalClient{redisClient}, nil)
	applier := NewAdjustApplier(host)

	err = applier.Apply(ctx, 42, payload)
	require.Error(t, err)
...
func TestApplyReconciliationAdjust_redisRetryAfterPgCommit(t *testing.T) {
	...
	host := newTestHost(t, pool, []redis.UniversalClient{failRdb}, nil)
	applier := NewAdjustApplier(host)
	...
	err = applier.Apply(ctx, 77, payload)
	require.Error(t, err)
	...
	require.NoError(t, applier.Apply(ctx, 77, payload))
