...
func writeCampaignOverviewCSV(ctx context.Context, w *csv.Writer, deps reports.ReportExportDeps, customerID string) error {
...
func writeCustomerPortfolioCSV(ctx context.Context, w *csv.Writer, deps reports.ReportExportDeps, customerID string) error {
	if deps.BuyerPortfolio == nil {
		return fmt.Errorf("portfolio reader not configured")
	}
	cid, err := uuid.Parse(customerID)
