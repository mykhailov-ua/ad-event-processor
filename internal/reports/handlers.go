...
func (h *ReportsHTTPHandlers) Register(mux *http.ServeMux) {
	...
	h.registerCustomerFraudByTypeReport(mux)
	h.registerWireSignalBreakdownReport(mux)
	h.registerReportCatalog(mux)
	h.registerLayerDesyncSummaryReport(mux)
	h.registerLayerDesyncDrilldownReport(mux)
	h.registerCustomerFraudByDimensionReport(mux)
	h.registerSignalEffectivenessReport(mux)
	h.registerRTTSplitTunnelReport(mux)
	h.registerCampaignToggleCohortReport(mux)
	h.registerFraudEvidencePackReport(mux)
