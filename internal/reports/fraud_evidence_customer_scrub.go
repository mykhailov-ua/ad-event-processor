...
func scrubFraudEvidenceTimelineRows(rows []FraudEvidenceTimelineRowDTO) []FraudEvidenceTimelineRowDTO {
	if len(rows) == 0 {
		return rows
	}
	out := make([]FraudEvidenceTimelineRowDTO, len(rows))
	for i := range rows {
		out[i] = rows[i]
		out[i].Country = ""
		out[i].Sub1 = ""
...
func scrubFraudEvidenceFraudRows(rows []FraudEvidenceFraudRowDTO) []FraudEvidenceFraudRowDTO {
	if len(rows) == 0 {
		return rows
	}
	out := make([]FraudEvidenceFraudRowDTO, len(rows))
	for i := range rows {
		row := rows[i]
