...
func registerCustomerFraudByDimensionReport(h *reports.ReportsHTTPHandlers, mux *http.ServeMux) {
	limit := h.ApplyRateLimit
	permAny := h.RequireAnyPermission
	if permAny == nil {
		permAny = func(_ []string, next http.HandlerFunc) http.HandlerFunc { return next }
	}
	mux.HandleFunc("GET /api/v1/reports/customer-fraud-by-dimension", limit(permAny(ReportPermsFraudCustomer(), h.WrapReport("customer-fraud-by-dimension", func(w http.ResponseWriter, r *http.Request) { getCustomerFraudByDimensionReport(h, w, r) }))))
}

func getCustomerFraudByDimensionReport(h *reports.ReportsHTTPHandlers, w http.ResponseWriter, r *http.Request) {
	...
	if len(campaignIDs) == 0 {
		httpresponse.JSON(w, http.StatusOK, reports.CustomerFraudByDimensionReportResponse{
...
func buildCustomerFraudByDimensionRows(
	...
	topCategoryByCampaign := make(map[string]reports.DimensionTopCategorySlot)
	switch dimension {
	case "placement", "campaign":
		...
		agg := make(map[reports.DimensionAggKey]*reports.CustomerFraudByDimensionRowDTO)
		for _, row := range rawRows {
			...
			key := reports.DimensionAggKey{CampaignID: row.CampaignID, Value: value}
			slot := agg[key]
			if slot == nil {
				slot = &reports.CustomerFraudByDimensionRowDTO{
			...
			if row.EventCount > topCategoryByCampaign[row.CampaignID].Events {
				topCategoryByCampaign[row.CampaignID] = reports.DimensionTopCategorySlot{
					Category: category,
...
func dimensionValueFromIVT(row reports.IVTBySourceCHRow, dimension string) string {
	switch dimension {
	case "sub1":
		return row.Sub1
	case "sub2":
...
func dimensionRowsFromAgg(agg map[reports.DimensionAggKey]*reports.CustomerFraudByDimensionRowDTO) []reports.CustomerFraudByDimensionRowDTO {
	out := make([]reports.CustomerFraudByDimensionRowDTO, 0, len(agg))
	for _, row := range agg {
		if row.Clicks > 0 {
			row.IVTRate = float64(row.IVTEvents) / float64(row.Clicks)
		}
		row.IVTRateLabel = reports.FormatRateDisplay(row.IVTRate)
		out = append(out, *row)
	}
...
func applyDimensionCompareDeltas(current, prior []reports.CustomerFraudByDimensionRowDTO) {
	...
	for i := range current {
		...
		current[i].DeltaLabel = reports.FormatDeltaLabel(delta)
		current[i].DeltaTone = reports.DeltaTone(delta / 100)
	}
}
