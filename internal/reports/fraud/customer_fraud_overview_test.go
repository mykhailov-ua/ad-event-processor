...
func TestBuildCustomerFraudOverview_staleFreshness(t *testing.T) {
	t.Parallel()
	out := BuildCustomerFraudOverview(0, 0, 0, reports.DataFreshnessDTO{Stale: true, CHLagSeconds: 400})
...
func TestBuildCustomerFraudOverview_ratesAndDisplay(t *testing.T) {
	t.Parallel()
	out := BuildCustomerFraudOverview(100, 25, 10, reports.DataFreshnessDTO{Stale: true})
