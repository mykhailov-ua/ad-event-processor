...
func UpsertMLShadowDeltaSnapshot(ctx context.Context, pool *pgxpool.Pool, snap reports.MLShadowDeltaSnapshot) error {
	if pool == nil {
...
func LoadMLShadowDeltaSnapshot(ctx context.Context, pool *pgxpool.Pool) (reports.MLShadowDeltaSnapshot, bool, error) {
	if pool == nil {
		return reports.MLShadowDeltaSnapshot{}, false, nil
	}
	var snap reports.MLShadowDeltaSnapshot
	var rowsJSON []byte
...
WHERE snapshot_key = $1`, mlShadowDeltaSnapshotKey).
	...
	if err != nil {
		return reports.MLShadowDeltaSnapshot{}, false, err
	}
	if len(rowsJSON) > 0 {
...
func MLShadowDeltaSnapshotFreshness(snap reports.MLShadowDeltaSnapshot, now time.Time) reports.DataFreshnessDTO {
	dto := reports.DataFreshnessDTO{
