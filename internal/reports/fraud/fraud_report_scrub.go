...
func scrubFraudBreakdownRow(ctx context.Context, row reports.FraudBreakdownRowDTO) reports.FraudBreakdownRowDTO {
...
func scrubFraudBreakdownRows(ctx context.Context, rows []reports.FraudBreakdownRowDTO) []reports.FraudBreakdownRowDTO {
	if maskLevelFromContext(ctx) == authz.MaskFull {
		return rows
	}
	out := make([]reports.FraudBreakdownRowDTO, len(rows))
	for i := range rows {
