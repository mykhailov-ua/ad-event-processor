...
func registerFraudBreakdownReport(h *reports.ReportsHTTPHandlers, mux *http.ServeMux) {
	...
	mux.HandleFunc("GET /api/v1/reports/fraud-breakdown", limit(permAny(perms, h.WrapReport("fraud-breakdown", func(w http.ResponseWriter, r *http.Request) { getFraudBreakdownReport(h, w, r) }))))
}

func getFraudBreakdownReport(h *reports.ReportsHTTPHandlers, w http.ResponseWriter, r *http.Request) {
	...
	if len(campaignIDs) == 0 {
		httpresponse.JSON(w, http.StatusOK, reports.FraudBreakdownReportResponse{
			Rows:      []reports.FraudBreakdownRowDTO{},
	...
	rows = scrubFraudBreakdownRows(r.Context(), rows)
...
func queryFraudBreakdownRows(
	...
	out := make([]reports.FraudBreakdownRowDTO, 0, limit)
	for clickhouseRows.Next() {
		var row reports.FraudBreakdownRowDTO
