...
//   - lifecycle.go: CancelCampaign / FinalizeCancelledCampaign with outbox in same Postgres txn.
//   - runtime subpackage: Postgres list/get/patch/publish without HTTP (Effects delegation).
...
//
// Invariants:
//   - Publish, budget, and status mutations enqueue outbox_events in the same Postgres transaction
