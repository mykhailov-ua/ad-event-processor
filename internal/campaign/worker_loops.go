...
func (w *PacingControllerWorker) Start(ctx context.Context, interval time.Duration) {
	...
	for {
		...
		case <-ctx.Done():
			return
		case <-ticker.C:
			if err := w.host.ClosedLoopPacingController(ctx, w.syncWorkers); err != nil && !errors.Is(err, ErrPostgresGateRejected) {
				slog.Error("closed-loop pacing controller run failed", "err", err)
			}
			if err := w.host.RunVPPPacingController(ctx); err != nil && !errors.Is(err, ErrPostgresGateRejected) {
