...
func TestFault_logEvacuatorCheckpointCrashRecovery(t *testing.T) {
	...
	evac, err := NewEvacuator(EvacuatorConfig{
		LogDir:         logDir,
	...
	record, err := NewEvacuatorCheckpointStore(checkpointPath).Load()
	if err != nil {
	...
	faultproof.Log(t, "log_evacuator_checkpoint_crash_recovery", map[string]string{
...
func TestFault_logEvacuatorS3UploadRetry(t *testing.T) {
	logDir := t.TempDir()
	checkpointPath := filepath.Join(t.TempDir(), "checkpoint")
	memory := NewMemoryStore()
	store := &faultInjectedStore{inner: memory}
	...
	faultproof.Log(t, "log_evacuator_s3_upload_retry", map[string]string{
		"subsystem":    "log_evacuator",
		"upload_calls": "2",
		"objects":      "1",
		"exactly_once": "true",
...
func TestFault_logEvacuatorRotationRaceRecovery(t *testing.T) {
	logDir := t.TempDir()
	checkpointPath := filepath.Join(t.TempDir(), "checkpoint")
	memory := NewMemoryStore()
	...
	faultproof.Log(t, "log_evacuator_rotation_race_recovery", map[string]string{
		"subsystem":          "log_evacuator",
...
func TestFault_logEvacuatorConcurrentStress(t *testing.T) {
	logDir := t.TempDir()
	checkpointPath := filepath.Join(t.TempDir(), "checkpoint")
	memory := NewMemoryStore()
	...
	for i := range 20 {
		...
		go func(index int) {
			defer wg.Done()
			name := "segment_stress_" + itoaEvacuator(index)
			writeReadySegment(t, logDir, name, []byte("payload-"+itoaEvacuator(index)))
		}(i)
	}
	wg.Wait()
	...
	faultproof.Log(t, "log_evacuator_concurrent_stress", map[string]string{
		"subsystem":  "log_evacuator",
		"goroutines": "20",
		"objects":    "20",
	})
...
func itoaEvacuator(value int) string {
	if value == 0 {
		return "0"
	}
	var buf [20]byte
