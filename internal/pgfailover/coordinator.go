...
func (c Config) normalized() Config {
	...
	if out.Coord.LeaseTTL <= 0 {
		out.Coord = broker.DefaultCoordConfig()
	}
	return out
}
...
func NewCoordinator(cfg Config, promoter Promoter, health HealthCheck) (*Coordinator, error) {
	...
	host := NewCoordHost()
	coord, err := broker.NewCoordinatorWithConfig(cfg.NodeID, "pgfailover:"+cfg.NodeID, cfg.RedisURL, host, cfg.Coord)
	if err != nil {
		return nil, err
	}
	return &Coordinator{
