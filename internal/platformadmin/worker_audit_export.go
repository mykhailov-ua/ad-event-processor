...
type AuditExportHost interface {
	Pool() *pgxpool.Pool
}
...
func NewAuditExportWorker(host AuditExportHost, exportPath string, retentionDays int) *AuditExportWorker {
	if retentionDays <= 0 {
		retentionDays = 90
	}
...
func (w *AuditExportWorker) CleanupOldExports(now time.Time) error {
	return w.cleanupOldExports(now)
}
