...
func ReadBudgetInvariants(ctx context.Context, pool *pgxpool.Pool, redisClient redis.Cmdable, campaignIDs []uuid.UUID) (map[uuid.UUID]BudgetInvariantSnapshot, error) {
	...
	for i, id := range campaignIDs {
		budgetCmds[i] = pipe.Get(ctx, shard.BudgetCampaignKey(id))
		syncCmds[i] = pipe.Get(ctx, shard.CampaignSyncKey(id))
	}
	...
	for i, id := range campaignIDs {
		...
		if errors.Is(err, redis.Nil) {
			...
			return nil, fmt.Errorf("read %s: %w", shard.CampaignSyncKey(id), err)
		}
		...
		if errors.Is(err, redis.Nil) {
			...
			return nil, fmt.Errorf("read %s: %w", shard.BudgetCampaignKey(id), err)
		}
