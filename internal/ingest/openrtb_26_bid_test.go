...
func TestRunOpenRTBExchange_bcatBlocksWinner(t *testing.T) {
	...
	out := runOpenRTBExchangeParsedOutcome(proc, &p.OpenRTB26Hot, &p.OpenRTB26Cold, []byte("bid-1"), "8.8.8.8", openrtb.ExchangeConfig{MultiImpMax: 1}, nil, &evt)
...
func TestRunOpenRTBExchange_multiImp(t *testing.T) {
	...
	var evt domain.Event
	out := runOpenRTBExchangeParsedOutcome(proc, &p.OpenRTB26Hot, &p.OpenRTB26Cold, []byte("bid-multi"), "8.8.8.8", openrtb.ExchangeConfig{MultiImpMax: 10}, &admBuf, &evt)
	require.True(t, out.HasBid, "reason=%v", out.NoBid)
...
func TestRunOpenRTBExchange_bseatBlocks(t *testing.T) {
	...
	out := runOpenRTBExchangeParsedOutcome(proc, &p.OpenRTB26Hot, &p.OpenRTB26Cold, []byte("bid-1"), "8.8.8.8", exCfg, nil, &evt)
	assert.False(t, out.HasBid)
}
...
func TestRunOpenRTBExchange_wseatDealMatch(t *testing.T) {
	...
	out := runOpenRTBExchangeParsedOutcome(proc, &p.OpenRTB26Hot, &p.OpenRTB26Cold, []byte("bid-ws"), "8.8.8.8", exCfg, nil, &evt)
	require.True(t, out.HasBid, "reason=%v", out.NoBid)
	...
	out2 := runOpenRTBExchangeParsedOutcome(proc, &p2.OpenRTB26Hot, &p2.OpenRTB26Cold, []byte("bid-ws"), "8.8.8.8", openrtb.ExchangeConfig{MultiImpMax: 1, SeatID: []byte("other")}, nil, &evt)
