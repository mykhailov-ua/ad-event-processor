...
func (w *CampaignUpdateWatcher) consumeOnce(ctx context.Context) error {
	...
	for {
		...
		for iter.Next() {
			...
			id := uuid.UUID{}
			if !parser.ParseUUID(payload, &id) {
...
func (w *SlotMapWatcher) consumeBrokerOnce(ctx context.Context) error {
	...
	for {
		...
		for iter.Next() {
			got = true
			msg, err := shard.DecodeSlotMapReloadMessage(iter.Payload)
			if err != nil {
...
func (w *SlotMapWatcher) tryReload(ctx context.Context, source string) {
	version, changed, err := shard.ReloadStaticSlotMapIfChanged(ctx, w.cfg.Pool, w.cfg.Sharder, w.cfg.NumShards)
	if err != nil {
