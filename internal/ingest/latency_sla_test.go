...
func TestUnifiedFilter_LatencySLA(t *testing.T) {
	...
	beforeBudget1, _ := redisClient.Get(ctx, budgetSourceKey).Int64()
	err := f.Check(ctx, evt1)
	assert.NoError(t, err)
	afterBudget1, _ := redisClient.Get(ctx, budgetSourceKey).Int64()
	...
	beforeBudget3, _ := redisClient.Get(ctx, budgetSourceKey).Int64()
	err = f.Check(ctx, evt3)
	assert.NoError(t, err)
	afterBudget3, _ := redisClient.Get(ctx, budgetSourceKey).Int64()
