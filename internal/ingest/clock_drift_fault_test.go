...
func shiftCachedWallClock(d time.Duration) (restore func(), err error) {
	...
	before := cachedUnixMilliLoad()
	shifted := before + d.Milliseconds()
	cachedUnixMilliStore(shifted)
	cachedUnixMilliAnyStore(shifted)
	cachedNowUTCSetFromUnixMilli(shifted)

	return func() {
		cachedUnixMilliStore(before)
		cachedUnixMilliAnyStore(before)
		storeCachedNowUTC()
