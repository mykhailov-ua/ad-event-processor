...
type TLSFingerprintFeedLoader struct {
	loader *tlsFingerprintFeedLoader
}
...
func NewTLSFingerprintFeedLoader(cfg *config.Config, table *TLSFingerprintTable) *TLSFingerprintFeedLoader {
	...
	return &TLSFingerprintFeedLoader{loader: &tlsFingerprintFeedLoader{
		dir:     dir,
...
func (l *tlsFingerprintFeedLoader) refreshOnce() {
	...
	if corpus := filter.LoadH2FrameTraceCorpusFromDir(l.dir); corpus != nil {
		filter.PublishH2FrameTraceCorpus(corpus)
	}
...
func UAClaimsChromeNotChromium(ua string) bool {
	return filter.UAClaimsChromeNotChromium(ua)
}
