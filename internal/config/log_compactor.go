...
func LoadLogCompactor() (LogCompactor, error) {
	cfg := LogCompactor{
		...
		WarmDir:                  envOrDefault("LOG_COMPACTOR_WARM_DIR", "/var/lib/ad-event-processor/log-store/warm"),
		CheckpointPath:           envOrDefault("LOG_COMPACTOR_CHECKPOINT_PATH", "/var/lib/ad-event-processor/log-compactor.checkpoint.jsonl"),
		...
		ColdCheckpointPath:       envOrDefault("LOG_COMPACTOR_COLD_CHECKPOINT_PATH", "/var/lib/ad-event-processor/log-compactor-cold.checkpoint.jsonl"),
		...
		S3ScratchDir:             envOrDefault("LOG_COMPACTOR_S3_SCRATCH_DIR", "/var/lib/ad-event-processor/log-compactor/scratch"),
	...
	if cfg.SourceDir == "" {
		cfg.SourceDir = "/var/log/ad-event-processor"
	}
	if cfg.SampleRate <= 0 {
