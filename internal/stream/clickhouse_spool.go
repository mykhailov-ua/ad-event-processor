...
var ErrCHSpoolMaxSegments = errCHSpoolMaxSegments

type ClickHouseSpoolConfig struct {
	SegmentSizeBytes int64
	MaxSegments      int
}

func DefaultClickHouseSpoolConfig() ClickHouseSpoolConfig {
	return ClickHouseSpoolConfig{
...
func OpenClickHouseSpool(dir string) (*ClickHouseSpool, error) {
	return OpenClickHouseSpoolWithConfig(dir, DefaultClickHouseSpoolConfig())
}

func OpenClickHouseSpoolWithConfig(dir string, cfg ClickHouseSpoolConfig) (*ClickHouseSpool, error) {
	if dir == "" {
	...
	spool := &ClickHouseSpool{dir: dir, cfg: cfg}
...
func (s *ClickHouseSpool) flusherLoop(interval time.Duration) {
...
func (s *ClickHouseSpool) Sync() error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *ClickHouseSpool) AppendDurably(dedupToken string, events []*domain.Event) error {
	...
	start := time.Now()
	defer func() {
		metrics.ClickHouseSpoolAppendDurationSeconds.Observe(time.Since(start).Seconds())
	}()
	...
	if err := appendFn(payload, recordLen); err != nil {
		...
		metrics.ClickHouseSpoolRotateTotal.Inc()
		return appendFn(payload, recordLen)
	}
	return nil
}

func (s *ClickHouseSpool) appendLocked(payload []byte, recordLen int) error {
	seg := s.active
	pos := seg.writePos
...
func (s *ClickHouseSpool) appendLockedAsync(payload []byte, recordLen int) error {
	seg := s.active
	pos := seg.writePos
	if pos+int64(recordLen) > int64(len(seg.mmap)) {
		return errCHSpoolFull
...
func MarshalCHSpoolPayload(dedupToken string, events []*domain.Event) ([]byte, error) {
	return marshalCHSpoolPayload(dedupToken, events)
}
...
func (s *ClickHouseSpool) scanPathLocked(path string) ([]CHSpoolRecord, error) {
...
func (s *ClickHouseSpool) removeRotatedLocked(path string) error {
...
func (s *ClickHouseSpool) TruncatePrefix(endOffset int64) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	return s.truncateActiveLocked(endOffset)
}

func (s *ClickHouseSpool) truncateActiveLocked(endOffset int64) error {
...
func (s *ClickHouseSpool) SegmentCount() int {
	s.mu.Lock()
	defer s.mu.Unlock()
	return len(s.rotated) + 1
}

func (s *ClickHouseSpool) OpenFDCount() int {
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.active != nil && s.active.file != nil {
		return 1
...
func (s *ClickHouseSpool) Close() error {
	s.mu.Lock()
...
func (s *ClickHouseSpool) WritePos() int64 {
