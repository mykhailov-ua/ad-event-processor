...
const (
	...
	fraudAggMaxProbe        = 64
	fraudAggregateEventType = "fraud_aggregate"
)
...
func fraudAggregateExempt(evt *domain.Event) bool {
	...
	if fraudReasonContainsCode(evt.FraudReason, filter.FraudReasonCodeL3Blocklist) {
		return true
	}
...
func countL1HighSignalsInReason(reason string) int {
	n := 0
	for id := filter.FraudReasonID(1); id < filter.FraudReasonID(filter.FraudReasonCount); id++ {
		if filter.FraudSignalFlags(id)&filter.FraudSignalL1High == 0 {
			continue
		}
		if fraudReasonContainsCode(reason, filter.FraudReasonCode(id)) {
			n++
		}
	}
	return n
...
func primaryFraudReasonID(reason string) uint8 {
	...
	for id := filter.FraudReasonID(1); id < filter.FraudReasonID(filter.FraudReasonCount); id++ {
		code := filter.FraudReasonCode(id)
...
func (q *FraudStreamWriter) FlushAggregatesForTest(final bool) {
	q.flushAggregates(final)
}

func (q *FraudStreamWriter) SeedAggSlotForTest(idx int, subnet uint32, reasonID uint32, count uint64) {
	if q == nil || idx < 0 || idx >= len(q.aggSlots) {
		return
	}
	cell := &q.aggSlots[idx]
	cell.prefixKind.Store(uint32(fraudAggPrefixV4))
	cell.subnetPrefix.Store(subnet)
	cell.reasonID.Store(reasonID)
	cell.count.Store(count)
}
...
func (q *FraudStreamWriter) flushAggregates(final bool) {
	...
	if q.useBroker && q.brokerSink != nil {
		...
		wraps := make([]*codec.ByteSliceValue, 0, len(entries))
...
func fillFraudAggregateValues(e fraudAggFlushEntry, windowMs int64, valSlice []any) {
	...
	reasonStr := filter.FraudReasonCode(filter.FraudReasonID(e.reason))
