...
func Test_fraudAggregateExempt_L3(t *testing.T) {
	evt := &domain.Event{FraudReason: filter.FraudReasonCodeL3Blocklist}
	assert.True(t, fraudAggregateExempt(evt))
}

func Test_fraudAggregateExempt_dualL1(t *testing.T) {
	evt := &domain.Event{
		FraudReason: filter.FraudReasonCodeDatacenterIP + "," + filter.FraudReasonCodeLowTTC,
	}
	assert.True(t, fraudAggregateExempt(evt))
}

func Test_fraudAggregateExempt_singleL2(t *testing.T) {
	evt := &domain.Event{FraudReason: filter.FraudReasonCodeMissingImpTS}
...
func TestFraudStreamWriter_aggregateIncrementsWithoutRing(t *testing.T) {
	...
	evt := &domain.Event{
		IP:          "10.20.30.40",
		FraudReason: filter.FraudReasonCodeLowTTC,
		Type:        "click",
	}
...
func TestFraudStreamWriter_L3NeverAggregated(t *testing.T) {
	...
	evt := &domain.Event{
		...
		CampaignID:  uuid.New(),
		IP:          "10.0.0.1",
		FraudReason: filter.FraudReasonCodeL3Blocklist,
		Type:        "click",
	}
	require.True(t, q.Enqueue(0, evt))
...
func TestFault_FraudStreamL3NeverAggregated(t *testing.T) {
	...
	l2 := &domain.Event{
		IP:          "198.18.1.2",
		FraudReason: filter.FraudReasonCodeMissingImpTS,
		Type:        "click",
	}
...
func TestFraudStreamWriter_aggregateFlushToStream(t *testing.T) {
	...
	assert.Equal(t, filter.FraudReasonCodeLowTTC, redisClient.lastArgs["fraud_reason"])
...
func TestFraudStreamWriter_aggregateTableOverflowIncrementsDropped(t *testing.T) {
	...
	for i := range q.aggSlots {
		...
		cell.reasonID.Store(uint32(filter.FraudReasonLowTTC))
		cell.count.Store(1)
	}
