...
//   - broker_consumer.go: BrokerStreamConsumer reads WAL batches, optional shadow mode, ClickHouse StoreBatch with breaker.
...
//   - Consumer runs in cmd/processor; BROKER_SHADOW_MODE=1 (cfg.ShadowMode) counts without ClickHouse commit or offset advance.
...
//   - Shadow mode increments ad_broker_shadow_messages_total; live mode commits offsets after durable ClickHouse write.
//   - Corrupt broker payloads skipped with metric; consumer breaker opens on repeated StoreBatch failure.
//
// Tradeoffs:
//   - Broker ring vs Redis XADD: mmap WAL + partition HA replaces Redis stream RAM and PEL for ClickHouse ingest when
...
//   - Shadow cutover: BROKER_SHADOW_MODE=1 observes WAL without ClickHouse commit or offset advance; ad_broker_ingest_divergence_high
...
//   - Claiming broker-primary ClickHouse ingest from mockBrokerClient unit tests alone.
//
// Verify:
//
