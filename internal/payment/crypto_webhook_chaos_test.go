...
// TestChaos_CryptoWebhookReplay guards webhook replay x3 credits balance_ledger exactly once.
func TestChaos_CryptoWebhookReplay(t *testing.T) {
	if testing.Short() {
		t.Skip("chaos integration test")
	}
	...
	assertPaymentChaosInvariants(t, infra.Pool, seed, amountMicro, 1)

	logChaosProof(t, "crypto_webhook_replay", map[string]string{
		"subsystem":     "payment_crypto_webhook",
		"replays":       itoaPaymentChaos(replays),
		"proposal_rows": "1",
		"ledger_rows":   "1",
		"baseline_ok":   "true",
	})
