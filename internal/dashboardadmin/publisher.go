...
func NewPublisherService(host PublisherHost) *PublisherService {
	return &PublisherService{host: host}
}

func (st *PublisherService) ResolvePublisherBind(ctx context.Context, userID uuid.UUID) (PublisherBind, error) {
	pool := st.host.Pool()
	if pool == nil {
...
func (st *PublisherService) GetPublisherDashboard(ctx context.Context, bind PublisherBind, from, to time.Time) (PublisherDashboardDTO, error) {
	...
	clickhouseQuery := st.host.ClickHouseQuery()
...
func (st *PublisherService) ListPublisherStatements(ctx context.Context, bind PublisherBind, from, to time.Time, limit, offset int32) ([]PublisherStatementDTO, int64, error) {
	pool := st.host.Pool()
	if pool == nil {
...
func (st *PublisherService) ValidateSupplyFiles(ctx context.Context) (SupplyValidationReport, error) {
	out := SupplyValidationReport{Issues: []string{}}
	sellersBody, err := st.host.BuildSellersJSON(ctx)
	if err != nil {
	...
	adsTxt, err := st.host.BuildAdsTxt(ctx)
	if err != nil {
