...
//
// Topology:
//   - Wired via dashboardadmin_bridge.go; RoleHost, RoleReportHost, CampaignHost, PortfolioHost, and PublisherHost ports read Postgres, ClickHouse, and ledger via injected deps.
...
//   - ClickHouse queries respect ReportCHTimeout from Host; empty campaign scope returns zeroed metrics not errors.
...
//	go test ./internal/dashboardadmin/ -short -run TestApplyCampaignReportBreakdownQuery -count=1
//	go test ./internal/dashboardadmin/ -short -run TestParseCampaignReportDimension -count=1
