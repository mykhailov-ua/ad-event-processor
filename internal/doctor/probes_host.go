...
func (k KernelProbe) Name() string { return "kernel" }

func (k KernelProbe) Run(ctx context.Context) Result {
	start := time.Now()
...
func (sy SysctlProbe) Name() string { return "sysctl" }

func (sy SysctlProbe) Run(ctx context.Context) Result {
	start := time.Now()
