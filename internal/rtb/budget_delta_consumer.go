...
type BudgetDeltaRecorder interface {
	Record(campaignID uuid.UUID, amountMicro int64)
}

type BudgetDeltaConsumerConfig struct {
	BrokerAddr string
	RedisURL   string
	Topic      string
	Partition  uint16
...
type BudgetDeltaConsumer struct {
	aggregator BudgetDeltaRecorder
	cfg        BudgetDeltaConsumerConfig
	cli        *client.Client
	cancel     context.CancelFunc
	wg         sync.WaitGroup
}

func NewBudgetDeltaConsumer(agg BudgetDeltaRecorder, cfg BudgetDeltaConsumerConfig) *BudgetDeltaConsumer {
