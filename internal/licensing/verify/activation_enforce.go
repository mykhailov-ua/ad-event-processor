...
func activationLicenseKey(claims *entitlements.LicenseClaims) string {
	if claims == nil {
		return ""
	}
...
func CheckHostActivation(ctx context.Context, pool *pgxpool.Pool, claims *entitlements.LicenseClaims, fingerprint string) error {
	if claims == nil {
	...
	maxAct := entitlements.NormalizeMaxActivationsLimit(claims.Limits)
...
func ensureVendorLicenseRow(ctx context.Context, pool *pgxpool.Pool, claims *entitlements.LicenseClaims, licenseKey string) error {
	limitsJSON, err := json.Marshal(claims.Limits)
	if err != nil {
		return err
	}
	featuresJSON, err := json.Marshal(entitlements.SanitizeFeaturesForSKU(claims.SKU, claims.Features))
	if err != nil {
		return err
	}
	maxAct := entitlements.NormalizeMaxActivationsLimit(claims.Limits)
