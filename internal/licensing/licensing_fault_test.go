...
func TestLicenseSpool_BufferOverflowRejected(t *testing.T) {
	...
	logLicensingFaultProof(t, "license_spool_buffer_overflow", map[string]string{
		"subsystem":    "licensing",
		"segment_size": "4096",
		"rejected":     "true",
	})
...
func TestFault_LicenseSpoolConcurrentAppend(t *testing.T) {
	...
	logLicensingFaultProof(t, "license_spool_concurrent_append", map[string]string{
		"subsystem": "licensing",
		"workers":   "24",
		"recovered": "true",
	})
...
func logLicensingFaultProof(t *testing.T, fault string, kv map[string]string) {
	t.Helper()
	msg := "fault_proof fault=" + fault
	for k, v := range kv {
		msg += " " + k + "=" + v
	}
	t.Log(msg)
