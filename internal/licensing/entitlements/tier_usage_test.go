...
func TestTierUsageWarnings_campaignCap(t *testing.T) {
	...
	w := TierUsageWarnings(limits, TierUsageCounts{}, 10, StateActive, time.Time{}, time.Now(), 7)
	assert.Contains(t, w[0], "cap reached")

	w = TierUsageWarnings(limits, TierUsageCounts{}, 8, StateActive, time.Time{}, time.Now(), 7)
...
func TestTierUsageWarnings_renewalWindow(t *testing.T) {
	...
	w := TierUsageWarnings(Limits{}, TierUsageCounts{}, 0, StateActive, validUntil, now, 7)
	assert.Len(t, w, 1)
	assert.Contains(t, w[0], "renews in 5 day")
}

func TestTierUsageWarnings_graceState(t *testing.T) {
	w := TierUsageWarnings(Limits{}, TierUsageCounts{}, 0, StateGrace, time.Time{}, time.Now(), 7)
