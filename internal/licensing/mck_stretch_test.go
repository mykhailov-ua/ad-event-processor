...
func TestArgonRecheckStretch_GoldenVector(t *testing.T) {
	...
	for _, fx := range doc.MCKStretchV1 {
		t.Run(fx.Name, func(t *testing.T) {
			...
			work, err := licensing.StretchMCKForRecheck(mck, fx.DeploymentID)
			require.NoError(t, err)
			require.Equal(t, fx.MCKWorkHex, hex.EncodeToString(work[:]))
			seed := licensing.FeatureSeedFromMCK(work)
...
func TestArgonRecheckStretch_DeriveMCKWorkForRecheck(t *testing.T) {
	...
	work, err := licensing.DeriveMCKWorkForRecheck(doc.Fixtures[0].Token, doc.Fixtures[0].HWID)
	require.NoError(t, err)
...
func TestArgonRecheckStretch_UnstretchedMCKUnchanged(t *testing.T) {
	...
	require.Equal(t, fx.FeatureSeedHex, fmt.Sprintf("%08x", licensing.FeatureSeedFromMCK(mck)))

	work, err := licensing.DeriveMCKWorkForRecheck(fx.Token, fx.HWID)
	require.NoError(t, err)
...
func TestArgonRecheckStretch_DifferentDeploymentID(t *testing.T) {
	var mck [32]byte
	mck[0] = 1
	workA, err := licensing.StretchMCKForRecheck(mck, "dep-a")
	require.NoError(t, err)
	workB, err := licensing.StretchMCKForRecheck(mck, "dep-b")
	require.NoError(t, err)
