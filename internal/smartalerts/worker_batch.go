...
func (w *Worker) evaluateRulesBatch(ctx context.Context, clickhouseQuery *database.ClickHouseQuery, rules []smartAlertRuleRow, now time.Time) error {
	if len(rules) == 0 {
		return nil
	}
	...
	metricsByKey, err := w.loadAlertMetricsBatch(ctx, clickhouseQuery, metricKeys)
	if err != nil {
		return err
	}
	...
	for windowStart, idxs := range rulesByWindow {
		...
		rows, err := db.New(w.host.Pool()).ListExistingAlertRuleWindows(ctx, db.ListExistingAlertRuleWindowsParams{
			Column1:     ruleIDs,
...
func (w *Worker) batchCampaignIDsByCustomer(ctx context.Context, rules []smartAlertRuleRow) (map[uuid.UUID][]uuid.UUID, error) {
	...
	rows, err := db.New(w.host.Pool()).ListCampaignIDsByCustomers(ctx, customerIDs)
	if err != nil {
		return nil, err
	}
	for _, row := range rows {
...
func (w *Worker) loadAlertMetricsBatch(
	ctx context.Context,
	...
	for key, campaignSet := range metricKeys {
		...
		perCampaign, err := querySmartAlertMetricBatch(ctx, clickhouseQuery, key.metric, campaignIDs, key.start, key.end)
		if err != nil {
			return nil, err
		}
		out[key] = perCampaign
...
func (w *Worker) fireAlertRule(ctx context.Context, rule smartAlertRuleRow, windowStart, windowEnd time.Time, observed float64) error {
...
func querySmartAlertMetricBatch(
	...
	clickhouseCtx, cancel := context.WithTimeout(ctx, smartAlertCHTimeout)
	defer cancel()

	out := make(map[uuid.UUID]alertCampaignMetrics, len(campaignIDs))
	switch metric {
	case "clicks":
		rows, err := clickhouseQuery.Query(clickhouseCtx, smartAlertClicksByCampaignQuery, campaignIDs, from, to)
		if err != nil {
			return nil, err
		}
		...
		return out, rows.Err()
	case "bot_clicks":
		rows, err := clickhouseQuery.Query(clickhouseCtx, smartAlertBotClicksByCampaignQuery, campaignIDs, from, to)
		if err != nil {
			return nil, err
		}
		defer func() { _ = rows.Close() }()
		...
		return out, rows.Err()
	case "cr":
		if err := mergeClickCounts(clickhouseCtx, clickhouseQuery, smartAlertClicksByCampaignQuery, campaignIDs, from, to, out, false); err != nil {
			return nil, err
		}
		return out, mergeClickCounts(clickhouseCtx, clickhouseQuery, smartAlertConversionsByCampaignQuery, campaignIDs, from, to, out, true)
	case "roi_pct":
		rows, err := clickhouseQuery.Query(clickhouseCtx, smartAlertROIByCampaignQuery, campaignIDs, from, to)
		if err != nil {
			return nil, err
		}
		defer func() { _ = rows.Close() }()
...
func mergeClickCounts(
	...
	rows, err := clickhouseQuery.Query(ctx, query, campaignIDs, from, to)
	if err != nil {
		return err
	}
	defer func() { _ = rows.Close() }()
