...
/** Inline lander/docs snippet; HTTP-safe (getRandomValues). Parity with newRandomUuid() fallback path. */
export const LANDER_CONVERSION_EVENT_ID_LINE = `const conversionEventId = (function () {
  const bytes = new Uint8Array(16);
  if (typeof crypto !== 'undefined' && typeof crypto.getRandomValues === 'function') {
    crypto.getRandomValues(bytes);
  } else {
