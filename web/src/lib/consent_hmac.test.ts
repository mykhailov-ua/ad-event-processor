...
import { seedDeterministicUuid } from './uuid.ts';

const consentUserId = seedDeterministicUuid('user', 1);
...
test('buildConsentRecordJson omits empty timestamp', () => {
  assert.equal(
    ...
    `{"user_id":"${consentUserId}","purposes":1,"source":"cmp"}`
  );
});
...
test('buildConsentRecordJson includes timestamp when set', () => {
  ...
  assert.equal(
    body,
    `{"user_id":"${consentUserId}","purposes":2,"source":"admin","timestamp":"2026-09-07T12:00:00Z"}`
  );
