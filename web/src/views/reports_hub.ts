...
type ReportsHubState = {
  savedViews: SavedViewRow[];
  savedError: string | null;
  exportStatus: string | null;
  exportLoading: boolean;
  probing: boolean;
  probeStub: Record<string, boolean | null>;
};
 ...
 */
function renderReportRow(opts: any) {
  ...
  return el('tr', { 'data-report-key': opts.key },
    el('td', null,
      el('a', {
        ...
        renderIcon(reportIcon(opts.key), { size: 14, className: 'reports-hub__link-icon' }),
        opts.title,
      ),
    ),
    el('td', null,
...
export function mount(container: HTMLElement, ctx: RouteContext): ViewHandle {
  ...
  async function loadProbes() {
    ...
    const tasks = keys.map((key: any) => async () => {
    ...
    if (results) {
      for (let i = 0; i < results.length; i++) {
        ...
        state.probeStub[slot.key] = slot.stub;
      }
    }
    render();
  }
  ...
  async function loadSaved() {
    ...
    state.savedViews = (views ?? []) as SavedViewRow[];
    render();
  }

  let exportAbort: any = null;

  async function handleExport(reportKey: any) {
  ...
  function render() {
    ...
    const liveCount = REPORT_CATALOG.filter((c: any) => c.live).length;
        ...
        el('div', { className: 'settings-panel__body' },
          el('div', { className: 'table-wrapper table-wrapper--scroll' },
            el('table', { className: 'data-table' },
              ...
              el('tbody', null,
                REPORT_CATALOG.map((card: any) => renderReportRow({
                  key: card.key,
                  title: card.title,
                  live: card.live,
