...
export function mount(container: HTMLElement, ctx: RouteContext): ViewHandle {
  ...
  async function load() {
    ...
    const [res, err] = await to(api(`/api/v1/reports/ivt-by-source?${params.toString()}`));
    if (destroyed) return;
    loading = false;
    if (err) {
      error = err;
      ...
      if (!sessionScoped && cid) {
        ...
        window.history.replaceState(null, '', `/reports/ivt-by-source?${qs}`);
      }
    }
    render();
  }
    ...
    replaceChildren(container,
      ...
      el('form', {
        ...
        renderFormField({
          label: 'From',
          htmlFor: 'ivt-from',
          children: el('input', { id: 'ivt-from', className: 'form-input', value: from, onInput: (e: Event) => { from = eventTargetValue(e); } }),
        }),
        renderFormField({
          label: 'To',
          htmlFor: 'ivt-to',
          children: el('input', { id: 'ivt-to', className: 'form-input', value: rangeTo, onInput: (e: Event) => { rangeTo = eventTargetValue(e); } }),
        }),
