...
export type CustomerApiKeysPanelOpts = {
  canCreate: boolean;
};
 ...
 * @param {CustomerApiKeysPanelOpts} opts
 * @returns {{ destroy: () => void }}
 */
export function mountCustomerApiKeysPanel(container: HTMLElement, opts: CustomerApiKeysPanelOpts): ViewHandle {
  let destroyed = false;
  let name = '';
  let busy = false;
  let error: any = null;
  /** @type {{ destroy: () => void }|null} */
  let keyModal: any = null;
          ...
          ? el('div', { className: 'flex items-end gap-2 flex-wrap' },
            el('label', { className: 'form-field flex-1', htmlFor: 'api-key-name', style: { minWidth: '12rem' } },
              ...
              el('input', {
                ...
                value: name,
                disabled: busy,
                onInput: (e: Event) => { name = eventTargetValue(e); },
              }),
            ),
            renderButton({
