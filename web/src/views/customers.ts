...
type CustomersViewState = ResourceState<CustomerListResponse> & {
  page: number;
  searchQuery: string;
};
 ...
 * @returns {string}
 */
function formatBalance(bal: any) {
  if (!bal) return '—';
...
export function mount(container: HTMLElement, ctx: RouteContext): ViewHandle {
  ...
  const state: CustomersViewState = { data: null, loading: true, error: null, page: 0, searchQuery: '' };
  ...
  function render() {
    ...
    mountFilterToolbar(toolbarWrap, {
      ...
      pagination: totalPages > 1
        ? renderPaginationBar({
          label: `${state.page + 1} / ${totalPages}`,
          prevDisabled: state.page === 0,
          nextDisabled: state.page >= totalPages - 1,
          onPrev: () => { state.page = Math.max(0, state.page - 1); resource.reload(); },
          onNext: () => { state.page += 1; resource.reload(); },
        })
        : null,
    });
  ...
  const resource = createResource<CustomerListResponse>(
    () => buildUrl(state.page),
    {
      onUpdate: (s) => {
