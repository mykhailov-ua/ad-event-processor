 ...
 * @param {string} color
 */
function drawDonutSlice(ctx, cx, cy, outerR, innerR, start, end, color) {
  ctx.beginPath();
 ...
 */
export function mountPieChart(container, items, ariaLabel = 'Donut chart') {
  ...
  rows.forEach((item, i) => {
    ...
    legend.appendChild(
      el('div', { className: 'chart-legend__item' },
        el('span', { className: 'chart-legend__swatch', style: `background:${colors[i]}` }),
