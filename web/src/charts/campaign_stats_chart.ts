...
import type { ChartHandle } from './area_chart.js';
 ...
 */
function yScaleInto(y: Float64Array, length: number, out: YScaleOut): void {
  let yMin = y[0];
  let yMax = y[0];
...
export function mountChart(
  ...
  const yScaleOut: YScaleOut = { yMin: 0, yMax: 1, invYRange: 1 };
