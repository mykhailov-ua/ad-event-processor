      ...
      <CreativeDirectoryStack>
        <section className="grid gap-2">
          ...
          {previewAdsTxtError
            ...
            : null}
          {previewSellersJson ? (
            <pre className="max-h-64 overflow-auto border bg-muted/40 p-3 text-xs whitespace-pre-wrap">
              {previewSellersJson}
            </pre>
          ) : null}
          {previewAdsTxt ? (
          ...
          {exportPath?.path ? (
            <p className="text-sm text-muted-foreground">
              Nginx export path: <span className="text-xs text-foreground">{exportPath.path}</span>
            </p>
          ) : null}
        </section>
            ...
            <TableHost>
              <DirectoryTable nested>
                ...
                <TableBody>
                  {sellers.map((row) => (
                    <TableRow key={row.id}>
                      <TableCell className="text-xs">{row.seller_id}</TableCell>
              ...
              <DirectoryTable nested>
                ...
                <TableBody>
                  {adsTxt.map((row) => (
                    <TableRow key={row.id}>
                      <TableCell>{row.domain}</TableCell>
                      <TableCell className="text-xs">{row.publisher_account_id}</TableCell>
