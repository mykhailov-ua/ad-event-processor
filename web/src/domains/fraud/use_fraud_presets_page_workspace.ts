...
export function useFraudPresetsPageWorkspace() {
  ...
  const onPresetDraftChange = useCallback((name: string, patch: Partial<FraudPresetEditDraft>) => {
    setPresetDrafts((prev) => ({
      ...prev,
      [name]: {
        pass: prev[name]?.pass ?? '',
  ...
  const onSavePreset = useCallback(
    async (name: string) => {
      ...
      if (body.pass == null && body.suspect == null && body.ivt == null && body.block == null) {
  ...
  return {
    ...
    savingPresetName,
    onPresetDraftChange,
    onSavePreset: (name: string) => void onSavePreset(name),
  };
}
