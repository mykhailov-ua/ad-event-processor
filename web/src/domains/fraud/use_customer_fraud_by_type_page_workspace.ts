// L3 customer fraud by type report: URL filters and offset pagination.
import { useCallback, useEffect, useMemo, useState } from 'react';

import { getCustomerFraudByTypeReport } from '@/api/reports_api';
import type { CustomerFraudByTypeRow } from '@/api/types';
...
export function useCustomerFraudByTypePageWorkspace() {
  ...
  const { data, error, fetching, revalidating: listRevalidating } = useResource(
    (signal) => {
      ...
      return getCustomerFraudByTypeReport(
        {
  ...
  const rows: CustomerFraudByTypeRow[] = data?.rows ?? [];
  const totalEstimate = appliedOffset + rows.length + (data?.next_cursor ? appliedLimit : 0);
  ...
  return {
    title: 'Fraud by type',
    description: 'Customer-facing fraud categories and share of events.',
    rows,
