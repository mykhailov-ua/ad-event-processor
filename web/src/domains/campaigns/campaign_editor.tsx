...
import { CampaignEditorTools } from '@/domains/campaigns/campaign_editor_tools';
...
function displayOrWire(display: string | undefined, wire: string): string {
...
export function CampaignEditor({
  ...
  const budgetDisplay = displayOrWire(
    displayCampaign.budget_limit_display,
    campaign.budget_limit,
  );
  const spendDisplay = displayOrWire(
    displayCampaign.current_spend_display,
    campaign.current_spend,
  );
  const dailyBudgetDisplay = displayOrWire(
    displayCampaign.daily_budget_display,
    campaign.daily_budget,
  );
  const statusLabel = displayCampaign.status_label ?? campaign.status;
  ...
  return (
    ...
    >
      <p className="flex flex-wrap items-center gap-x-1 gap-y-2 text-sm text-muted-foreground">
        <Link className="text-primary hover:underline" to="/campaigns">
          Campaigns
        </Link>
        <span aria-hidden="true">*</span>
        <Link className="text-primary hover:underline" to={`/dashboards/campaign/${campaign.id}`}>
          Campaign dashboard
        </Link>
        <span aria-hidden="true">*</span>
        ...
        {publishCheck && !checking ? (
          <ValidityBadge
            ...
            validLabel="Publish ready"
            invalidLabel="Publish blocked"
          />
        ) : null}
      </p>
      ...
      {saveError ? (
        saveError instanceof ApiError && saveError.status === 501 ? (
          <StubBanner title="Save not available" message={saveError.message} />
        ) : (
          <ErrorBlock title="Could not save campaign" message={saveError.message} />
        )
      ) : null}
      ...
      <Card>
        ...
        <CardContent className="grid gap-4">
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            <div className="grid gap-2">
              <Label htmlFor="campaign-budget-limit">Budget limit</Label>
              <Input
              ...
              <p className="text-xs text-muted-foreground">
                Display: {formatReadonly(budgetDisplay)}
              </p>
            </div>
            <div className="grid gap-2">
              <Label>Daily budget</Label>
              <p className="text-sm">{formatReadonly(dailyBudgetDisplay)}</p>
            </div>
          </div>
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            <div className="grid gap-2">
              ...
              <p className="text-sm">{formatReadonly(spendDisplay)}</p>
            </div>
            <div className="grid gap-2">
              <Label htmlFor="campaign-pacing-mode">Pacing mode</Label>
              <Input
                id="campaign-pacing-mode"
                value={form.pacing_mode}
                disabled={saving}
                onChange={(event) => onFieldChange('pacing_mode', event.target.value)}
              />
            </div>
          </div>

          <div className="flex justify-end">
            <Button type="button" disabled={saving || fetching} onClick={onSave}>
              {saving ? 'Saving...' : 'Save'}
            </Button>
          </div>
        </CardContent>
      ...
      <Card>
        ...
        <CardContent className="grid gap-4">
          ...
          {macroPreviewResult ? (
            <div className="ui-surface grid gap-3 p-3">
              <div className="grid gap-2">
      ...
      <Card>
        ...
        <CardContent className="grid gap-4">
          ...
          {publishBlocked ? (
            <div className="ui-surface grid gap-3 border border-destructive/50 p-3">
              <div className="flex flex-wrap items-center gap-2">
