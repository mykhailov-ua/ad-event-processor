...
export type DashboardChartSeriesStyle = {
  ...
  label: string;
  axis: 'volume' | 'money';
  chartToken: DashboardChartToken;
  stroke: string;
  fill: string;
};

export const DASHBOARD_VOLUME_AXIS_COLOR = 'hsl(var(--chart-1) / 0.95)';
export const DASHBOARD_MONEY_AXIS_COLOR = 'hsl(var(--chart-4) / 0.95)';

function chartStroke(token: DashboardChartToken, alpha = 0.78): string {
  return `hsl(var(--chart-${token}) / ${alpha})`;
}

function chartFill(token: DashboardChartToken, alpha = 0.12): string {
  return `hsl(var(--chart-${token}) / ${alpha})`;
}
