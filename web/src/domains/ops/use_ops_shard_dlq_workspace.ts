...
import { listOpsDlq, retryOpsDlqEntry } from '@/api/ops_api';
...
const CURSOR_STACK_KEY = 'shard_cursor_stack';
...
export function useOpsShardDlqWorkspace() {
  ...
  const limit = parseListLimit(searchParams.get('shard_limit'), 200);
  const cursor = searchParams.get('shard_cursor') ?? undefined;
  ...
  const updateCursors = useCallback(
    (nextCursor: string | undefined, nextStack: string[]) => {
      ...
      next.set('shard_limit', String(limit));
      if (nextCursor) {
        next.set('shard_cursor', nextCursor);
      } else {
        next.delete('shard_cursor');
      }
  ...
  const onRetry = useCallback(
    async (entry: DLQEntry) => {
      if (!entry.id || retryingId != null) {
        return;
      }
      setRetryingId(entry.id);
      setRetryError(undefined);
      try {
        await retryOpsDlqEntry(entry.id);
