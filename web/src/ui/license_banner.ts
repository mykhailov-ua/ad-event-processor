...
export type LicenseBannerOpts = {
  license: LicenseInfo;
};

/**
 * Render license / tier warning banners from GET /api/v1/meta license block.
 */
export function renderLicenseBanner(opts: LicenseBannerOpts): HTMLElement {
  ...
  if (healthyActive) return el('div');
  ...
  if (license.plan_code) {
    parts.push(`Plan: ${license.plan_code}`);
  }
  if (license.valid_until && (state === 'valid' || state === 'active' || state === 'grace')) {
    parts.push(`until ${license.valid_until}`);
  }
  if (renewDays != null && renewDays <= 7) {
    parts.push(`renew ${renewDays}d`);
  }
  ...
  return el('div', {
    ...
    el('span', {}, parts.join(' · ')),
    el('a', { href: '/settings/license', style: { color: 'var(--accent)', fontSize: '12px' } }, 'License'),
  );
}
