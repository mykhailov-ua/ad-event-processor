...
export function ReportQueryPage({ endpoint, title }: ReportQueryPageProps) {
  ...
  const fetchReport = useCallback(
    async (cursor = '') => {
      ...
      const [apiRes, apiErr] = await to(
        api(`/api/v1/reports/${endpoint}?${params.toString()}`, {
          signal: ctrl.signal,
        })
      );
              ...
              {tableRows.map((row, index) => {
                ...
                return (
                  <tr key={`${isPlacements ? row.placement_id : row.keyword}-${index}`}>
                    ...
                    {showFinancials ? (
                      <td className="font-mono">
                        {formatAmountMicro(Number(row.spend_micro ?? 0))}
                      </td>
                    ) : null}
                    {showFinancials ? (
                      <td className="font-mono">
                        {formatAmountMicro(Number(row.revenue_micro ?? 0))}
                      </td>
                    ) : null}
                    {showFinancials ? (
