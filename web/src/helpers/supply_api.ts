...
export async function fetchSupplyValidation(): Promise<
  import('../types/publisher.js').SupplyValidation
> {
  const { data } = await api('/api/v1/supply/validation');
  return data as import('../types/publisher.js').SupplyValidation;
}
