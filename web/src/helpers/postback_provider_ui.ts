...
/**
 * Provider-specific copy and field layout for postback / CAPI config.
 */
export const POSTBACK_PROVIDER_UI: Record<PostbackProvider, PostbackProviderUi> = {
  webhook: {
    label: 'Webhook (affiliate S2S)',
    blurb: 'GET request with macro-expanded query string. Use for affiliate networks and custom partners.',
    primaryLabel: 'URL template',
    primaryPlaceholder: 'https://partner.example/postback?cid={click_id}&payout={payout}',
    primaryHelp: 'Macros: {click_id}, {payout}, {tx_id}, {event_type}, {sub1}…{sub30}, {subid1}…{subid30}',
    ...
    eventMappingHint: 'Outbound webhook keeps BidShard event_type; map names in the partner template if needed.',
  },
  facebook: {
    label: 'Meta Conversions API',
    blurb: 'Server-side events to Graph API. Requires fbclid on the conversion (from click URL or /track payload).',
    primaryLabel: 'Pixel ID',
    primaryPlaceholder: '123456789012345',
    primaryHelp: 'Numeric Meta pixel ID. Stored as the postback template; Graph URL is built automatically.',
    tokenLabel: 'CAPI access token',
    tokenPlaceholder: 'EAA…',
    tokenHelp: 'System user or long-lived token with ads_management. Encrypted at rest; never shown after save.',
    requiresToken: true,
    showMacros: false,
    supportsTestEventCode: true,
    eventMappingHint: 'BidShard → Meta: conversion/purchase → Purchase; lead → Lead; install → CompleteRegistration; click → ViewContent.',
  },
  google: {
    ...
    tokenHelp: 'Short-lived OAuth 2.0 access token with https://www.googleapis.com/auth/adwords scope.',
    requiresToken: true,
    showMacros: false,
    supportsTestEventCode: false,
    eventMappingHint: 'Google uses the conversion action resource above; BidShard event_type selects when to fire, not the Ads action name.',
  },
  tiktok: {
    ...
    showMacros: false,
    supportsTestEventCode: true,
    eventMappingHint: 'BidShard → TikTok: conversion/purchase → CompletePayment; lead → Contact; install → Download; click → ClickButton.',
  },
};

/** Return known postback provider ids. */
export function postbackProviderIds(): PostbackProvider[] {
  return Object.keys(POSTBACK_PROVIDER_UI) as PostbackProvider[];
}

/**
 * Normalize a provider string to a known PostbackProvider.
 */
