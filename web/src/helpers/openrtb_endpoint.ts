/**
 * Build OpenRTB 2.6 bid endpoint URL for a tracking host.
 */
export function buildOpenRTBBidURL(host: string): string {
  const h = (host || '').replace(/^https?:\/\//, '').trim();
  if (!h) {
...
/**
 * Human-readable routing hint for SSP onboarding.
 */
...
/** Minimal OpenRTB 2.6 bid request for validate-bid smoke. */
