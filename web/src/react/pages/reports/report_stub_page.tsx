...
/**
 * Catch-all stub report page for planned / retired keys.
 */
    ...
    <div className="stack stack--lg" data-testid={`report-stub-${reportKey}`}>
      ...
      {!loading && isRetiredReport(reportKey) ? (
        <ImperativeDomHost
          ...
          deps={[title, retiredAltLink?.label]}
        />
      ) : null}

      {!loading && !isRetiredReport(reportKey) && probe?.stub ? (
        <>
          <ImperativeDomHost build={() => renderStubBanner({ message: copy.message })} deps={[copy.message]} />
      ...
      {exportStatus ? (
        <p id="stub-export-status" className="text-sm text-muted">{exportStatus}</p>
      ) : null}

      <ImperativeDomHost build={() => renderPerfBlock('report-stub-perf')} deps={[]} />
    </div>
  );
}
