...
import { renderPacingPanel } from '../../panels/pacing_panel.js';
...
import { mountCampaignTrackingPanel } from '../../panels/campaign_tracking_panel.js';
import { mountCampaignPostbackPanel } from '../../panels/campaign_postback_panel.js';
import { mountCampaignFiltersPanel } from '../../panels/campaign_filters_panel.js';
import { mountCampaignMarginGuardPanel } from '../../panels/campaign_margin_guard_panel.js';
...
import { ImperativeDomHost, LegacyPanelHost } from '../components/legacy_panel_host.js';
...
export function CampaignDetailPage() {
  ...
  const chartHandleRef = useRef<{ destroy: () => void } | null>(null);
  ...
  useEffect(() => {
    ...
    void import('../../charts/campaign_stats_chart.js').then((mod) => {
      if (cancelled || !chartMountRef.current) return;
      chartHandleRef.current?.destroy();
      chartHandleRef.current = mod.mountChart(chartMountRef.current, (stats.hourly ?? []) as HourlyMetricRow[]);
    });
    return () => {
      cancelled = true;
      chartHandleRef.current?.destroy();
  ...
  const mountTracking = useCallback(
    (host: HTMLElement) => mountCampaignTrackingPanel(host, { campaignId: id, navigate }),
    [id, navigate],
  );
  const mountPostbacks = useCallback(
    (host: HTMLElement) => mountCampaignPostbackPanel(host, {
      campaignId: id,
      canWrite: canWriteCampaign,
      navigate,
    }),
    [id, canWriteCampaign, navigate],
  );
  const mountFilters = useCallback(
    (host: HTMLElement) => mountCampaignFiltersPanel(host, {
      campaignId: id,
      referrerFilter: campaign?.referrer_filter ?? '',
      canWrite: canWriteCampaign,
      onSaved: () => reloadCampaign(),
    }),
    [id, campaign?.referrer_filter, canWriteCampaign, reloadCampaign],
  );
  const mountMargin = useCallback(
    (host: HTMLElement) => mountCampaignMarginGuardPanel(host, {
      campaignId: id,
      canWrite: canWriteCampaign,
    }),
    [id, canWriteCampaign],
  );
  const mountTelegram = useCallback(
    (host: HTMLElement) => mountCampaignTelegramPanel(host, {
      campaignId: id,
      canWrite: canWriteCampaign,
    }),
    [id, canWriteCampaign],
  );
  const mountCreative = useCallback(
    (host: HTMLElement) => mountCampaignBrandCreativesPanel(host, {
    ...
    [campaign?.brand_id, campaign?.customer_id, canWriteCampaign],
  );

  const buildCommercialMetrics = useCallback(
    () => renderCommercialMetrics(dashboard.data?.kpis, { masked }),
    [dashboard.data?.kpis, masked],
  );

  const buildPacingPanel = useCallback(() => {
    if (!campaign) return document.createElement('div');
    return renderPacingPanel({
  ...
  }, [campaign, overviewImpressions7d]);

  const buildFreshnessBadge = useCallback(
    () => renderFreshnessBadge({ stale: stats?.stale }),
    [stats?.stale],
  );
    ...
    <>
      ...
      {tab === 'overview' ? (
        <div className="section-block stack">
          ...
          {!dashboard.loading ? (
            <ImperativeDomHost build={buildCommercialMetrics} deps={[dashboard.data, masked]} />
          ) : null}
          {!masked && campaign ? (
            <Button
          ...
          {masked ? (
            <ImperativeDomHost build={buildPacingPanel} deps={[campaign, overviewImpressions7d]} />
          ) : (
            <div className="grid-stats section-block">
          ...
          {stats ? (
            <div className="stack">
              <div className="flex items-center gap-2">
                ...
                <ImperativeDomHost build={buildFreshnessBadge} deps={[stats.stale]} />
              </div>
      ...
      {!masked && tab === 'tracking' ? (
        <div className="section-block">
          <LegacyPanelHost active mount={mountTracking} deps={[id]} />
        </div>
      ) : null}

      {!masked && tab === 'postbacks' ? (
        <div className="section-block">
          <LegacyPanelHost active mount={mountPostbacks} deps={[id, canWriteCampaign]} />
        </div>
      ) : null}

      {!masked && tab === 'filters' ? (
        <div className="section-block">
          <LegacyPanelHost active mount={mountFilters} deps={[id, campaign.referrer_filter, canWriteCampaign]} />
        </div>
      ) : null}

      {!masked && tab === 'margin' ? (
        <div className="section-block">
          <LegacyPanelHost active mount={mountMargin} deps={[id, canWriteCampaign]} />
        </div>
      ) : null}
      ...
      {!masked && tab === 'creative' ? (
        <div className="section-block">
          <div className="stack">
            ...
            />
            <LegacyPanelHost active mount={mountCreative} deps={[campaign.brand_id, campaign.customer_id, canWriteCampaign]} />
          </div>
        </div>
      ) : null}
      ...
      {!masked && tab === 'telegram' ? (
        <div className="section-block">
          <LegacyPanelHost active mount={mountTelegram} deps={[id, canWriteCampaign]} />
        </div>
      ) : null}
    </>
  );
