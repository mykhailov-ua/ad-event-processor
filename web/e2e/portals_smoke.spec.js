...
test('self-serve invoices load from GET /api/v1/selfserve/invoices', async ({ page }) => {
  ...
  const listResponse = page.waitForResponse(isApiGet('/api/v1/selfserve/invoices'), { timeout: 20_000 });
...
test('publisher dashboard read contract on GET /api/v1/publisher/dashboard', async ({ page }) => {
  ...
  if (response.status() === 403) {
    await expect(page.getByText('Could not load publisher dashboard')).toBeVisible({ timeout: 15_000 });
    return;
  }
...
test('report schedules load from GET /api/v1/report-schedules', async ({ page }) => {
  await loginAsAdmin(page);
  const listResponse = page.waitForResponse(isApiGet('/api/v1/report-schedules'), { timeout: 20_000 });
