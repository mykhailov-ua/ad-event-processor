...
test('cost sync run posts to API and refreshes snapshot', { tag: '@write' }, async ({ page }) => {
...
test('platform campaign sync posts to API', { tag: '@write' }, async ({ page }) => {
  await loginAsAdmin(page);
...
test(
  'postback DLQ retry posts to API and refreshes snapshot',
  { tag: '@write' },
  async ({ page }) => {
    await loginAsAdmin(page);
