// Package main is the cold-path stream/broker consumer: settlement, ClickHouse ingest, budget sync.
//
// Role:
...
//   - SettlementWorker: Postgres events/stats from REDIS_GROUP_NAME_pg (or _pg_broker) consumer groups.
...
//   - Dual Postgres pools: general read (DB_PROCESSOR_MAX_CONNS) and settlement (PostgresPoolSettleConns).
...
//   - ClickHouse writes batched via CLICKHOUSE_BATCH_SIZE and CLICKHOUSE_FLUSH_INTERVAL_MS; optional CH_SPOOL_DIR.
//
// Forbidden:
...
//   - BROKER_SHADOW_MODE=1: broker ClickHouse group counts without ClickHouse write until cutover.
//
// Verify:
//
