// Dev-only BPF collector for load-test and dev observability sessions.
...
func main() {
	...
	loadgenComms := flag.String("loadgen-comms", "", "comma-separated /proc comm names (default k6; env ESPX_BPF_LOADGEN_COMM)")
	...
	trackerBinary := flag.String("tracker-binary", "", "tracker executable for uprobes (default: /proc/<tracker-pid>/exe)")
	flag.Parse()
...
func (r *probeRun) writeTimeline() error {
	...
	row := map[string]any{
		...
		"prometheus_query_tpl": "sum(rate(ad_http_request_duration_seconds_count{job=\"tracker\"}[5m]))",
