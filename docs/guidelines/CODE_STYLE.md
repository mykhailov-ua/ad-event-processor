...
- `cmd/<binary>/main.go` - config, pools, constructors (R4)
- `internal/campaignmodel/` - shared hot-path types
- `internal/config/`, `internal/database/`, `internal/metrics/`
- `internal/<service>/` - flat package (R1); subdirs: `db/`, `queries/`, `migrations/`, `pb/`
- `pkg/` - utilities without domain imports
- `deploy/` - nginx, compose, operator resources

Root service folders contain non-Go resources bound to that binary (e.g., `unified-filter.lua` in ingestion). Generated code must reside only in the allowed subdirectories (R1).
...
3. Repository interfaces for every table in a separate package - sqlc's `db.Queries` is a repository in itself; an extra layer of indirection adds no function.
...
5. Use-case structs that only forward calls to another level - extra indirection; call `Service` methods directly.
...
Imports between services must be rare and explicit (e.g., calling ingestion registry helpers from management). Prefer interfaces in `internal/campaignmodel` for contracts shared by multiple binaries.
...
1. Hot-Path Model - `internal/campaignmodel`. No tags allowed (`json`, `db` tags are forbidden). Names: `Campaign`, `Event`.
...
- Hot path builds `campaignmodel.Campaign` in `registry.go` from `db` rows. Banned: processing catalog state through JSON on the ingestion path.

The `internal/campaignmodel` package is a shared vocabulary (status enums, `Event` pool, `Campaign` struct with precomputed Redis keys). It is not an entity layer: no persistence tags, no handlers, no `toDTO` helpers.
...
db.Campaign -> registry build -> campaignmodel.Campaign   (in-memory; no JSON)

pb.Event <-> field copy / vtproto <-> Handler     (ingest; campaignmodel.Event pool)
```
...
5. Hot-Path - `requests_parse.go` parses into `campaignmodel.Event`; the registry returns pointers to `*campaignmodel.Campaign`.

Banned: introducing `Entity`, `Model`, or `View` types duplicating the same table. Allowed: converting `pgtype` / `uuid` inside `to*DTO` or database write methods.
...
Packages under `pkg/` do not import `internal/<service>`. Packages under `internal/` may import `pkg/**`.
...
1. Package name matches the directory: `package ingestion`, `package payment`.
...
Non-Go resources bound to the service (e.g., `unified-filter.lua` in ingestion) must remain in the service root.

Banned: nested domain packages under service roots such as `internal/ingestion/filter/`, `internal/rtb/ (nested packages forbidden)`, `internal/management/fraud/`. `internal/management/` is one flat package - navigate by filename prefix (`blacklist_*`, `service_fraud.go`), not subfolders. Only `pb/` (generated) is allowed under `management/`.

### R1b. Cold-path JSON API

JSON `/api/v1` lives in `internal/management/` (`handler_api.go`, `handler_*.go`, `api_*.go`) and `internal/adminapi/` (reporting scaffolds). Wire routes from `cmd/management`.

1. Tag = first `_` segment - `api_billing_handlers.go`, `handler_rtb.go`, `billing_handlers.go`.
2. Single flat package per directory; no nested domain packages.
3. sqlc - tracker queries in `internal/ingestion/sqlc/`; other services in `internal/<svc>/db/`.

Anti-patterns (reject in review):
- Nested packages under `internal/ingestion/filter/`, `internal/management/fraud/`.
- Names like `api`, `http`, `common`, `utils` as filename tags.
...
5. Shared Hot-Path Model - `internal/campaignmodel`, no struct tags (e.g., `campaignmodel.Campaign`).
...
Binary name must match the process name (`tracker`, `management`, `payment`, `auth`). `main` only binds configuration, pools, and `internal/<service>` constructors - Banned: placing business logic in `main`.
...
3. Banned: importing non-existent sub-packages (`management/service`, `ads/filter`).

### R6. Domain Terminology

Consistently use abbreviations accepted in the project:
1. `evt` - `*campaignmodel.Event` on the hot path in ads.
...
3. `pgx.ErrNoRows` - only for "resource not found". Other Postgres errors do not be masked as `ErrNotFound` / 404. Example: `GetRtbDeal` returns `ErrRtbDealNotFound` only on `errors.Is(err, pgx.ErrNoRows)`.
...
6. Serialization - `json.Marshal`, `uuid.Parse`, body decode: explicit check and return (R7). Banned: `payload, _ := json.Marshal(...)` in production code.
7. gRPC - map to `codes.NotFound`, `InvalidArgument`, `Internal` at the handler boundary; do not pass raw `err.Error()` to the client.
8. Banned: `log.Fatal` and `panic` in request processing paths (acceptable only in `main` on fatal misconfiguration).
...
1. Sentinels Without Formatting - `var ErrBudgetExhausted = errors.New("budget exhausted")` in `filters.go`. Banned: `fmt.Errorf` / `errors.New(fmt.Sprintf(...))` on the parse -> filter -> respond path.
...
4. Allocation-Free Client Response - `filterRejectSpecs` table: pre-built `[]byte` gnet responses and HTTP statuses. Banned: `json.Marshal` / `fmt.Sprintf` for each reject.
...
6. Best-Effort Side Effects - `FraudStreamWriter`, `LatencyRing`: drop on overflow, increment drop counter; rare sampled logs (R7). Banned: blocking the hot loop waiting for I/O.
7. Banned on the hot path: `defer` in loops, `interface{}` error boxing in inner loops, `log.Fatal`, dynamic error string construction for metrics (use pre-bound label values).
8. Type Assertions - only safe snapshots (`campaignMapSnapshot()` - R8.7 pt. 2). Banned: `m, _ := x.Load().(map[...])`.
...
1. `pkg/cold` - single point for `json.Marshal`/`Unmarshal`, `uuid.Parse`, pagination (`MapSlice`, `PaginatedList`). Banned: duplicating wrappers in each service.
2. Typed Validation Errors - for form fields and queries, use unexported types (`invalidQueryError`, `validationError`) and `errors.As` in the mapper. Banned: relying on `strings.Contains(err.Error(), "required")` for new code if a stable type exists.
...
6. Silent `json.Unmarshal` - Banned: `if json.Unmarshal(...) != nil { return }` without `slog` and/or counter. Corrupt replica -> warn + drop, not panic.
...
2. Typed Snapshot for `atomic.Value` - Stores write an immutable snapshot wrapper struct (e.g., containing `byID map[uuid.UUID]campaignInfo`). Readers access the snapshot via a dedicated helper function that performs a safe type assertion to the concrete pointer, returning a zero-value fallback struct on failure. Direct untyped `Load().(map[...])` assertions are strictly forbidden.
   ...
   Banned: `m, _ := r.data.Load().(map[uuid.UUID]...)`.

3. Redis on Hot-Path - one `EVALSHA` per event (unified filter, IP rate limit Lua). Banned: `Pipeline()` + multiple `New*Cmd` per filter check if an equivalent Lua script exists.
...
Comments record constraints the code cannot express: invariants, failure modes, trust boundaries, performance budgets. They do not narrate control flow, duplicate identifiers, or market the design.

#### Tone

1. English only; ASCII `0x20`-`0x7E` only.
2. No emojis, Unicode section signs, Unicode dashes, curly quotes, decorative symbols, arrows (`->`, `<-`) in comments.
3. Punctuation: ASCII hyphen `-` for ranges and parenthetical breaks; end multi-sentence blocks with `.`.
4. Dry and factual: state what holds, what fails, or what the caller must do. No hype, reassurance, or self-reference ("we", "this function will").
5. Complete sentences with explicit subject and verb. No telegraphic fragments or stacked semicolon chains.
6. Cross-references: `docs/FOO.md section 4.3`, not `FOO.md Sec.4.3`.

Banned tone words (case-insensitive; CI enforces tier 1): `simple`, `elegant`, `clean`, `obviously`, `just`, `simply`, `nice`, `obvious`, `trivial`, `minimal`.

#### AI slop (comments, godoc, commit messages in repo)

LLM-generated prose clusters predictable vocabulary, filler transitions, and meta-narration. Treat as a defect in comments the same as a banned word.

Tier 1 (CI `check_comments`): tier-1 tone words plus `leverage`, `delve`, `seamless`, `seamlessly`, `moreover`, `furthermore`, `additionally`, `holistic`, `navigate`, `testament`, `harness`, `effortlessly`, `notably`, `essentially`, `basically`.

Tier 2 (review; rewrite if clustered): `robust`, `comprehensive`, `pivotal`, `crucial`, `vital`, `landscape`, `ecosystem`, `streamline`, `empower`, `unlock`, `elevate`, `cutting-edge`, `best-in-class`, `world-class`, `mission-critical`, `ever-evolving`, `fast-paced`, `deep dive`, `at its core`, `when it comes to`, `it is important to note`, `worth noting`, `in conclusion`, `overall`, `ultimately`, `going forward`, `rest assured`, `plays a vital role`, `serves as a`.

Structural tells (always rewrite):
1. Opening with what the code "will do" instead of the constraint (`// This function handles...` -> delete or state invariant).
2. Restating the function or type name (`// parseTrack parses the track request`).
3. Rule-of-three garnish (`fast, reliable, and scalable`).
4. Negative parallelism (`not only X but also Y`) without a technical distinction.
5. Hedge stacks (`might possibly`, `could potentially`).
6. Bold or markdown emphasis inside Go comments.
7. Outcome narration after the code already shows the outcome.

Docs follow the same rules; see `docs/guidelines/` style (no milestone catalogs, no ASCII box diagrams).

#### When to comment

| Criterion | Comment if |
| :--- | :--- |
| Contract | Exported symbol has a side effect, goroutine, or ordering obligation the caller must know (`Close`, `StartBackgroundWorker`). |
| Invariant | Financial, idempotency, or sharding rule not visible from types (`current_spend <= budget_limit`, fence epoch). |
| Performance | Hot path budget or allocation rule (`one EVALSHA per event`, BCE hint). |
| Failure mode | Error path changes durable state or retry semantics (outbox returns to `PENDING`). |
| Topology | Shard-0-only, fail-open vs fail-closed, or cross-service trust boundary. |
| External spec | Implements a named runbook, chaos scenario, or doc section (cite `docs/DEVELOPMENT.md section N`). |
| Compiler | `//go:noinline`, `//nolint:` with reason, BCE boundary comment. |
| Test | Scenario and expected invariant are not obvious from the test name alone (R9.2). |

If none of the rows apply, omit the comment.

#### When not to comment

1. Name already states behavior (`getCampaignByID`, `incrementCounter`).
2. Step-by-step narration of loops or branches the reader can read in order.
3. Trivial getters, wire-only DTO fields, generated sqlc/protobuf fields.
4. Commented-out code (delete the code).
5. TODO without owner, ticket, or expiry (use issue ID or delete).
6. Change log in source (`// added 2026-07`); use git history.

Default: unexported symbols need no godoc unless a row in "When to comment" applies. Exported API: godoc on types and entry points the caller invokes.

Placement:
1. Package comment (optional): one block in `service.go` or `handler.go` when the exported surface needs a service-level contract. In `tests/`, one package comment per package; per-file headers only when files split distinct areas.
2. Struct fields: only when the name does not carry the rule (enums, bitmasks, TTL semantics).
3. Inside function body: almost never. Exceptions: BCE, `//go:` directives, `//nolint:` with reason, workaround with issue link.
...
- Comment patterns to avoid: Fragments, subjective words, non-ASCII emojis, or telegraphic text (e.g., `verifies Sec.4.3 - no double debit `).
...
3. Tier-1 banned words (case-insensitive): tone list in R9 plus AI slop tier 1 (`leverage`, `delve`, `seamless`, `moreover`, `furthermore`, `additionally`, `holistic`, `navigate`, `testament`, `harness`, `effortlessly`, `notably`, `essentially`, `basically`, and tone words `simple`, `elegant`, `clean`, `obviously`, `just`, `simply`, `nice`, `obvious`, `trivial`, `minimal`).
4. Unicode Dashes and Section Signs - `-`, `-`, `Sec.` in comments (use `-` and `section N`).
...
3. Banned: globally disabling errcheck for all of `internal/management` or `internal/ingestion`.
...
6. Extract shared hot-path types or store interfaces to `internal/campaignmodel` only when required by multiple binaries.
...
1. Hot-path or contract for multiple binaries: `internal/campaignmodel`, no tags, precomputed fields allowed.
...
5. Banned: adding an intermediate `FooEntity` model between `db.Foo` and `FooDTO`.
...
Follow R9: tone, AI slop tiers, and the when-to-comment table. Good comments state invariants, failure modes, or doc cross-references. Bad comments restate names, narrate control flow, or use tier-1/tier-2 slop vocabulary.
...
- [ ] Comments: R9 tone, no AI slop tier 1, when-to-comment criteria (R9, R9.2)
