...
**Related:** [ARCHITECTURE.md](ARCHITECTURE.md) §1.1 (request lifecycle), [PARSER.md](PARSER.md) (edge wire policy).
...
#### DMR referer hiding (`dmr=1`)

`302` redirects can leak `Referer` to the lander. Enable **DMR** (meta refresh + `location.replace`) via query `dmr=1` / `dmr=true` or campaign flag `dmr_enabled` (admin UI Config tab; migration `00086`).

- **Response:** `200 OK` HTML with meta refresh and JS replace (not `302`).
- **Coverage:** `/click`, `/tg/click`, tracking-domain rotation. Reverse-proxy click path is skipped when DMR is active.
- **Plain 302:** still sends `Referrer-Policy: no-referrer`.
- **Limits:** no-JS clients may still send referrer (browser-dependent).

```http
GET /click?campaign_id=<uuid>&dmr=1&sub1=fb HTTP/1.1
Host: trk.example.com
```

Verification: `go test ./internal/ingestion/ -run 'TestClickRedirectGnet_DMR_|TestBuildDmr' -count=1`
...
- **Edge policy:** `Content-Length` required; `Transfer-Encoding: chunked` **rejected** on `/track` (see [PARSER.md](PARSER.md) §2).
...
- **Edge policy:** Chunked transfer encoding **allowed** (extensions rejected). See [PARSER.md](PARSER.md) §4.
...
- **Runbook:** [RTB.md](RTB.md) (shadow soak, deals, reconcile).
...
Shadow promotion checklist: [RTB.md](RTB.md) §3–5. Integration profile lint: `GET /api/v1/rtb/integration-profile`.
...
Match parser policies documented in [PARSER.md](PARSER.md) so nginx and gnet stay aligned.
