...
For exact SLA targets and benchmarks, see [.cursor/rules/core.mdc](../.cursor/rules/core.mdc), [ARCHITECTURE.md](ARCHITECTURE.md), and [DEVELOPMENT.md](DEVELOPMENT.md#benchmark-harness-matrix).
...
**Goals** (see [DEVELOPMENT.md](DEVELOPMENT.md#broker-cutover-ch_ingest_source) broker cutover):
...
3. **Controlled cutover** — Dual-path shadow (`BROKER_SHADOW_MODE=1`), PEL drain ([DEVELOPMENT.md](DEVELOPMENT.md#broker-cutover-ch_ingest_source)), then `CH_INGEST_SOURCE=broker`.
...
| Shard 0 isolation | `tests/resilience/chaos_fault_suite_test.go`, [SHARDING.md](SHARDING.md) | Control-plane kill drills |
...
**Agent/human must paste command output for prod SLA claims** — see `.cursor/rules/quality.mdc` verification checklist.
...
### How Migration Works
When rebalancing Redis instances, operators perform slot migrations without relying on Redis Cluster protocols ([DEVELOPMENT.md](DEVELOPMENT.md#slot-migration)):
...
| Multi-region (Enterprise) | Cross-region Redis stream replication is heavy | `region-proxy` uplink + broker segments ([REGIONS.md](REGIONS.md)) |
...
Documented in [DEVELOPMENT.md](DEVELOPMENT.md#broker-cutover-ch_ingest_source): shadow → drain PEL → `CH_INGEST_SOURCE=broker` → optional rollback to Redis consumers while broker offsets persist on disk.
