# ADMIN_DASHBOARD_MILESTONE_OPERATOR

Dashboard: Operator.

**Status:** DRAFT  
**Slug:** `admin_dashboard_operator`  
...
- Route `/dashboards/operator`
- XDP + edge ingress panels; stack health tiles
- API: `GET /api/v1/dashboards/operator` (`dashboardOperator`)
- KPI keys: xdp.pass, xdp.pass_allowlist, xdp.fingerprints, edge.ingress_h1, edge.ingress_h2, edge.ingress_h3, edge.tarpit_total, edge.blacklist_stale
- FreshnessBadge from freshness DTO only
- RBAC: `shards:read` per OpenAPI x-permissions
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Operator | static |
| xdp | XDPPanel | pass, drops map, fingerprints | xdp |
| edge | EdgeMetricsPanel | ingress_h*, blocked map | edge |
| links | DrillDownLinks | /ops, /ops/edge-parity | permissions |
...
```
GET /api/v1/dashboards/operator?customer_id={uuid}&from={iso}&to={iso}
```
...
| web/src/ui/dashboard/operator_dashboard.tsx | KPI + sections |
...
| :--- | :--- | :--- | :--- |
| 1 | GET /api/v1/dashboards/operator | Confirm operation_id + handler DTO fields | openapi_gate.sh + handler test |
...
| KPI key parity | rg 'xdp.pass' web/src/ui/dashboard/ | maps to API field name |
