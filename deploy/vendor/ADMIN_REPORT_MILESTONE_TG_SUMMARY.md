# ADMIN_REPORT_MILESTONE_TG_SUMMARY

Report: telegram.

**Status:** DRAFT  
**Slug:** `admin_report_telegram`  
...
- `/reports/telegram` — Telegram Mini App summary KPIs
- API: `GET /api/v1/reports/telegram/summary`
- Column contract: Handler row keys (ReportMapEnvelope): clicks, impressions, conversions, premium, motivated
...
| :--- | :--- | :--- |
| GET | GET /api/v1/reports/telegram/summary | reportTelegramSummary |
| Columns | Handler row keys (ReportMapEnvelope): clicks, impressions, conversions, premium, motivated | handler |
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Report title from catalog or static (telegram) | GET /api/v1/reports/catalog or reportTitle() |
...
| content | report_telegram | KPI tiles from envelope rows or summary fields | rows[] or typed response |
...
| `live` | true |
| Handler | internal/reports/ — `reportTelegramSummary` |

### 4.3 Layout and placement (grid contract)


**Non-sortable headers:** clicks, impressions, conversions, premium, motivated
...
```
/api/v1/reports/telegram/summary?from={iso}&to={iso}&customer_id={uuid}
```
...
| Path | Role |
| :--- | :--- |
| web/src/ui/reports/report_telegram.tsx | ReportKpiStrip section |
| web/src/ui/reports/report_telegram.module.css | Grid/heatmap CSS module |
| web/src/helpers/reports_api.ts | fetchReport('telegram', params) |
| web/src/pages/report_telegram_summary_page.tsx | Compose until ui/reports migration |

**Legacy page:** `report_telegram_summary_page.tsx`
...
| :--- | :--- | :--- | :--- |
| 1 | api/openapi/paths/ops_reports.yaml | Confirm `reportTelegramSummary` params + response schema | openapi_gate.sh |
| 2 | make openapi-types | Regenerate openapi.d.ts | typecheck |
| 3 | web/src/helpers/reports_api.ts | fetchReport('telegram', params) | Compiles |
| 4 | web/src/app_routes.tsx | Register `/reports/telegram` (legacy compose until ui/reports lands) | Route resolves |
| 5 | web/src/ui/reports/report_telegram.tsx | ReportKpiStrip per 4.3 | surface gate |
...
```bash
curl -s '/api/v1/reports/telegram/summary?from={iso}&to={iso}&customer_id={uuid}' | jq '.freshness'
```

| Check | Command / procedure | Pass criteria |
| :--- | :--- | :--- |
| No ghost labels | rg 'ghost_' web/src/ui/reports/report_telegram | no matches |
...
- [ ] Legacy `report_telegram_summary_page.tsx` replaced or delegates to `ui/reports/`
