# ADMIN_REPORT_MILESTONE_SIGNAL_EFFECTIVENESS

Report: signal-effectiveness.

**Route gap:** register `/reports/signal-effectiveness` in `app_routes.tsx` before `live: true`.

**Status:** DRAFT  
**Slug:** `admin_report_signal_effectiveness`  
...
- `/reports/signal-effectiveness` — Wire signal block and silent-reject rates
- API: `GET /api/v1/reports/signal-effectiveness`
- Column contract: Handler row keys (ReportMapEnvelope): signal_code, fraud_category, fraud_category_label, event_volume, block_rate, block_rate_display, silent_reject_rate, silent_reject_rate_display, suggested_weight_tier
...
| :--- | :--- | :--- |
| GET | GET /api/v1/reports/signal-effectiveness | reportSignalEffectiveness |
| Columns | Handler row keys (ReportMapEnvelope): signal_code, fraud_category, fraud_category_label, event_volume, block_rate, block_rate_display, silent_reject_rate, silent_reject_rate_display, suggested_weight_tier | handler |
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Report title from catalog or static (signal-effectiveness) | GET /api/v1/reports/catalog or reportTitle() |
...
| content | report_signal_effectiveness | role=grid; column keys from OpenAPI/handler rows[] | rows[] or typed response |
...
| `live` | false |
| Handler | internal/reports/ — `reportSignalEffectiveness` |

### 4.3 Layout and placement (grid contract)


**Non-sortable headers:** signal_code, fraud_category, fraud_category_label, event_volume, block_rate, block_rate_display, silent_reject_rate, silent_reject_rate_display, suggested_weight_tier
...
```
/api/v1/reports/signal-effectiveness?customer_id={uuid}&from={iso}&to={iso}
```
...
| Path | Role |
| :--- | :--- |
| web/src/ui/reports/report_signal_effectiveness.tsx | ReportDataGrid section |
| web/src/ui/reports/report_signal_effectiveness.module.css | Grid/heatmap CSS module |
| web/src/helpers/reports_api.ts | fetchReport('signal-effectiveness', params) |
| web/src/pages/report_route_pages.tsx | Compose until ui/reports migration |

**Legacy page:** `report_stub_page.tsx (signal-effectiveness)`
...
| :--- | :--- | :--- | :--- |
| 1 | api/openapi/paths/ops_reports.yaml | Confirm `reportSignalEffectiveness` params + response schema | openapi_gate.sh |
| 2 | make openapi-types | Regenerate openapi.d.ts | typecheck |
| 3 | web/src/helpers/reports_api.ts | fetchReport('signal-effectiveness', params) | Compiles |
| 4 | web/src/app_routes.tsx | Register gap route `/reports/signal-effectiveness` + nav/catalog entry | Route resolves |
| 5 | web/src/ui/reports/report_signal_effectiveness.tsx | ReportDataGrid section | check_ui_surface_gate.sh |
...
```bash
Register `/reports/signal-effectiveness` in app_routes before catalog live:true
curl -s '/api/v1/reports/signal-effectiveness?customer_id={uuid}&from={iso}&to={iso}' | jq '.freshness'
```

| Check | Command / procedure | Pass criteria |
| :--- | :--- | :--- |
| No ghost labels | rg 'ghost_' web/src/ui/reports/report_signal_effectiveness | no matches |
...
- [ ] Legacy `report_stub_page.tsx (signal-effectiveness)` replaced or delegates to `ui/reports/`
