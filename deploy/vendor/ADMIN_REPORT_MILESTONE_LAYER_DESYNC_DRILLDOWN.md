# ADMIN_REPORT_MILESTONE_LAYER_DESYNC_DRILLDOWN

Report: layer-desync-drilldown.

**Route gap:** register `/reports/layer-desync-drilldown` in `app_routes.tsx` before `live: true`.

**Status:** DRAFT  
**Slug:** `admin_report_layer_desync_drilldown`  
...
- `/reports/layer-desync-drilldown` — Layer desync drilldown with hourly series
- API: `GET /api/v1/reports/layer-desync-drilldown`
- Column contract: Handler row keys (ReportMapEnvelope): fraud_reason, fraud_category, fraud_category_label, event_count, silent_reject_count, signals_degraded
...
| :--- | :--- | :--- |
| GET | GET /api/v1/reports/layer-desync-drilldown | reportLayerDesyncDrilldown |
| Columns | Handler row keys (ReportMapEnvelope): fraud_reason, fraud_category, fraud_category_label, event_count, silent_reject_count, signals_degraded | handler |
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Report title from catalog or static (layer-desync-drilldown) | GET /api/v1/reports/catalog or reportTitle() |
...
| content | report_layer_desync_drilldown | role=grid; column keys from OpenAPI/handler rows[] | rows[] or typed response |
...
| `live` | false |
| Handler | internal/reports/ — `reportLayerDesyncDrilldown` |

### 4.3 Layout and placement (grid contract)


**Non-sortable headers:** fraud_reason, fraud_category, fraud_category_label, event_count, silent_reject_count, signals_degraded
...
```
/api/v1/reports/layer-desync-drilldown?customer_id={uuid}&from={iso}&to={iso}
```
...
| Path | Role |
| :--- | :--- |
| web/src/ui/reports/report_layer_desync_drilldown.tsx | ReportDataGrid section |
| web/src/ui/reports/report_layer_desync_drilldown.module.css | Grid/heatmap CSS module |
| web/src/helpers/reports_api.ts | fetchReport('layer-desync-drilldown', params) |
| web/src/pages/report_route_pages.tsx | Compose until ui/reports migration |

**Legacy page:** `report_stub_page.tsx (layer-desync-drilldown)`
...
| series client merge | Plot handler series[]; no client resample |
...
| :--- | :--- | :--- | :--- |
| 1 | api/openapi/paths/ops_reports.yaml | Confirm `reportLayerDesyncDrilldown` params + response schema | openapi_gate.sh |
| 2 | make openapi-types | Regenerate openapi.d.ts | typecheck |
| 3 | web/src/helpers/reports_api.ts | fetchReport('layer-desync-drilldown', params) | Compiles |
| 4 | web/src/app_routes.tsx | Register gap route `/reports/layer-desync-drilldown` + nav/catalog entry | Route resolves |
| 5 | web/src/ui/reports/report_layer_desync_drilldown.tsx | ReportDataGrid section | check_ui_surface_gate.sh |
...
```bash
Register `/reports/layer-desync-drilldown` in app_routes before catalog live:true
curl -s '/api/v1/reports/layer-desync-drilldown?customer_id={uuid}&from={iso}&to={iso}' | jq '.freshness'
```

| Check | Command / procedure | Pass criteria |
| :--- | :--- | :--- |
| No ghost labels | rg 'ghost_' web/src/ui/reports/report_layer_desync_drilldown | no matches |
...
- [ ] Legacy `report_stub_page.tsx (layer-desync-drilldown)` replaced or delegates to `ui/reports/`
