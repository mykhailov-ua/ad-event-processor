# ADMIN_REPORT_MILESTONE_TRAFFIC_SOURCES

Report: traffic-sources.

**Status:** DRAFT  
**Slug:** `admin_report_traffic_sources`  
...
- `/reports/traffic-sources` — Traffic channel rollup funnel
- API: `GET /api/v1/reports/traffic-sources`
- Column contract: Handler row keys (ReportMapEnvelope): channel, impressions, clicks, spend_micro, roi_pct
...
| :--- | :--- | :--- |
| GET | GET /api/v1/reports/traffic-sources | reportTrafficSources |
| Columns | Handler row keys (ReportMapEnvelope): channel, impressions, clicks, spend_micro, roi_pct | handler |
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Report title from catalog or static (traffic-sources) | GET /api/v1/reports/catalog or reportTitle() |
...
| content | report_traffic_sources | role=grid; column keys from OpenAPI/handler rows[] | rows[] or typed response |
...
| `live` | true |
| Handler | internal/reports/ — `reportTrafficSources` |

### 4.3 Layout and placement (grid contract)


**Non-sortable headers:** channel, impressions, clicks, spend_micro, roi_pct
...
```
/api/v1/reports/traffic-sources?customer_id={uuid}&from={iso}&to={iso}
```
...
| Path | Role |
| :--- | :--- |
| web/src/ui/reports/report_traffic_sources.tsx | ReportDataGrid section |
| web/src/ui/reports/report_traffic_sources.module.css | Grid/heatmap CSS module |
| web/src/helpers/reports_api.ts | fetchReport('traffic-sources', params) |
| web/src/pages/report_route_pages.tsx | Compose until ui/reports migration |

**Legacy page:** `report_route_pages.tsx TrafficSourcesReportPage`
...
| :--- | :--- | :--- | :--- |
| 1 | api/openapi/paths/ops_reports.yaml | Confirm `reportTrafficSources` params + response schema | openapi_gate.sh |
| 2 | make openapi-types | Regenerate openapi.d.ts | typecheck |
| 3 | web/src/helpers/reports_api.ts | fetchReport('traffic-sources', params) | Compiles |
| 4 | web/src/app_routes.tsx | Register `/reports/traffic-sources` (legacy compose until ui/reports lands) | Route resolves |
| 5 | web/src/ui/reports/report_traffic_sources.tsx | ReportDataGrid per 4.3 | surface gate |
...
```bash
curl -s '/api/v1/reports/traffic-sources?customer_id={uuid}&from={iso}&to={iso}' | jq '.freshness'
```

| Check | Command / procedure | Pass criteria |
| :--- | :--- | :--- |
| No ghost labels | rg 'ghost_' web/src/ui/reports/report_traffic_sources | no matches |
...
- [ ] Legacy `report_route_pages.tsx TrafficSourcesReportPage` replaced or delegates to `ui/reports/`
