# ADMIN_OPS_MILESTONE_BLACKLIST

Ops: /ops/blacklist.

**Status:** DRAFT  
**Slug:** `admin_ops_blacklist`  
...
- `/ops/blacklist` — Fraud IP blacklist; dry-run preview (X-Dry-Run); block/unblock with confirm
- API: GET/POST/DELETE /api/v1/ops/blacklist
- Confirm modal: POST /api/v1/ops/blacklist (block), DELETE /api/v1/ops/blacklist (unblock)
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Fraud blacklist | static |
| form | BlacklistAddForm | ip, source, ttl_seconds | OpsBlacklistWriteRequest |
| preview | DryRunPreview | X-Dry-Run: 1 preview panel | POST dry-run response |
| toolbar | BlacklistToolbar | Block IP action | apiConfirmed POST |
| grid | BlacklistGrid | Paginated entries | opsListBlacklist limit/offset |
| actions | UnblockButton | Per-row DELETE | apiConfirmed DELETE |
| error | ErrorBlock | List/mutation failure | errors |
...
| Path | Role |
| :--- | :--- |
| web/src/pages/ops_blacklist_page.tsx | Compose only |
...
**Legacy page:** `ops_blacklist_page.tsx`
...
| 5 | web/src/pages/ops_blacklist_page.tsx | Thin compose; permission gates | No *.module.css on page |
...
- [ ] Legacy `ops_blacklist_page.tsx` replaced or delegates to `ui/ops/`
