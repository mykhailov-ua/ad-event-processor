# ADMIN_REPORT_MILESTONE_CAMPAIGN_OVERVIEW

Report: campaign-overview.

**Status:** DRAFT  
**Slug:** `admin_report_campaign_overview`  
...
- `/reports/campaign-overview` — Multi-campaign economics overview
- API: `GET /api/v1/reports/campaign-overview`
- Column contract: Handler row keys (ReportMapEnvelope): name, status, impressions_7d, clicks_7d, utilization_pct, pacing_drift_pct, overspend_risk
...
| :--- | :--- | :--- |
| GET | GET /api/v1/reports/campaign-overview | reportCampaignOverview |
| Columns | Handler row keys (ReportMapEnvelope): name, status, impressions_7d, clicks_7d, utilization_pct, pacing_drift_pct, overspend_risk | handler |
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Report title from catalog or static (campaign-overview) | GET /api/v1/reports/catalog or reportTitle() |
...
| content | report_campaign_overview | role=grid; column keys from OpenAPI/handler rows[] | rows[] or typed response |
...
| `live` | true |
| Handler | internal/reports/ — `reportCampaignOverview` |

### 4.3 Layout and placement (grid contract)


**Non-sortable headers:** name, status, impressions_7d, clicks_7d, utilization_pct, pacing_drift_pct, overspend_risk
...
```
/api/v1/reports/campaign-overview?customer_id={uuid}&from={iso}&to={iso}
```
...
| Path | Role |
| :--- | :--- |
| web/src/ui/reports/report_campaign_overview.tsx | ReportDataGrid section |
| web/src/ui/reports/report_campaign_overview.module.css | Grid/heatmap CSS module |
| web/src/helpers/reports_api.ts | fetchReport('campaign-overview', params) |
| web/src/pages/report_route_pages.tsx | Compose until ui/reports migration |

**Legacy page:** `report_configs.ts campaign-overview`
...
| :--- | :--- | :--- | :--- |
| 1 | api/openapi/paths/ops_reports.yaml | Confirm `reportCampaignOverview` params + response schema | openapi_gate.sh |
| 2 | make openapi-types | Regenerate openapi.d.ts | typecheck |
| 3 | web/src/helpers/reports_api.ts | fetchReport('campaign-overview', params) | Compiles |
| 4 | web/src/app_routes.tsx | Register `/reports/campaign-overview` (legacy compose until ui/reports lands) | Route resolves |
| 5 | web/src/ui/reports/report_campaign_overview.tsx | ReportDataGrid per 4.3 | surface gate |
...
```bash
curl -s '/api/v1/reports/campaign-overview?customer_id={uuid}&from={iso}&to={iso}' | jq '.freshness'
```

| Check | Command / procedure | Pass criteria |
| :--- | :--- | :--- |
| No ghost labels | rg 'ghost_' web/src/ui/reports/report_campaign_overview | no matches |
...
| Export POST | POST /api/v1/reports/jobs body report_key=campaign-overview | 202 + job id |
...
- [ ] Legacy `report_configs.ts campaign-overview` replaced or delegates to `ui/reports/`
