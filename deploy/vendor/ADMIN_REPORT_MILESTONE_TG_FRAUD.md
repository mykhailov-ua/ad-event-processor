# ADMIN_REPORT_MILESTONE_TG_FRAUD

Report: telegram-fraud.

**Status:** DRAFT  
**Slug:** `admin_report_telegram_fraud`  
...
- `/reports/telegram/fraud` — Telegram fraud signals
- API: `GET /api/v1/reports/telegram/fraud`
- Column contract: Handler row keys (ReportMapEnvelope): fraud_reason, event_count, silent_reject_count, share_pct
...
| :--- | :--- | :--- |
| GET | GET /api/v1/reports/telegram/fraud | reportTelegramFraud |
| Columns | Handler row keys (ReportMapEnvelope): fraud_reason, event_count, silent_reject_count, share_pct | handler |
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Report title from catalog or static (telegram-fraud) | GET /api/v1/reports/catalog or reportTitle() |
...
| content | report_telegram_fraud | role=grid; column keys from OpenAPI/handler rows[] | rows[] or typed response |
...
| `live` | false |
| Handler | internal/reports/ — `reportTelegramFraud` |

### 4.3 Layout and placement (grid contract)


**Non-sortable headers:** fraud_reason, event_count, silent_reject_count, share_pct
...
```
/api/v1/reports/telegram/fraud?from={iso}&to={iso}&customer_id={uuid}
```
...
| Path | Role |
| :--- | :--- |
| web/src/ui/reports/report_telegram_fraud.tsx | ReportDataGrid section |
| web/src/ui/reports/report_telegram_fraud.module.css | Grid/heatmap CSS module |
| web/src/helpers/reports_api.ts | fetchReport('telegram-fraud', params) |
| web/src/pages/report_telegram_fraud_page.tsx | Compose until ui/reports migration |

**Legacy page:** `report_telegram_fraud_page.tsx`
...
| :--- | :--- | :--- | :--- |
| 1 | api/openapi/paths/ops_reports.yaml | Confirm `reportTelegramFraud` params + response schema | openapi_gate.sh |
| 2 | make openapi-types | Regenerate openapi.d.ts | typecheck |
| 3 | web/src/helpers/reports_api.ts | fetchReport('telegram-fraud', params) | Compiles |
| 4 | web/src/app_routes.tsx | Register `/reports/telegram/fraud` (legacy compose until ui/reports lands) | Route resolves |
| 5 | web/src/ui/reports/report_telegram_fraud.tsx | ReportDataGrid per 4.3 | surface gate |
...
```bash
curl -s '/api/v1/reports/telegram/fraud?from={iso}&to={iso}&customer_id={uuid}' | jq '.freshness'
```

| Check | Command / procedure | Pass criteria |
| :--- | :--- | :--- |
| No ghost labels | rg 'ghost_' web/src/ui/reports/report_telegram_fraud | no matches |
...
- [ ] Legacy `report_telegram_fraud_page.tsx` replaced or delegates to `ui/reports/`
