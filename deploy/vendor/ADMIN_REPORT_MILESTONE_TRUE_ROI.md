# ADMIN_REPORT_MILESTONE_TRUE_ROI

Report: true-roi.

**Status:** DRAFT  
**Slug:** `admin_report_true_roi`  
...
- `/reports/true-roi` — True ROI after cost sync
- API: `GET /api/v1/reports/true-roi`
- Column contract: `ops_reports.yaml#/TrueRoiReportRow` row keys: campaign_id, ad_spend_micro, revenue_micro, true_profit_micro, true_roi_pct, true_cpa_micro, conversions
...
| :--- | :--- | :--- |
| GET | GET /api/v1/reports/true-roi | reportTrueRoi |
| Columns | `ops_reports.yaml#/TrueRoiReportRow` row keys: campaign_id, ad_spend_micro, revenue_micro, true_profit_micro, true_roi_pct, true_cpa_micro, conversions | openapi |
| Envelope | rows + freshness (+ next_cursor when paginated) | ReportMapEnvelope or typed response |
| Schema | ops_reports.yaml#/TrueRoiReportRow | openapi.d.ts row type |
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Report title from catalog or static (true-roi) | GET /api/v1/reports/catalog or reportTitle() |
...
| content | report_true_roi | role=grid; column keys from OpenAPI/handler rows[] | rows[] or typed response |
...
| `live` | true |
| Handler | internal/reports/ — `reportTrueRoi` |

### 4.3 Layout and placement (grid contract)


**Non-sortable headers:** campaign_id, ad_spend_micro, revenue_micro, true_profit_micro, true_roi_pct, true_cpa_micro, conversions
...
```
/api/v1/reports/true-roi?customer_id={uuid}&from={iso}&to={iso}
```
...
| Path | Role |
| :--- | :--- |
| web/src/ui/reports/report_true_roi.tsx | ReportDataGrid section |
| web/src/ui/reports/report_true_roi.module.css | Grid/heatmap CSS module |
| web/src/helpers/reports_api.ts | fetchReport('true-roi', params) |
| web/src/pages/report_route_pages.tsx | Compose until ui/reports migration |

**Legacy page:** `report_configs.ts true-roi`
...
| :--- | :--- | :--- | :--- |
| 1 | api/openapi/paths/ops_reports.yaml | Confirm `reportTrueRoi` params + response schema | openapi_gate.sh |
| 2 | make openapi-types | Regenerate openapi.d.ts | typecheck |
| 3 | web/src/helpers/reports_api.ts | fetchReport('true-roi', params) | Compiles |
| 4 | web/src/app_routes.tsx | Register `/reports/true-roi` (legacy compose until ui/reports lands) | Route resolves |
| 5 | web/src/ui/reports/report_true_roi.tsx | ReportDataGrid per 4.3 | surface gate |
...
```bash
curl -s '/api/v1/reports/true-roi?customer_id={uuid}&from={iso}&to={iso}' | jq '.freshness'
```

| Check | Command / procedure | Pass criteria |
| :--- | :--- | :--- |
| No ghost labels | rg 'ghost_' web/src/ui/reports/report_true_roi | no matches |
...
| Export POST | POST /api/v1/reports/jobs body report_key=true-roi | 202 + job id |
...
- [ ] Legacy `report_configs.ts true-roi` replaced or delegates to `ui/reports/`
