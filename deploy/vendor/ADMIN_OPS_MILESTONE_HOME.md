...
- `/ops` — Incident cards; metric tiles; overview/outbox/dlq tabs; optional SSE live feed
- API: GET /api/v1/ops/dashboard/summary; GET .../dashboard/metrics; GET .../dashboard/stream (SSE optional); GET .../ops/doctor; GET .../ops/outbox; GET .../ops/dlq
- Confirm modal: roles reload, DLQ retry (outbox tab), DLQ retry (dlq tab)
- Optional SSE via GET /api/v1/ops/dashboard/stream; poll fallback OK
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Operations dashboard | static |
| tabs | TabBar | overview \| outbox \| dlq | URL ?tab= optional |
| incidents | IncidentCards | open incidents from summary | opsDashboardSummary |
| metrics | OpsMetricCharts | Prometheus series tiles | opsDashboardMetrics ?range= |
| sse | OpsLiveFeed | Optional EventSource stream | opsDashboardStream text/event-stream |
| partial | AlertBanner | Parallel slot partial errors | parallelAll failures |
| doctor | DoctorPanel | Stack health checks | GET /api/v1/ops/doctor |
| error | ErrorBlock | Blocking summary failure | errors |
...
PageChrome → TabBar → [IncidentCards + MetricCharts | Outbox grid | DLQ grid] → AlertBanner → ErrorBlock
...
| Path | Role |
| :--- | :--- |
| web/src/pages/ops_home_page.tsx | Compose only |
...
| 5 | web/src/pages/ops_home_page.tsx | Thin compose; permission gates | No *.module.css on page |
...
- [ ] Legacy `ops_home_page.tsx` replaced or delegates to `ui/ops/`
