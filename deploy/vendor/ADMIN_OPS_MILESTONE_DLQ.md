...
- `/ops/dlq` — Unified DLQ inbox; source filter; cursor pagination; per-row retry with confirm
- API: GET /api/v1/ops/dlq; GET /api/v1/ops/dlq/inbox; POST .../dlq/{id}/retry; POST .../dlq/inbox/{id}/retry
- Confirm modal: POST /api/v1/ops/dlq/{id}/retry, POST /api/v1/ops/dlq/inbox/{id}/retry
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Unified DLQ inbox | static |
| filters | DlqSourceFilter | stream \| postback \| capi \| all | local source state |
| toolbar | DlqToolbar | Load more cursor | next_cursor |
| grid | DlqInboxGrid | Merged DLQ rows | inbox API items[] |
| actions | DlqRetryButton | Per-row retry | isDlqInboxEntryRetryable |
| partial | AlertBanner | Partial source errors | fetchDlqInboxPage partialErrors |
| error | ErrorBlock | Blocking inbox fetch | errors |
...
| :--- | :--- | :--- | :--- |
| source | source |  | stream\|postback\|capi filter |
...
| Path | Role |
| :--- | :--- |
| web/src/pages/ops_dlq_page.tsx | Compose only |
...
| 5 | web/src/pages/ops_dlq_page.tsx | Thin compose; permission gates | No *.module.css on page |
...
- [ ] Legacy `ops_dlq_page.tsx` replaced or delegates to `ui/ops/`
