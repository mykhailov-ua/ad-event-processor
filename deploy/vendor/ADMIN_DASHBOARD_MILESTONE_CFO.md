# ADMIN_DASHBOARD_MILESTONE_CFO

Dashboard: CFO.

**Status:** DRAFT  
**Slug:** `admin_dashboard_cfo`  
...
- Route `/dashboards/cfo`
- Unit economics + billed/AR/fees/dispute exposure
- API: `GET /api/v1/dashboards/cfo` (`dashboardCfo`)
- KPI keys: kpis.spend_micro, kpis.revenue_micro, kpis.profit_micro, kpis.roi_pct, billed_micro, ar_aging_micro, fee_total_micro, dispute_exposure_micro
- FreshnessBadge from freshness DTO only
- RBAC: `customers:read` per OpenAPI x-permissions
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | CFO | static |
| freshness | FreshnessBadge | kpis.freshness | kpis.freshness |
| kpis | KpiGrid | repeat(4, minmax(12rem, 1fr)) | unit economics + ledger highlights |
| links | DrillDownLinks | /billing, /reports/discrepancy-buy-sell | permissions |
...
```
GET /api/v1/dashboards/cfo?customer_id={uuid}&from={iso}&to={iso}
```
...
| web/src/ui/dashboard/cfo_dashboard.tsx | KPI + sections |
...
| :--- | :--- | :--- | :--- |
| 1 | GET /api/v1/dashboards/cfo | Confirm operation_id + handler DTO fields | openapi_gate.sh + handler test |
