# ADMIN_REPORT_MILESTONE_SPEND_VELOCITY

Report: spend-velocity.

**Status:** DRAFT  
**Slug:** `admin_report_spend_velocity`  
...
- `/reports/spend-velocity` — Spend velocity hourly series
- API: `GET /api/v1/reports/spend-velocity`
- Column contract: Handler row keys (ReportMapEnvelope): bucket, spend_micro, clicks
...
| :--- | :--- | :--- |
| GET | GET /api/v1/reports/spend-velocity | reportSpendVelocity |
| Columns | Handler row keys (ReportMapEnvelope): bucket, spend_micro, clicks | handler |
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Report title from catalog or static (spend-velocity) | GET /api/v1/reports/catalog or reportTitle() |
...
| content | report_spend_velocity | role=grid; column keys from OpenAPI/handler rows[] | rows[] or typed response |
...
| `live` | true |
| Handler | internal/reports/ — `reportSpendVelocity` |

### 4.3 Layout and placement (grid contract)


**Non-sortable headers:** bucket, spend_micro, clicks
...
```
/api/v1/reports/spend-velocity?customer_id={uuid}&from={iso}&to={iso}
```
...
| Path | Role |
| :--- | :--- |
| web/src/ui/reports/report_spend_velocity.tsx | ReportDataGrid section |
| web/src/ui/reports/report_spend_velocity.module.css | Grid/heatmap CSS module |
| web/src/helpers/reports_api.ts | fetchReport('spend-velocity', params) |
| web/src/pages/report_route_pages.tsx | Compose until ui/reports migration |

**Legacy page:** `report_configs.ts SIMPLE_REPORT_CONFIGS spend-velocity`
...
| :--- | :--- | :--- | :--- |
| 1 | api/openapi/paths/ops_reports.yaml | Confirm `reportSpendVelocity` params + response schema | openapi_gate.sh |
| 2 | make openapi-types | Regenerate openapi.d.ts | typecheck |
| 3 | web/src/helpers/reports_api.ts | fetchReport('spend-velocity', params) | Compiles |
| 4 | web/src/app_routes.tsx | Register `/reports/spend-velocity` (legacy compose until ui/reports lands) | Route resolves |
| 5 | web/src/ui/reports/report_spend_velocity.tsx | ReportDataGrid per 4.3 | surface gate |
...
```bash
curl -s '/api/v1/reports/spend-velocity?customer_id={uuid}&from={iso}&to={iso}' | jq '.freshness'
```

| Check | Command / procedure | Pass criteria |
| :--- | :--- | :--- |
| No ghost labels | rg 'ghost_' web/src/ui/reports/report_spend_velocity | no matches |
...
- [ ] Legacy `report_configs.ts SIMPLE_REPORT_CONFIGS spend-velocity` replaced or delegates to `ui/reports/`
