# ADMIN_REPORT_MILESTONE_PACING_DRIFT

Report: pacing-drift.

**Status:** DRAFT  
**Slug:** `admin_report_pacing_drift`  
...
- `/reports/pacing-drift` — Campaign pacing drift vs plan
- API: `GET /api/v1/reports/pacing-drift`
- Column contract: Handler row keys (ReportMapEnvelope): campaign_id, date, planned_spend_micro, actual_spend_micro, drift_pct, pacing_mode
...
| :--- | :--- | :--- |
| GET | GET /api/v1/reports/pacing-drift | reportPacingDrift |
| Columns | Handler row keys (ReportMapEnvelope): campaign_id, date, planned_spend_micro, actual_spend_micro, drift_pct, pacing_mode | handler |
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Report title from catalog or static (pacing-drift) | GET /api/v1/reports/catalog or reportTitle() |
...
| content | report_pacing_drift | role=grid; column keys from OpenAPI/handler rows[] | rows[] or typed response |
...
| `live` | true |
| Handler | internal/reports/ — `reportPacingDrift` |

### 4.3 Layout and placement (grid contract)


**Non-sortable headers:** campaign_id, date, planned_spend_micro, actual_spend_micro, drift_pct, pacing_mode
...
```
/api/v1/reports/pacing-drift?customer_id={uuid}&from={iso}&to={iso}
```
...
| Path | Role |
| :--- | :--- |
| web/src/ui/reports/report_pacing_drift.tsx | ReportDataGrid section |
| web/src/ui/reports/report_pacing_drift.module.css | Grid/heatmap CSS module |
| web/src/helpers/reports_api.ts | fetchReport('pacing-drift', params) |
| web/src/pages/report_route_pages.tsx | Compose until ui/reports migration |

**Legacy page:** `report_route_pages.tsx PacingDriftReportPage`
...
| :--- | :--- | :--- | :--- |
| 1 | api/openapi/paths/ops_reports.yaml | Confirm `reportPacingDrift` params + response schema | openapi_gate.sh |
| 2 | make openapi-types | Regenerate openapi.d.ts | typecheck |
| 3 | web/src/helpers/reports_api.ts | fetchReport('pacing-drift', params) | Compiles |
| 4 | web/src/app_routes.tsx | Register `/reports/pacing-drift` (legacy compose until ui/reports lands) | Route resolves |
| 5 | web/src/ui/reports/report_pacing_drift.tsx | ReportDataGrid per 4.3 | surface gate |
...
```bash
curl -s '/api/v1/reports/pacing-drift?customer_id={uuid}&from={iso}&to={iso}' | jq '.freshness'
```

| Check | Command / procedure | Pass criteria |
| :--- | :--- | :--- |
| No ghost labels | rg 'ghost_' web/src/ui/reports/report_pacing_drift | no matches |
...
- [ ] Legacy `report_route_pages.tsx PacingDriftReportPage` replaced or delegates to `ui/reports/`
