# ADMIN_SETTINGS_MILESTONE_DISPUTES

Settings: /settings/disputes.

**Route gap:** register `/settings/disputes` in `app_routes.tsx` before `live: true`.

**Status:** DRAFT  
**Slug:** `admin_settings_disputes`  
...
- `/settings/disputes` — Chargeback/dispute grid; customer_id filter; server pagination
...
| :--- | :--- | :--- |
| Route absent | Handler exists; SPA route not registered | Follow route_registration steps in in_scope |
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Payment disputes | static |
| filters | DisputesFilter | customer_id required/optional per OpenAPI | CustomerIdQuery |
| grid | DisputesGrid | dispute rows | DisputeListResponse items[] |
| footer | PaginationBar | limit/offset | envelope |
| empty | EmptyState | No disputes | items.length===0 |
| error | ErrorBlock | List failure | errors |
...
| Topic | Spec |
| :--- | :--- |
| CSS ownership | `web/src/ui/disputes/*.module.css` only |
...
| :--- | :--- | :--- |
| Primary fetch | GET /api/v1/disputes | Server owns business rules |
...
| Path | Role |
| :--- | :--- |
| web/src/pages/disputes_page.tsx | Compose |
| web/src/ui/disputes/* | Settings sections + CSS modules |
| web/src/helpers/disputes_api.ts | API helpers |
...
| 3 | web/src/helpers/disputes_api.ts | Typed GET/PATCH/POST helpers | Compiles |
| 4 | web/src/ui/disputes/* | Form/grid sections per 4.1 | check_ui_surface_gate.sh |
| 5 | web/src/pages/disputes_page.tsx | Compose; no CSS modules on page | ≤120 lines |
...
Revert `web/src/ui/disputes/`, helpers, and page compose in one slice; no half migration.
