# ADMIN_SETTINGS_MILESTONE_PLATFORM

Settings: /settings.

**Status:** DRAFT  
**Slug:** `admin_settings_platform`  
...
- `/settings` — Platform knobs; PATCH ⊆ platformconfig.Patch; apply writes install root; doctor panel
- API: GET/PATCH /api/v1/settings/platform; POST .../platform/apply; GET .../ops/doctor; POST .../ops/roles/reload
- PATCH fields: tracking_domain, default_currency, timezone, ingress_schema, telemetry_enabled, profile, edge_xdp, edge_expose_click, edge_expose_openrtb, network_interface, stripe.enabled, stripe.secret_key, stripe.webhook_secret, stripe.checkout_success_url, stripe.checkout_cancel_url
- Confirm modal: PATCH platform, POST platform/apply, POST ops/roles/reload
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Platform settings | static |
| summary | SettingsSummary | Read-only config snapshot | GET platform config |
| form | PlatformConfigForm | PATCH fields per OpenAPI | PlatformSettingsPatch |
| stripe | StripeSecretsForm | secret_key/webhook inputs cleared after save | stripe patch |
| doctor | DoctorPanel | Stack health | GET /api/v1/ops/doctor |
| toolbar | SettingsToolbar | Save PATCH; Apply to disk | settings:write |
| restart | RestartRequiredBanner | restart_required[] chips | PATCH response |
| error | ErrorBlock | GET/PATCH failure | errors |
...
| :--- | :--- | :--- |
| Primary fetch | GET/PATCH /api/v1/settings/platform; POST .../platform/apply; GET .../ops/doctor; POST .../ops/roles/reload | Server owns business rules |
...
| Path | Role |
| :--- | :--- |
| web/src/pages/settings_page.tsx | Compose |
...
| 5 | web/src/pages/settings_page.tsx | Compose; no CSS modules on page | ≤120 lines |
...
- [ ] Legacy `settings_page.tsx` replaced or delegates to `ui/settings/`
