...
- `/ops/ml-model` — ML model status; eval run; manual label import
- API: GET/PATCH /api/v1/ops/ml-model; POST .../ml-model/eval; GET/POST .../ml-model/labels
- Confirm modal: POST /api/v1/ops/ml-model/eval, POST .../ml-model/labels import
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | ML model ops | static |
| status | MlModelStatus | Current model metadata | GET .../ml-model |
| config | MlModelPatchForm | PATCH model settings | PATCH .../ml-model |
| eval | MlEvalPanel | Run eval job | POST .../eval |
| labels | MlLabelsImport | Label CSV/import | POST .../labels |
| error | ErrorBlock | Fetch/eval failure | errors |
...
| :--- | :--- | :--- |
| Primary fetch | GET/PATCH /api/v1/ops/ml-model; POST .../ml-model/eval; GET/POST .../ml-model/labels | Server owns business rules |
...
| Path | Role |
| :--- | :--- |
| web/src/pages/ops_ml_model_page.tsx | Compose only |
...
**Legacy page:** `ops_ml_model_page.tsx`
...
| 5 | web/src/pages/ops_ml_model_page.tsx | Thin compose; permission gates | No *.module.css on page |
...
- [ ] Legacy `ops_ml_model_page.tsx` replaced or delegates to `ui/ops/`
