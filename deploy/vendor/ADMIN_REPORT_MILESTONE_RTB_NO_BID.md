# ADMIN_REPORT_MILESTONE_RTB_NO_BID

Report: rtb-no-bid-reasons.

**Status:** DRAFT  
**Slug:** `admin_report_rtb_no_bid_reasons`  
...
- `/reports/rtb/no-bid-reasons` — RTB no-bid reason breakdown
- API: `GET /api/v1/reports/rtb/no-bid-reasons`
- Column contract: Handler row keys (ReportMapEnvelope): no_bid_reason, bid_count
...
| :--- | :--- | :--- |
| GET | GET /api/v1/reports/rtb/no-bid-reasons | reportRtbNoBidReasons |
| Columns | Handler row keys (ReportMapEnvelope): no_bid_reason, bid_count | handler |
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Report title from catalog or static (rtb-no-bid-reasons) | GET /api/v1/reports/catalog or reportTitle() |
...
| content | report_rtb_no_bid_reasons | role=grid; column keys from OpenAPI/handler rows[] | rows[] or typed response |
...
| `live` | true |
| Handler | internal/reports/ — `reportRtbNoBidReasons` |

### 4.3 Layout and placement (grid contract)


**Non-sortable headers:** no_bid_reason, bid_count
...
```
/api/v1/reports/rtb/no-bid-reasons?customer_id={uuid}&from={iso}&to={iso}
```
...
| Path | Role |
| :--- | :--- |
| web/src/ui/reports/report_rtb_no_bid_reasons.tsx | ReportDataGrid section |
| web/src/ui/reports/report_rtb_no_bid_reasons.module.css | Grid/heatmap CSS module |
| web/src/helpers/reports_api.ts | fetchReport('rtb-no-bid-reasons', params) |
| web/src/pages/report_route_pages.tsx | Compose until ui/reports migration |

**Legacy page:** `report_route_pages.tsx RtbNoBidReasonsReportPage`
...
| :--- | :--- | :--- | :--- |
| 1 | api/openapi/paths/ops_reports.yaml | Confirm `reportRtbNoBidReasons` params + response schema | openapi_gate.sh |
| 2 | make openapi-types | Regenerate openapi.d.ts | typecheck |
| 3 | web/src/helpers/reports_api.ts | fetchReport('rtb-no-bid-reasons', params) | Compiles |
| 4 | web/src/app_routes.tsx | Register `/reports/rtb/no-bid-reasons` (legacy compose until ui/reports lands) | Route resolves |
| 5 | web/src/ui/reports/report_rtb_no_bid_reasons.tsx | ReportDataGrid per 4.3 | surface gate |
...
```bash
curl -s '/api/v1/reports/rtb/no-bid-reasons?customer_id={uuid}&from={iso}&to={iso}' | jq '.freshness'
```

| Check | Command / procedure | Pass criteria |
| :--- | :--- | :--- |
| No ghost labels | rg 'ghost_' web/src/ui/reports/report_rtb_no_bid_reasons | no matches |
...
- [ ] Legacy `report_route_pages.tsx RtbNoBidReasonsReportPage` replaced or delegates to `ui/reports/`
