# ADMIN_INTEGRATIONS_MILESTONE_HUB

Integration: /integrations.

**Status:** DRAFT  
**Slug:** `admin_integrations_hub`  
...
- `/integrations` — Card grid hub linking to each integration sub-route
- APIs: GET /api/v1/session (nav permissions); static card catalog
- Forms ⊆ OpenAPI; StubBanner on 501
- Confirm modal on destructive/retry/run actions
- RBAC: `session permissions gate cards` on write actions
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Integrations | static |
| intro | HubIntro | One-line hub description | static |
| cards | IntegrationCardGrid | repeat(auto-fill, minmax(16rem, 1fr)) | static routes + permission filter |
| card_cost_sync | IntegrationCard | Cost sync -> /integrations/cost-sync | campaigns:read |
| card_postbacks | IntegrationCard | Postbacks -> /integrations/postbacks | campaigns:read |
| card_supply | IntegrationCard | Supply -> /integrations/supply | settings:read |
| card_platform | IntegrationCard | Platform links -> /integrations/platform-campaigns | license gated |
| error | ErrorBlock | session fetch failure | errors |
...
| Topic | Spec |
| :--- | :--- |
| CSS ownership | `web/src/ui/integrations/*.module.css` only |
...
| Grid token | `--integrations-cols`: repeat(auto-fill, minmax(16rem, 1fr)) |

### 4.5 API and cold path (what the browser does not compute)

| Concern | Handler / OpenAPI | Browser |
| :--- | :--- | :--- |
| Primary fetch | GET /api/v1/session (nav permissions); static card catalog | Server owns business rules |
...
| Path | Role |
| :--- | :--- |
| web/src/pages/integrations_hub_page.tsx | Compose |
| web/src/ui/integrations/* | Sections + CSS modules |
| web/src/helpers/integrations_api.ts | Typed API helpers |
| web/src/types/generated/openapi.d.ts | Generated types |

**Legacy page:** `integrations_hub_page.tsx`
...
| :--- | :--- | :--- | :--- |
| 1 | GET /api/v1/session (nav permissions) | Confirm OpenAPI ops + x-permissions | openapi_gate.sh |
...
| 4 | web/src/helpers/integrations_api.ts | Typed API helpers; secrets never logged | Compiles |
| 5 | web/src/ui/integrations/* | Sections per regions + grid_ascii | check_ui_surface_gate.sh |
| 6 | web/src/pages/integrations_hub_page.tsx | Compose + URL params | No client filter on items[] |
...
| :--- | :--- | :--- |
| No client sort/filter | rg 'useMemo.*filter\|table_sort' web/src/pages/integrations_hub_page.tsx web/src/ui/integrations/ | no matches |
...
- [ ] Legacy `integrations_hub_page.tsx` replaced or delegates to `ui/integrations/`
- [ ] Verification output pasted in PR

## 9. Rollback

Revert `web/src/ui/integrations/`, helpers, and page compose in one slice; no half migration.
