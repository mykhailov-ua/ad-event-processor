# ADMIN_DASHBOARD_MILESTONE_CAMPAIGN

Dashboard: Campaign dashboard.

**Status:** DRAFT  
**Slug:** `admin_dashboard_campaign`  
...
- Route `/dashboards/campaign/:id`
- Single-campaign KPI strip + series
- API: `GET /api/v1/dashboards/campaign/{id}` (`dashboardCampaign`)
- KPI keys: kpis.spend_micro, kpis.revenue_micro, kpis.profit_micro, kpis.conversions, kpis.cpa_micro, kpis.roi_pct, freshness.stale, freshness.ch_lag_seconds
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Campaign name from GET /campaigns/{id} or dashboard | campaign_id |
| freshness | FreshnessBadge | freshness.stale, ch_lag_seconds | freshness |
| kpis | KpiGrid | repeat(3, minmax(12rem, 1fr)) | kpis MetricsBlockDTO |
| series | CampaignSeriesChart | series[] label, spend_micros | series[] |
| links | DrillDownLinks | /campaigns/:id, /reports/campaign-overview | static |
...
```
GET /api/v1/dashboards/campaign/{id}?customer_id={uuid}&from={iso}&to={iso}
```
...
| web/src/ui/dashboard/campaign_dashboard.tsx | KPI + sections |
...
| :--- | :--- | :--- | :--- |
| 1 | GET /api/v1/dashboards/campaign/{id} | Confirm operation_id + handler DTO fields | openapi_gate.sh + handler test |
