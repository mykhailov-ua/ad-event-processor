# ADMIN_REPORT_MILESTONE_DAYPART

Report: daypart-heatmap.

**Status:** DRAFT  
**Slug:** `admin_report_daypart_heatmap`  
...
- `/reports/daypart-heatmap` — Daypart heatmap (hour × weekday)
- API: `GET /api/v1/reports/daypart-heatmap`
- Column contract: Handler row keys (ReportMapEnvelope): hour, clicks
...
| :--- | :--- | :--- |
| GET | GET /api/v1/reports/daypart-heatmap | reportDaypartHeatmap |
| Columns | Handler row keys (ReportMapEnvelope): hour, clicks | handler |
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Report title from catalog or static (daypart-heatmap) | GET /api/v1/reports/catalog or reportTitle() |
...
| content | report_daypart_heatmap | hour × weekday heatmap; intensity from row.clicks | rows[] or typed response |
...
| `live` | true |
| Handler | internal/reports/ — `reportDaypartHeatmap` |

### 4.3 Layout and placement (grid contract)


**Non-sortable headers:** hour, clicks
...
```
/api/v1/reports/daypart-heatmap?customer_id={uuid}&from={iso}&to={iso}
```
...
| Path | Role |
| :--- | :--- |
| web/src/ui/reports/report_daypart_heatmap.tsx | DaypartHeatmap section |
| web/src/ui/reports/report_daypart_heatmap.module.css | Grid/heatmap CSS module |
| web/src/helpers/reports_api.ts | fetchReport('daypart-heatmap', params) |
| web/src/pages/report_route_pages.tsx | Compose until ui/reports migration |

**Legacy page:** `report_configs.ts daypart-heatmap`
...
| :--- | :--- | :--- | :--- |
| 1 | api/openapi/paths/ops_reports.yaml | Confirm `reportDaypartHeatmap` params + response schema | openapi_gate.sh |
| 2 | make openapi-types | Regenerate openapi.d.ts | typecheck |
| 3 | web/src/helpers/reports_api.ts | fetchReport('daypart-heatmap', params) | Compiles |
| 4 | web/src/app_routes.tsx | Register `/reports/daypart-heatmap` (legacy compose until ui/reports lands) | Route resolves |
| 5 | web/src/ui/reports/report_daypart_heatmap.tsx | DaypartHeatmap per 4.3 | surface gate |
...
```bash
curl -s '/api/v1/reports/daypart-heatmap?customer_id={uuid}&from={iso}&to={iso}' | jq '.freshness'
```

| Check | Command / procedure | Pass criteria |
| :--- | :--- | :--- |
| No ghost labels | rg 'ghost_' web/src/ui/reports/report_daypart_heatmap | no matches |
...
- [ ] Legacy `report_configs.ts daypart-heatmap` replaced or delegates to `ui/reports/`
