# ADMIN_REPORT_MILESTONE_CUSTOMER_PORTFOLIO

Report: customer-portfolio.

**Status:** DRAFT  
**Slug:** `admin_report_customer_portfolio`  
...
- `/reports/customer-portfolio` — Customer portfolio rollup
- API: `GET /api/v1/reports/customer-portfolio`
- Column contract: Handler row keys (ReportMapEnvelope): row_type, campaign_id, name, status, active, paused, impressions_7d, clicks_7d, utilization_pct, pacing_drift_pct, overspend_risk
...
| :--- | :--- | :--- |
| GET | GET /api/v1/reports/customer-portfolio | reportCustomerPortfolio |
| Columns | Handler row keys (ReportMapEnvelope): row_type, campaign_id, name, status, active, paused, impressions_7d, clicks_7d, utilization_pct, pacing_drift_pct, overspend_risk | handler |
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Report title from catalog or static (customer-portfolio) | GET /api/v1/reports/catalog or reportTitle() |
...
| content | report_customer_portfolio | role=grid; column keys from OpenAPI/handler rows[] | rows[] or typed response |
...
| `live` | true |
| Handler | internal/reports/ — `reportCustomerPortfolio` |

### 4.3 Layout and placement (grid contract)


**Non-sortable headers:** row_type, campaign_id, name, status, active, paused, impressions_7d, clicks_7d, utilization_pct, pacing_drift_pct, overspend_risk
...
```
/api/v1/reports/customer-portfolio?customer_id={uuid}&from={iso}&to={iso}
```
...
| Path | Role |
| :--- | :--- |
| web/src/ui/reports/report_customer_portfolio.tsx | ReportDataGrid section |
| web/src/ui/reports/report_customer_portfolio.module.css | Grid/heatmap CSS module |
| web/src/helpers/reports_api.ts | fetchReport('customer-portfolio', params) |
| web/src/pages/report_route_pages.tsx | Compose until ui/reports migration |

**Legacy page:** `report_configs.ts customer-portfolio`
...
| :--- | :--- | :--- | :--- |
| 1 | api/openapi/paths/ops_reports.yaml | Confirm `reportCustomerPortfolio` params + response schema | openapi_gate.sh |
| 2 | make openapi-types | Regenerate openapi.d.ts | typecheck |
| 3 | web/src/helpers/reports_api.ts | fetchReport('customer-portfolio', params) | Compiles |
| 4 | web/src/app_routes.tsx | Register `/reports/customer-portfolio` (legacy compose until ui/reports lands) | Route resolves |
| 5 | web/src/ui/reports/report_customer_portfolio.tsx | ReportDataGrid per 4.3 | surface gate |
...
```bash
curl -s '/api/v1/reports/customer-portfolio?customer_id={uuid}&from={iso}&to={iso}' | jq '.freshness'
```

| Check | Command / procedure | Pass criteria |
| :--- | :--- | :--- |
| No ghost labels | rg 'ghost_' web/src/ui/reports/report_customer_portfolio | no matches |
...
| Export POST | POST /api/v1/reports/jobs body report_key=customer-portfolio | 202 + job id |
...
- [ ] Legacy `report_configs.ts customer-portfolio` replaced or delegates to `ui/reports/`
