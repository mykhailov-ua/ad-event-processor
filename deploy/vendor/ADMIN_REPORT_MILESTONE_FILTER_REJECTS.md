# ADMIN_REPORT_MILESTONE_FILTER_REJECTS

Report: filter-rejects.

**Status:** DRAFT  
**Slug:** `admin_report_filter_rejects`  
...
- `/reports/filter-rejects` — Unified filter reject reason breakdown
- API: `GET /api/v1/reports/filter-rejects`
- Column contract: Handler row keys (ReportMapEnvelope): reject_kind, reject_count
...
| :--- | :--- | :--- |
| GET | GET /api/v1/reports/filter-rejects | reportFilterRejects |
| Columns | Handler row keys (ReportMapEnvelope): reject_kind, reject_count | handler |
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Report title from catalog or static (filter-rejects) | GET /api/v1/reports/catalog or reportTitle() |
...
| content | report_filter_rejects | role=grid; column keys from OpenAPI/handler rows[] | rows[] or typed response |
...
| `live` | true |
| Handler | internal/reports/ — `reportFilterRejects` |

### 4.3 Layout and placement (grid contract)


**Non-sortable headers:** reject_kind, reject_count
...
```
/api/v1/reports/filter-rejects?customer_id={uuid}&from={iso}&to={iso}
```
...
| Path | Role |
| :--- | :--- |
| web/src/ui/reports/report_filter_rejects.tsx | ReportDataGrid section |
| web/src/ui/reports/report_filter_rejects.module.css | Grid/heatmap CSS module |
| web/src/helpers/reports_api.ts | fetchReport('filter-rejects', params) |
| web/src/pages/report_route_pages.tsx | Compose until ui/reports migration |

**Legacy page:** `report_route_pages.tsx FilterRejectsReportPage`
...
| :--- | :--- | :--- | :--- |
| 1 | api/openapi/paths/ops_reports.yaml | Confirm `reportFilterRejects` params + response schema | openapi_gate.sh |
| 2 | make openapi-types | Regenerate openapi.d.ts | typecheck |
| 3 | web/src/helpers/reports_api.ts | fetchReport('filter-rejects', params) | Compiles |
| 4 | web/src/app_routes.tsx | Register `/reports/filter-rejects` (legacy compose until ui/reports lands) | Route resolves |
| 5 | web/src/ui/reports/report_filter_rejects.tsx | ReportDataGrid per 4.3 | surface gate |
...
```bash
curl -s '/api/v1/reports/filter-rejects?customer_id={uuid}&from={iso}&to={iso}' | jq '.freshness'
```

| Check | Command / procedure | Pass criteria |
| :--- | :--- | :--- |
| No ghost labels | rg 'ghost_' web/src/ui/reports/report_filter_rejects | no matches |
...
| Export POST | POST /api/v1/reports/jobs body report_key=filter-rejects | 202 + job id |
...
- [ ] Legacy `report_route_pages.tsx FilterRejectsReportPage` replaced or delegates to `ui/reports/`
