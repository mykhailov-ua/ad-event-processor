# ADMIN_INTEGRATIONS_MILESTONE_AUTOMATION

Integration: /integrations/automation.

**Status:** DRAFT  
**Slug:** `admin_integrations_automation`  
...
- `/integrations/automation` — Preset catalog; rule CRUD; dry-run matches panel
- APIs: GET /api/v1/automation/presets; GET/POST /api/v1/automation/rules; PUT/DELETE .../rules/{id}; POST .../rules/{id}/dry-run
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Automation rules | static |
| scope | CustomerScopeBar | customer_id | URL |
| presets | PresetCatalog | bundled presets | GET presets |
| rules | RulesGrid | metric, operator, threshold, action | GET rules |
| rule_form | RuleForm | UpsertAutomationRuleRequest or preset_key | POST/PUT rules |
| dry_run | DryRunPanel | matches[] from CH metrics | POST .../dry-run |
...
| Topic | Spec |
| :--- | :--- |
| CSS ownership | `web/src/ui/automation/*.module.css` only |
...
| :--- | :--- | :--- | :--- |
| customer_id | customer_id |  | required on rules list |
...
| Path | Role |
| :--- | :--- |
| web/src/pages/integrations_automation_page.tsx | Compose |
| web/src/ui/automation/* | Sections + CSS modules |
| web/src/helpers/automation_api.ts | Typed API helpers |
| web/src/types/generated/openapi.d.ts | Generated types |

**Legacy page:** `integrations_automation_page.tsx`
...
| :--- | :--- | :--- | :--- |
| 1 | GET /api/v1/automation/presets | Confirm OpenAPI ops + x-permissions | openapi_gate.sh |
...
| 4 | web/src/helpers/automation_api.ts | Typed API helpers; secrets never logged | Compiles |
| 5 | web/src/ui/automation/* | Sections per regions + grid_ascii | check_ui_surface_gate.sh |
| 6 | web/src/pages/integrations_automation_page.tsx | Compose + URL params | No client filter on items[] |
...
| :--- | :--- | :--- |
| No client sort/filter | rg 'useMemo.*filter\|table_sort' web/src/pages/integrations_automation_page.tsx web/src/ui/automation/ | no matches |
...
- [ ] Legacy `integrations_automation_page.tsx` replaced or delegates to `ui/automation/`
- [ ] Verification output pasted in PR

## 9. Rollback

Revert `web/src/ui/automation/`, helpers, and page compose in one slice; no half migration.
