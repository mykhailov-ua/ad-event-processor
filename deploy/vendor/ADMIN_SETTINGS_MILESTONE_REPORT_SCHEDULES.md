# ADMIN_SETTINGS_MILESTONE_REPORT_SCHEDULES

Settings: /settings/report-schedules.

**Route gap:** register `/settings/report-schedules` in `app_routes.tsx` before `live: true`.

**Status:** DRAFT  
**Slug:** `admin_settings_report_schedules`  
...
- `/settings/report-schedules` — Scheduled report delivery CRUD; customer_id scoped
- API: GET/POST /api/v1/report-schedules; GET/PUT/DELETE .../report-schedules/{id}
- Confirm modal: DELETE schedule
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Report schedules | static |
| toolbar | SchedulesToolbar | Create schedule | reportSchedulesCreate |
| grid | SchedulesGrid | cron, report_key, destination | ReportSchedule[] |
| editor | ScheduleEditor | PUT update modal/drawer | reportSchedulesUpdate |
| empty | EmptyState | No schedules | array empty |
| error | ErrorBlock | CRUD failure | errors |
...
| :--- | :--- | :--- |
| Primary fetch | GET/POST /api/v1/report-schedules; GET/PUT/DELETE .../report-schedules/{id} | Server owns business rules |
