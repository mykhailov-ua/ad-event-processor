# ADMIN_REPORT_MILESTONE_COST_SYNC_COV

Report: cost-sync-coverage.

**Status:** DRAFT  
**Slug:** `admin_report_cost_sync_coverage`  
...
- `/reports/cost-sync-coverage` — Cost sync attribution coverage gaps
- API: `GET /api/v1/reports/cost-sync-coverage`
- Column contract: Handler row keys (ReportMapEnvelope): campaign_id, clicks, spend_micro, coverage_gap, network, last_sync_status
...
| :--- | :--- | :--- |
| GET | GET /api/v1/reports/cost-sync-coverage | reportCostSyncCoverage |
| Columns | Handler row keys (ReportMapEnvelope): campaign_id, clicks, spend_micro, coverage_gap, network, last_sync_status | handler |
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Report title from catalog or static (cost-sync-coverage) | GET /api/v1/reports/catalog or reportTitle() |
...
| content | report_cost_sync_coverage | role=grid; column keys from OpenAPI/handler rows[] | rows[] or typed response |
...
| `live` | true |
| Handler | internal/reports/ — `reportCostSyncCoverage` |

### 4.3 Layout and placement (grid contract)


**Non-sortable headers:** campaign_id, clicks, spend_micro, coverage_gap, network, last_sync_status
...
```
/api/v1/reports/cost-sync-coverage?customer_id={uuid}&from={iso}&to={iso}
```
...
| Path | Role |
| :--- | :--- |
| web/src/ui/reports/report_cost_sync_coverage.tsx | ReportDataGrid section |
| web/src/ui/reports/report_cost_sync_coverage.module.css | Grid/heatmap CSS module |
| web/src/helpers/reports_api.ts | fetchReport('cost-sync-coverage', params) |
| web/src/pages/report_route_pages.tsx | Compose until ui/reports migration |

**Legacy page:** `report_route_pages.tsx CostSyncCoverageReportPage`
...
| :--- | :--- | :--- | :--- |
| 1 | api/openapi/paths/ops_reports.yaml | Confirm `reportCostSyncCoverage` params + response schema | openapi_gate.sh |
| 2 | make openapi-types | Regenerate openapi.d.ts | typecheck |
| 3 | web/src/helpers/reports_api.ts | fetchReport('cost-sync-coverage', params) | Compiles |
| 4 | web/src/app_routes.tsx | Register `/reports/cost-sync-coverage` (legacy compose until ui/reports lands) | Route resolves |
| 5 | web/src/ui/reports/report_cost_sync_coverage.tsx | ReportDataGrid per 4.3 | surface gate |
...
```bash
curl -s '/api/v1/reports/cost-sync-coverage?customer_id={uuid}&from={iso}&to={iso}' | jq '.freshness'
```

| Check | Command / procedure | Pass criteria |
| :--- | :--- | :--- |
| No ghost labels | rg 'ghost_' web/src/ui/reports/report_cost_sync_coverage | no matches |
...
- [ ] Legacy `report_route_pages.tsx CostSyncCoverageReportPage` replaced or delegates to `ui/reports/`
