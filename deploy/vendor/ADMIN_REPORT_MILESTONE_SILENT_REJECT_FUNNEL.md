# ADMIN_REPORT_MILESTONE_SILENT_REJECT_FUNNEL

Report: silent-reject-impression-funnel.

**Status:** DRAFT  
**Slug:** `admin_report_silent_reject_impression_funnel`  
...
- Ghost impression funnel title
- ghost_* column headers
...
- `/reports/silent-reject-impression-funnel` — Silent reject impression funnel (canonical; API alias GET /ghost-impression-funnel)
- API: `GET /api/v1/reports/silent-reject-impression-funnel`
- Column contract: Handler row keys (ReportMapEnvelope): campaign_id, placement_id, billable_impressions, silent_reject_impressions, ivt_impressions, silent_reject_rate, ivt_impression_rate
...
| :--- | :--- | :--- |
| GET | GET /api/v1/reports/silent-reject-impression-funnel | reportSilentRejectFunnel |
| Columns | Handler row keys (ReportMapEnvelope): campaign_id, placement_id, billable_impressions, silent_reject_impressions, ivt_impressions, silent_reject_rate, ivt_impression_rate | handler |
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Report title from catalog or static (silent-reject-impression-funnel) | GET /api/v1/reports/catalog or reportTitle() |
...
| content | report_silent_reject_impression_funnel | role=grid; column keys from OpenAPI/handler rows[] | rows[] or typed response |
...
| `live` | true |
| Handler | internal/reports/ — `reportSilentRejectFunnel` |

### 4.3 Layout and placement (grid contract)


**Non-sortable headers:** campaign_id, placement_id, billable_impressions, silent_reject_impressions, ivt_impressions, silent_reject_rate, ivt_impression_rate
...
```
/api/v1/reports/silent-reject-impression-funnel?customer_id={uuid}&from={iso}&to={iso}
```
...
| Path | Role |
| :--- | :--- |
| web/src/ui/reports/report_silent_reject_impression_funnel.tsx | ReportDataGrid section |
| web/src/ui/reports/report_silent_reject_impression_funnel.module.css | Grid/heatmap CSS module |
| web/src/helpers/reports_api.ts | fetchReport('silent-reject-impression-funnel', params) |
| web/src/pages/report_route_pages.tsx | Compose until ui/reports migration |

**Legacy page:** `report_route_pages.tsx SilentRejectImpressionFunnelReportPage`
...
| :--- | :--- | :--- | :--- |
| 1 | api/openapi/paths/ops_reports.yaml | Confirm `reportSilentRejectFunnel` params + response schema | openapi_gate.sh |
| 2 | make openapi-types | Regenerate openapi.d.ts | typecheck |
| 3 | web/src/helpers/reports_api.ts | fetchReport('silent-reject-impression-funnel', params) | Compiles |
| 4 | web/src/app_routes.tsx | Register `/reports/silent-reject-impression-funnel` (legacy compose until ui/reports lands) | Route resolves |
| 5 | web/src/ui/reports/report_silent_reject_impression_funnel.tsx | ReportDataGrid per 4.3 | surface gate |
...
```bash
curl -s '/api/v1/reports/silent-reject-impression-funnel?customer_id={uuid}&from={iso}&to={iso}' | jq '.freshness'
```

| Check | Command / procedure | Pass criteria |
| :--- | :--- | :--- |
| No ghost labels | rg 'ghost_' web/src/ui/reports/report_silent_reject_impression_funnel | no matches |
...
| Export POST | POST /api/v1/reports/jobs body report_key=silent-reject-impression-funnel | 202 + job id |
| Export poll | GET /api/v1/reports/jobs/{id} until status complete\|failed | poll UI stops on terminal status |
| Export download | GET /api/v1/reports/jobs/{id}/download | CSV/JSON attachment when complete |
...
- [ ] Legacy `report_route_pages.tsx SilentRejectImpressionFunnelReportPage` replaced or delegates to `ui/reports/`
