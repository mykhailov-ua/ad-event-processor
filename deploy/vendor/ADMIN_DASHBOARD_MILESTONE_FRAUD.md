# ADMIN_DASHBOARD_MILESTONE_FRAUD

Dashboard: Fraud.

**Status:** DRAFT  
**Slug:** `admin_dashboard_fraud`  
...
- Route `/dashboards/fraud`
- ML eval, silent reject counts, edge blocks, geo hints — canonical silent_reject_*
- API: `GET /api/v1/dashboards/fraud` (`dashboardFraud`)
- KPI keys: silent_reject_campaigns, labels_pending, edge_blocked_fraud, ml_precision, ml_recall, ml_eval_stale, ml_drift_detected, fraud_tier_thresholds.pass_max, fraud_tier_thresholds.suspect_max, fraud_tier_thresholds.ivt_max
- FreshnessBadge from freshness DTO only
- RBAC: `audit:read` per OpenAPI x-permissions
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Fraud analyst | static |
| kpis | KpiGrid | repeat(auto-fill, minmax(11rem, 1fr)) | fraud KPI keys |
| ml | MLEvalPanel | ml_active_version_id, ml_artifact_hash, ml_eval_status | ML fields |
| series | FraudSeriesChart | series[] impressions/blocks/spend_micros | series[] |
| geo | GeoHintsGrid | geo_hints[] country, ivt_rate | geo_hints[] |
| labels | RecentLabelsGrid | recent_labels[] | recent_labels[] |
| links | DrillDownLinks | /reports/fraud-breakdown, /reports/silent-reject-impression-funnel | static |
...
```
GET /api/v1/dashboards/fraud?customer_id={uuid}&from={iso}&to={iso}
```
...
| web/src/ui/dashboard/fraud_dashboard.tsx | KPI + sections |
...
| :--- | :--- | :--- | :--- |
| 1 | GET /api/v1/dashboards/fraud | Confirm operation_id + handler DTO fields | openapi_gate.sh + handler test |
...
| KPI key parity | rg 'silent_reject_campaigns' web/src/ui/dashboard/ | maps to API field name |
