# ADMIN_REPORT_MILESTONE_IVT

Report: ivt-by-source.

**Status:** DRAFT  
**Slug:** `admin_report_ivt_by_source`  
...
- `/reports/ivt-by-source` — IVT rate by traffic source
- API: `GET /api/v1/reports/ivt-by-source`
- Column contract: Handler row keys (ReportMapEnvelope): campaign_id, sub1, sub2, country, impressions, ivt_events, ivt_rate
...
| :--- | :--- | :--- |
| GET | GET /api/v1/reports/ivt-by-source | reportIvtBySource |
| Columns | Handler row keys (ReportMapEnvelope): campaign_id, sub1, sub2, country, impressions, ivt_events, ivt_rate | handler |
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Report title from catalog or static (ivt-by-source) | GET /api/v1/reports/catalog or reportTitle() |
...
| content | report_ivt_by_source | role=grid; column keys from OpenAPI/handler rows[] | rows[] or typed response |
...
| `live` | true |
| Handler | internal/reports/ — `reportIvtBySource` |

### 4.3 Layout and placement (grid contract)


**Non-sortable headers:** campaign_id, sub1, sub2, country, impressions, ivt_events, ivt_rate
...
```
/api/v1/reports/ivt-by-source?customer_id={uuid}&from={iso}&to={iso}
```
...
| Path | Role |
| :--- | :--- |
| web/src/ui/reports/report_ivt_by_source.tsx | ReportDataGrid section |
| web/src/ui/reports/report_ivt_by_source.module.css | Grid/heatmap CSS module |
| web/src/helpers/reports_api.ts | fetchReport('ivt-by-source', params) |
| web/src/pages/report_route_pages.tsx | Compose until ui/reports migration |

**Legacy page:** `report_route_pages.tsx IvtBySourceReportPage`
...
| :--- | :--- | :--- | :--- |
| 1 | api/openapi/paths/ops_reports.yaml | Confirm `reportIvtBySource` params + response schema | openapi_gate.sh |
| 2 | make openapi-types | Regenerate openapi.d.ts | typecheck |
| 3 | web/src/helpers/reports_api.ts | fetchReport('ivt-by-source', params) | Compiles |
| 4 | web/src/app_routes.tsx | Register `/reports/ivt-by-source` (legacy compose until ui/reports lands) | Route resolves |
| 5 | web/src/ui/reports/report_ivt_by_source.tsx | ReportDataGrid per 4.3 | surface gate |
...
```bash
curl -s '/api/v1/reports/ivt-by-source?customer_id={uuid}&from={iso}&to={iso}' | jq '.freshness'
```

| Check | Command / procedure | Pass criteria |
| :--- | :--- | :--- |
| No ghost labels | rg 'ghost_' web/src/ui/reports/report_ivt_by_source | no matches |
...
| Export POST | POST /api/v1/reports/jobs body report_key=ivt-by-source | 202 + job id |
...
- [ ] Legacy `report_route_pages.tsx IvtBySourceReportPage` replaced or delegates to `ui/reports/`
