# ADMIN_REPORT_MILESTONE_LAYER_DESYNC_SUMMARY

Report: layer-desync-summary.

**Route gap:** register `/reports/layer-desync-summary` in `app_routes.tsx` before `live: true`.

**Status:** DRAFT  
**Slug:** `admin_report_layer_desync_summary`  
...
- `/reports/layer-desync-summary` — Cross-layer desync fraud counts
- API: `GET /api/v1/reports/layer-desync-summary`
- Column contract: Handler row keys (ReportMapEnvelope): campaign_id, layer_desync_count, event_count, silent_reject_count
...
| :--- | :--- | :--- |
| GET | GET /api/v1/reports/layer-desync-summary | reportLayerDesyncSummary |
| Columns | Handler row keys (ReportMapEnvelope): campaign_id, layer_desync_count, event_count, silent_reject_count | handler |
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Report title from catalog or static (layer-desync-summary) | GET /api/v1/reports/catalog or reportTitle() |
...
| content | report_layer_desync_summary | role=grid; column keys from OpenAPI/handler rows[] | rows[] or typed response |
...
| `live` | false |
| Handler | internal/reports/ — `reportLayerDesyncSummary` |

### 4.3 Layout and placement (grid contract)


**Non-sortable headers:** campaign_id, layer_desync_count, event_count, silent_reject_count
...
```
/api/v1/reports/layer-desync-summary?customer_id={uuid}&from={iso}&to={iso}
```
...
| Path | Role |
| :--- | :--- |
| web/src/ui/reports/report_layer_desync_summary.tsx | ReportDataGrid section |
| web/src/ui/reports/report_layer_desync_summary.module.css | Grid/heatmap CSS module |
| web/src/helpers/reports_api.ts | fetchReport('layer-desync-summary', params) |
| web/src/pages/report_route_pages.tsx | Compose until ui/reports migration |

**Legacy page:** `report_stub_page.tsx (layer-desync-summary)`
...
| :--- | :--- | :--- | :--- |
| 1 | api/openapi/paths/ops_reports.yaml | Confirm `reportLayerDesyncSummary` params + response schema | openapi_gate.sh |
| 2 | make openapi-types | Regenerate openapi.d.ts | typecheck |
| 3 | web/src/helpers/reports_api.ts | fetchReport('layer-desync-summary', params) | Compiles |
| 4 | web/src/app_routes.tsx | Register gap route `/reports/layer-desync-summary` + nav/catalog entry | Route resolves |
| 5 | web/src/ui/reports/report_layer_desync_summary.tsx | ReportDataGrid section | check_ui_surface_gate.sh |
...
```bash
Register `/reports/layer-desync-summary` in app_routes before catalog live:true
curl -s '/api/v1/reports/layer-desync-summary?customer_id={uuid}&from={iso}&to={iso}' | jq '.freshness'
```

| Check | Command / procedure | Pass criteria |
| :--- | :--- | :--- |
| No ghost labels | rg 'ghost_' web/src/ui/reports/report_layer_desync_summary | no matches |
...
- [ ] Legacy `report_stub_page.tsx (layer-desync-summary)` replaced or delegates to `ui/reports/`
