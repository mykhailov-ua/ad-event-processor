# ADMIN_REPORT_MILESTONE_RTB_GEO

Report: rtb-geo-device.

**Status:** DRAFT  
**Slug:** `admin_report_rtb_geo_device`  
...
- `/reports/rtb/geo-device` — RTB geo and device stats
- API: `GET /api/v1/reports/rtb/geo-device`
- Column contract: Handler row keys (ReportMapEnvelope): country, device_os, bids, wins, win_rate, spend_micro
...
| :--- | :--- | :--- |
| GET | GET /api/v1/reports/rtb/geo-device | reportRtbGeoDevice |
| Columns | Handler row keys (ReportMapEnvelope): country, device_os, bids, wins, win_rate, spend_micro | handler |
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Report title from catalog or static (rtb-geo-device) | GET /api/v1/reports/catalog or reportTitle() |
...
| content | report_rtb_geo_device | role=grid; column keys from OpenAPI/handler rows[] | rows[] or typed response |
...
| Field | Spec |
| :--- | :--- |
| Path | `/reports/rtb/geo-device` |
| `live` | true |
| Handler | internal/reports/ — `reportRtbGeoDevice` |

### 4.3 Layout and placement (grid contract)


**Non-sortable headers:** country, device_os, bids, wins, win_rate, spend_micro
...
```
/api/v1/reports/rtb/geo-device?customer_id={uuid}&from={iso}&to={iso}
```
...
| Path | Role |
| :--- | :--- |
| web/src/ui/reports/report_rtb_geo_device.tsx | ReportDataGrid section |
| web/src/ui/reports/report_rtb_geo_device.module.css | Grid/heatmap CSS module |
| web/src/helpers/reports_api.ts | fetchReport('rtb-geo-device', params) |
| web/src/pages/report_route_pages.tsx | Compose until ui/reports migration |

**Legacy page:** `report_route_pages.tsx RtbGeoDeviceReportPage`
...
| :--- | :--- | :--- | :--- |
| 1 | api/openapi/paths/ops_reports.yaml | Confirm `reportRtbGeoDevice` params + response schema | openapi_gate.sh |
| 2 | make openapi-types | Regenerate openapi.d.ts | typecheck |
| 3 | web/src/helpers/reports_api.ts | fetchReport('rtb-geo-device', params) | Compiles |
| 4 | web/src/app_routes.tsx | Register `/reports/rtb/geo-device` (legacy compose until ui/reports lands) | Route resolves |
| 5 | web/src/ui/reports/report_rtb_geo_device.tsx | ReportDataGrid per 4.3 | surface gate |
...
```bash
curl -s '/api/v1/reports/rtb/geo-device?customer_id={uuid}&from={iso}&to={iso}' | jq '.freshness'
```

| Check | Command / procedure | Pass criteria |
| :--- | :--- | :--- |
| No ghost labels | rg 'ghost_' web/src/ui/reports/report_rtb_geo_device | no matches |
...
- [ ] Legacy `report_route_pages.tsx RtbGeoDeviceReportPage` replaced or delegates to `ui/reports/`
