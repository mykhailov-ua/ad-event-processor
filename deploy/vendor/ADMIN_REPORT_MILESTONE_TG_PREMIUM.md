# ADMIN_REPORT_MILESTONE_TG_PREMIUM

Report: telegram-premium.

**Status:** DRAFT  
**Slug:** `admin_report_telegram_premium`  
...
- `/reports/telegram/premium` — Telegram premium user stats
- API: `GET /api/v1/reports/telegram/premium`
- Column contract: Handler row keys (ReportMapEnvelope): campaign_id, premium_users, premium_clicks, premium_conversions
...
| :--- | :--- | :--- |
| GET | GET /api/v1/reports/telegram/premium | reportTelegramPremium |
| Columns | Handler row keys (ReportMapEnvelope): campaign_id, premium_users, premium_clicks, premium_conversions | handler |
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Report title from catalog or static (telegram-premium) | GET /api/v1/reports/catalog or reportTitle() |
| freshness | FreshnessBadge | freshness.stale, freshness.ch_lag_seconds, freshness_label when present | envelope.freshness |
| filters | ReportFilterPanel | from, to, customer_id, campaign_id | URL searchParams → query string |
| toolbar | ReportToolbar | Export → POST /api/v1/reports/jobs; optional saved views | jobs API + /api/v1/views when permitted |
| content | report_telegram_premium | role=grid; column keys from OpenAPI/handler rows[] | rows[] or typed response |
...
| `live` | false |
| Handler | internal/reports/ — `reportTelegramPremium` |

### 4.3 Layout and placement (grid contract)


**Non-sortable headers:** campaign_id, premium_users, premium_clicks, premium_conversions
...
| customer_id | customer_id |  | CustomerIdQuery optional |
...
```
/api/v1/reports/telegram/premium?from={iso}&to={iso}&customer_id={uuid}
```
...
| Path | Role |
| :--- | :--- |
| web/src/ui/reports/report_telegram_premium.tsx | ReportDataGrid section |
| web/src/ui/reports/report_telegram_premium.module.css | Grid/heatmap CSS module |
| web/src/helpers/reports_api.ts | fetchReport('telegram-premium', params) |
| web/src/pages/report_telegram_premium_page.tsx | Compose until ui/reports migration |

**Legacy page:** `report_telegram_premium_page.tsx`
...
| :--- | :--- | :--- | :--- |
| 1 | api/openapi/paths/ops_reports.yaml | Confirm `reportTelegramPremium` params + response schema | openapi_gate.sh |
| 2 | make openapi-types | Regenerate openapi.d.ts | typecheck |
| 3 | web/src/helpers/reports_api.ts | fetchReport('telegram-premium', params) | Compiles |
| 4 | web/src/app_routes.tsx | Register `/reports/telegram/premium` (legacy compose until ui/reports lands) | Route resolves |
| 5 | web/src/ui/reports/report_telegram_premium.tsx | ReportDataGrid per 4.3 | surface gate |
...
```bash
curl -s '/api/v1/reports/telegram/premium?from={iso}&to={iso}&customer_id={uuid}' | jq '.freshness'
```

| Check | Command / procedure | Pass criteria |
| :--- | :--- | :--- |
| No ghost labels | rg 'ghost_' web/src/ui/reports/report_telegram_premium | no matches |
...
- [ ] Legacy `report_telegram_premium_page.tsx` replaced or delegates to `ui/reports/`
