# ADMIN_OPS_MILESTONE_SHARDS

Ops: /ops/shards.

**Status:** DRAFT  
**Slug:** `admin_ops_shards`  
...
- `/ops/shards` — Redis slot map; shard health; catch-up with confirm when config_version_synced=false
- API: GET /api/v1/ops/shards; POST /api/v1/ops/shards/0/catchup; GET .../dashboard/metrics?name=ad_shard0_catchup_last_success_timestamp
- Confirm modal: POST /api/v1/ops/shards/0/catchup
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Redis shards + breadcrumbs | static |
| banner | CatchupBanner | Shard 0 lag warning | shards[].config_version_synced |
| toolbar | CatchupToolbar | Run catch-up button | shards:write + catchupTarget |
| grid | ShardHealthGrid | Per-shard slot map rows | GET .../ops/shards |
| metric | CatchupLastSuccess | Last catchup timestamp | opsDashboardMetrics |
| error | ErrorBlock | GET failure (non-503) | errors |
| degraded | DegradedGrid | 503 body IncidentSnapshot | 503 payload shards[] |
...
| :--- | :--- | :--- |
| Primary fetch | GET /api/v1/ops/shards; POST /api/v1/ops/shards/0/catchup; GET .../dashboard/metrics?name=ad_shard0_catchup_last_success_timestamp | Server owns business rules |
...
| Path | Role |
| :--- | :--- |
| web/src/pages/ops_shards_page.tsx | Compose only |
...
**Legacy page:** `ops_shards_page.tsx`
...
| 5 | web/src/pages/ops_shards_page.tsx | Thin compose; permission gates | No *.module.css on page |
...
- [ ] Legacy `ops_shards_page.tsx` replaced or delegates to `ui/ops/`
