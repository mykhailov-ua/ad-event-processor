# ADMIN_SETTINGS_MILESTONE_LICENSE

Settings: /settings/license.

**Status:** DRAFT  
**Slug:** `admin_settings_license`  
...
- `/settings/license` — JWT paste apply flow; status diagnostics; no echo token after save
- API: GET /api/v1/license/status; POST /api/v1/license/apply
- Confirm modal: POST /api/v1/license/apply
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | License + breadcrumbs | static |
| status | LicenseStatusPanel | plan, valid_until, hwid_match, max_rps | licenseStatus |
| alerts | AlertBanner | hwid mismatch warning | hwid_match=false |
| form | LicenseApplyForm | JWT paste textarea | licenseApply {token} |
| upgrade | LicenseUpgradeLink | support_url when entitled | license status DTO |
| error | ErrorBlock | GET/apply failure | errors |
...
| :--- | :--- | :--- |
| Primary fetch | GET /api/v1/license/status; POST /api/v1/license/apply | Server owns business rules |
...
| Path | Role |
| :--- | :--- |
| web/src/pages/settings_license_page.tsx | Compose |
...
**Legacy page:** `settings_license_page.tsx`
...
| 5 | web/src/pages/settings_license_page.tsx | Compose; no CSS modules on page | ≤120 lines |
...
- [ ] Legacy `settings_license_page.tsx` replaced or delegates to `ui/settings/`
