# ADMIN_INTEGRATIONS_MILESTONE_SCHEMAS

Integration: /integrations/schemas.

**Status:** DRAFT  
**Slug:** `admin_integrations_schemas`  
...
- `/integrations/schemas` — Custom schema list; JSON detail; apply-to-campaign modal
- APIs: GET/POST /api/v1/integration/schemas; GET .../schemas/{id}; POST .../schemas/{id}/apply
...
| :--- | :--- | :--- |
| Schema PATCH/DELETE | OpenAPI lists GET/POST + apply only | No update/delete UI until ops added |
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Integration schemas | static |
| toolbar | SchemaToolbar | Create schema; import templates link | POST schemas |
| list | SchemaGrid | id, name, kind, updated_at | GET schemas |
| detail | SchemaDetailPanel | expand row JSON document | GET schemas/{id} |
| apply | ApplySchemaModal | campaign_id + apply | POST schemas/{id}/apply |
| templates_link | TemplatesLink | -> /integration/templates/import | static |
...
| Topic | Spec |
| :--- | :--- |
| CSS ownership | `web/src/ui/schemas/*.module.css` only |
...
| :--- | :--- | :--- |
| Primary fetch | GET/POST /api/v1/integration/schemas; GET .../schemas/{id}; POST .../schemas/{id}/apply | Server owns business rules |
...
| Path | Role |
| :--- | :--- |
| web/src/pages/integrations_schemas_page.tsx | Compose |
| web/src/ui/schemas/* | Sections + CSS modules |
| web/src/helpers/schemas_api.ts | Typed API helpers |
| web/src/types/generated/openapi.d.ts | Generated types |

**Legacy page:** `integrations_schemas_page.tsx`
...
| :--- | :--- | :--- | :--- |
| 1 | GET/POST /api/v1/integration/schemas | Confirm OpenAPI ops + x-permissions | openapi_gate.sh |
...
| 4 | web/src/helpers/schemas_api.ts | Typed API helpers; secrets never logged | Compiles |
| 5 | web/src/ui/schemas/* | Sections per regions + grid_ascii | check_ui_surface_gate.sh |
| 6 | web/src/pages/integrations_schemas_page.tsx | Compose + URL params | No client filter on items[] |
...
| :--- | :--- | :--- |
| No client sort/filter | rg 'useMemo.*filter\|table_sort' web/src/pages/integrations_schemas_page.tsx web/src/ui/schemas/ | no matches |
...
- [ ] Legacy `integrations_schemas_page.tsx` replaced or delegates to `ui/schemas/`
- [ ] Verification output pasted in PR

## 9. Rollback

Revert `web/src/ui/schemas/`, helpers, and page compose in one slice; no half migration.
