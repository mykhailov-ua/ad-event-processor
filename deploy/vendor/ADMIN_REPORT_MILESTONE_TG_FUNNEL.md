# ADMIN_REPORT_MILESTONE_TG_FUNNEL

Report: telegram-funnel.

**Status:** DRAFT  
**Slug:** `admin_report_telegram_funnel`  
...
- `/reports/telegram/funnel` — Telegram conversion funnel
- API: `GET /api/v1/reports/telegram/funnel`
- Column contract: Handler row keys (ReportMapEnvelope): step, count, conversion_rate
...
| :--- | :--- | :--- |
| GET | GET /api/v1/reports/telegram/funnel | reportTelegramFunnel |
| Columns | Handler row keys (ReportMapEnvelope): step, count, conversion_rate | handler |
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Report title from catalog or static (telegram-funnel) | GET /api/v1/reports/catalog or reportTitle() |
...
| content | report_telegram_funnel | role=grid; column keys from OpenAPI/handler rows[] | rows[] or typed response |
...
| `live` | false |
| Handler | internal/reports/ — `reportTelegramFunnel` |

### 4.3 Layout and placement (grid contract)


**Non-sortable headers:** step, count, conversion_rate
...
```
/api/v1/reports/telegram/funnel?from={iso}&to={iso}&customer_id={uuid}
```
...
| Path | Role |
| :--- | :--- |
| web/src/ui/reports/report_telegram_funnel.tsx | ReportDataGrid section |
| web/src/ui/reports/report_telegram_funnel.module.css | Grid/heatmap CSS module |
| web/src/helpers/reports_api.ts | fetchReport('telegram-funnel', params) |
| web/src/pages/report_telegram_funnel_page.tsx | Compose until ui/reports migration |

**Legacy page:** `report_telegram_funnel_page.tsx`
...
| :--- | :--- | :--- | :--- |
| 1 | api/openapi/paths/ops_reports.yaml | Confirm `reportTelegramFunnel` params + response schema | openapi_gate.sh |
| 2 | make openapi-types | Regenerate openapi.d.ts | typecheck |
| 3 | web/src/helpers/reports_api.ts | fetchReport('telegram-funnel', params) | Compiles |
| 4 | web/src/app_routes.tsx | Register `/reports/telegram/funnel` (legacy compose until ui/reports lands) | Route resolves |
| 5 | web/src/ui/reports/report_telegram_funnel.tsx | ReportDataGrid per 4.3 | surface gate |
...
```bash
curl -s '/api/v1/reports/telegram/funnel?from={iso}&to={iso}&customer_id={uuid}' | jq '.freshness'
```

| Check | Command / procedure | Pass criteria |
| :--- | :--- | :--- |
| No ghost labels | rg 'ghost_' web/src/ui/reports/report_telegram_funnel | no matches |
...
- [ ] Legacy `report_telegram_funnel_page.tsx` replaced or delegates to `ui/reports/`
