# ADMIN_REPORT_MILESTONE_GEO_DEVICE

Report: campaign-geo-device.

**Status:** DRAFT  
**Slug:** `admin_report_campaign_geo_device`  
...
- `/reports/campaign-geo-device` — Geo × device breakdown
- API: `GET /api/v1/reports/campaign-geo-device`
- Column contract: Handler row keys (ReportMapEnvelope): country, device, clicks
...
| :--- | :--- | :--- |
| GET | GET /api/v1/reports/campaign-geo-device | reportCampaignGeoDevice |
| Columns | Handler row keys (ReportMapEnvelope): country, device, clicks | handler |
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Report title from catalog or static (campaign-geo-device) | GET /api/v1/reports/catalog or reportTitle() |
...
| content | report_campaign_geo_device | role=grid; column keys from OpenAPI/handler rows[] | rows[] or typed response |
...
| `live` | true |
| Handler | internal/reports/ — `reportCampaignGeoDevice` |

### 4.3 Layout and placement (grid contract)


**Non-sortable headers:** country, device, clicks
...
```
/api/v1/reports/campaign-geo-device?customer_id={uuid}&from={iso}&to={iso}
```
...
| Path | Role |
| :--- | :--- |
| web/src/ui/reports/report_campaign_geo_device.tsx | ReportDataGrid section |
| web/src/ui/reports/report_campaign_geo_device.module.css | Grid/heatmap CSS module |
| web/src/helpers/reports_api.ts | fetchReport('campaign-geo-device', params) |
| web/src/pages/report_route_pages.tsx | Compose until ui/reports migration |

**Legacy page:** `report_configs.ts campaign-geo-device`
...
| :--- | :--- | :--- | :--- |
| 1 | api/openapi/paths/ops_reports.yaml | Confirm `reportCampaignGeoDevice` params + response schema | openapi_gate.sh |
| 2 | make openapi-types | Regenerate openapi.d.ts | typecheck |
| 3 | web/src/helpers/reports_api.ts | fetchReport('campaign-geo-device', params) | Compiles |
| 4 | web/src/app_routes.tsx | Register `/reports/campaign-geo-device` (legacy compose until ui/reports lands) | Route resolves |
| 5 | web/src/ui/reports/report_campaign_geo_device.tsx | ReportDataGrid per 4.3 | surface gate |
...
```bash
curl -s '/api/v1/reports/campaign-geo-device?customer_id={uuid}&from={iso}&to={iso}' | jq '.freshness'
```

| Check | Command / procedure | Pass criteria |
| :--- | :--- | :--- |
| No ghost labels | rg 'ghost_' web/src/ui/reports/report_campaign_geo_device | no matches |
...
- [ ] Legacy `report_configs.ts campaign-geo-device` replaced or delegates to `ui/reports/`
