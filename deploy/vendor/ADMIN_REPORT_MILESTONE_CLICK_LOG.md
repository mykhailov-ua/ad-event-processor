# ADMIN_REPORT_MILESTONE_CLICK_LOG

Report: click-log.

**Status:** DRAFT  
**Slug:** `admin_report_click_log`  
...
- `/reports/clicks` — Click log timeline (browse or click_id drilldown)
- API: `GET /api/v1/reports/click-log`
- Column contract: `ops_reports.yaml#/ClickLogEvent` row keys: event_type, click_id, campaign_id, placement_id, created_at, attributed_cost_micro, cost_source, revenue_micro, inbound_status, goal_name, sub1, country
...
- Buyer session: default customer_id from boundCustomerId()
...
| :--- | :--- | :--- |
| GET | GET /api/v1/reports/click-log | reportClickLog |
| Columns | `ops_reports.yaml#/ClickLogEvent` row keys: event_type, click_id, campaign_id, placement_id, created_at, attributed_cost_micro, cost_source, revenue_micro, inbound_status, goal_name, sub1, country | openapi |
| Envelope | rows + freshness (+ next_cursor when paginated) | ReportMapEnvelope or typed response |
| Schema | ops_reports.yaml#/ClickLogEvent | openapi.d.ts row type |
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Report title from catalog or static (click-log) | GET /api/v1/reports/catalog or reportTitle() |
| freshness | FreshnessBadge | freshness.stale, freshness.ch_lag_seconds, freshness_label when present | envelope.freshness |
| filters | ReportFilterPanel | customer_id, from, to, campaign_id, click_id, cursor | URL searchParams → query string |
| toolbar | ReportToolbar | Export → POST /api/v1/reports/jobs; optional saved views | jobs API + /api/v1/views when permitted |
| content | report_click_log | events[] timeline + optional postbacks[] | rows[] or typed response |
...
| Path | `/reports/clicks` |
| `live` | true |
| Handler | internal/reports/ — `reportClickLog` |

### 4.3 Layout and placement (grid contract)


**Non-sortable headers:** event_type, click_id, campaign_id, placement_id, created_at, attributed_cost_micro, cost_source, revenue_micro, inbound_status, goal_name, sub1, country
...
| from | from | now-7d | ReportFromQuery |
| to | to | now | ReportToQuery |
| campaign_id | campaign_id |  | browse mode when click_id omitted |
| click_id | click_id |  | timeline mode; required when campaign_id omitted |
...
```
/api/v1/reports/click-log?customer_id={uuid}&from={iso}&to={iso}&click_id={click_id}
```
...
| Path | Role |
| :--- | :--- |
| web/src/ui/reports/report_click_log.tsx | ClickLogTimeline section |
| web/src/ui/reports/report_click_log.module.css | Grid/heatmap CSS module |
| web/src/helpers/reports_api.ts | fetchReport('click-log', params) |
| web/src/pages/report_route_pages.tsx | Compose until ui/reports migration |

**Legacy page:** `report_stub_page.tsx (click-log; alias route /reports/clicks)`
...
| :--- | :--- | :--- | :--- |
| 1 | api/openapi/paths/ops_reports.yaml | Confirm `reportClickLog` params + response schema | openapi_gate.sh |
| 2 | make openapi-types | Regenerate openapi.d.ts | typecheck |
| 3 | web/src/helpers/reports_api.ts | fetchReport('click-log', params) | Compiles |
| 4 | web/src/app_routes.tsx | Register `/reports/clicks` (legacy compose until ui/reports lands) | Route resolves |
| 5 | web/src/ui/reports/report_click_log.tsx | ClickLogTimeline per 4.3 | surface gate |
...
```bash
curl -s '/api/v1/reports/click-log?customer_id={uuid}&from={iso}&to={iso}&click_id={click_id}' | jq '.freshness'
```

| Check | Command / procedure | Pass criteria |
| :--- | :--- | :--- |
| No ghost labels | rg 'ghost_' web/src/ui/reports/report_click_log | no matches |
...
| Export POST | POST /api/v1/reports/jobs body report_key=click-log | 202 + job id |
...
- [ ] Legacy `report_stub_page.tsx (click-log; alias route /reports/clicks)` replaced or delegates to `ui/reports/`
