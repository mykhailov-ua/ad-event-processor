# ADMIN_REPORT_MILESTONE_SOURCE_QUALITY

Report: source-quality.

**Status:** DRAFT  
**Slug:** `admin_report_source_quality`  
...
- `/reports/source-quality` — Sub-source quality scores
- API: `GET /api/v1/reports/source-quality`
- Column contract: Handler row keys (ReportMapEnvelope): placement_id, campaign_id, clicks, conversions, ivt_rate, roi_pct
...
| :--- | :--- | :--- |
| GET | GET /api/v1/reports/source-quality | reportSourceQuality |
| Columns | Handler row keys (ReportMapEnvelope): placement_id, campaign_id, clicks, conversions, ivt_rate, roi_pct | handler |
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Report title from catalog or static (source-quality) | GET /api/v1/reports/catalog or reportTitle() |
...
| content | report_source_quality | role=grid; column keys from OpenAPI/handler rows[] | rows[] or typed response |
...
| `live` | true |
| Handler | internal/reports/ — `reportSourceQuality` |

### 4.3 Layout and placement (grid contract)


**Non-sortable headers:** placement_id, campaign_id, clicks, conversions, ivt_rate, roi_pct
...
```
/api/v1/reports/source-quality?customer_id={uuid}&from={iso}&to={iso}
```
...
| Path | Role |
| :--- | :--- |
| web/src/ui/reports/report_source_quality.tsx | ReportDataGrid section |
| web/src/ui/reports/report_source_quality.module.css | Grid/heatmap CSS module |
| web/src/helpers/reports_api.ts | fetchReport('source-quality', params) |
| web/src/pages/report_route_pages.tsx | Compose until ui/reports migration |

**Legacy page:** `report_configs.ts source-quality`
...
| :--- | :--- | :--- | :--- |
| 1 | api/openapi/paths/ops_reports.yaml | Confirm `reportSourceQuality` params + response schema | openapi_gate.sh |
| 2 | make openapi-types | Regenerate openapi.d.ts | typecheck |
| 3 | web/src/helpers/reports_api.ts | fetchReport('source-quality', params) | Compiles |
| 4 | web/src/app_routes.tsx | Register `/reports/source-quality` (legacy compose until ui/reports lands) | Route resolves |
| 5 | web/src/ui/reports/report_source_quality.tsx | ReportDataGrid per 4.3 | surface gate |
...
```bash
curl -s '/api/v1/reports/source-quality?customer_id={uuid}&from={iso}&to={iso}' | jq '.freshness'
```

| Check | Command / procedure | Pass criteria |
| :--- | :--- | :--- |
| No ghost labels | rg 'ghost_' web/src/ui/reports/report_source_quality | no matches |
...
- [ ] Legacy `report_configs.ts source-quality` replaced or delegates to `ui/reports/`
