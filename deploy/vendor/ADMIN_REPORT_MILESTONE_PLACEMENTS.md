# ADMIN_REPORT_MILESTONE_PLACEMENTS

Report: placements.

**Status:** DRAFT  
**Slug:** `admin_report_placements`  
...
- `/reports/placements` — Zone/placement grid, spend, ROI, IVT
- API: `GET /api/v1/reports/placements`
- Column contract: `ops_reports.yaml#/PlacementReportRow` row keys: placement_id, campaign_id, impressions, clicks, conversions, spend_micro, revenue_micro, profit_micro, roi_pct, cpa_micro, ctr, ivt_rate
...
| :--- | :--- | :--- |
| GET | GET /api/v1/reports/placements | reportPlacements |
| Columns | `ops_reports.yaml#/PlacementReportRow` row keys: placement_id, campaign_id, impressions, clicks, conversions, spend_micro, revenue_micro, profit_micro, roi_pct, cpa_micro, ctr, ivt_rate | openapi |
| Envelope | rows + freshness (+ next_cursor when paginated) | ReportMapEnvelope or typed response |
| Schema | ops_reports.yaml#/PlacementReportRow | openapi.d.ts row type |
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Report title from catalog or static (placements) | GET /api/v1/reports/catalog or reportTitle() |
...
| content | report_placements | role=grid; column keys from OpenAPI/handler rows[] | rows[] or typed response |
...
| `live` | true |
| Handler | internal/reports/ — `reportPlacements` |

### 4.3 Layout and placement (grid contract)

Column template: `--reports-cols`: `minmax(10rem,1.2fr) minmax(8rem,1fr) repeat(10, minmax(6rem, auto))`


**Non-sortable headers:** placement_id, campaign_id, impressions, clicks, conversions, spend_micro, revenue_micro, profit_micro, roi_pct, cpa_micro, ctr, ivt_rate
...
| Grid token | `--reports-cols`: minmax(10rem,1.2fr) minmax(8rem,1fr) repeat(10, minmax(6rem, auto)) |
...
```
/api/v1/reports/placements?customer_id={uuid}&from={iso}&to={iso}
```
...
| Path | Role |
| :--- | :--- |
| web/src/ui/reports/report_placements.tsx | ReportDataGrid section |
| web/src/ui/reports/report_placements.module.css | Grid/heatmap CSS module |
| web/src/helpers/reports_api.ts | fetchReport('placements', params) |
| web/src/pages/report_route_pages.tsx | Compose until ui/reports migration |

**Legacy page:** `report_query_page.tsx (placements)`
...
| :--- | :--- | :--- | :--- |
| 1 | api/openapi/paths/ops_reports.yaml | Confirm `reportPlacements` params + response schema | openapi_gate.sh |
| 2 | make openapi-types | Regenerate openapi.d.ts | typecheck |
| 3 | web/src/helpers/reports_api.ts | fetchReport('placements', params) | Compiles |
| 4 | web/src/app_routes.tsx | Register `/reports/placements` (legacy compose until ui/reports lands) | Route resolves |
| 5 | web/src/ui/reports/report_placements.tsx | ReportDataGrid per 4.3 | surface gate |
...
```bash
curl -s '/api/v1/reports/placements?customer_id={uuid}&from={iso}&to={iso}' | jq '.freshness'
```

| Check | Command / procedure | Pass criteria |
| :--- | :--- | :--- |
| No ghost labels | rg 'ghost_' web/src/ui/reports/report_placements | no matches |
...
| Export POST | POST /api/v1/reports/jobs body report_key=placements | 202 + job id |
...
- [ ] Legacy `report_query_page.tsx (placements)` replaced or delegates to `ui/reports/`
