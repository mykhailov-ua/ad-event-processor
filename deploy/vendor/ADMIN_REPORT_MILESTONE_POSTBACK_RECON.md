# ADMIN_REPORT_MILESTONE_POSTBACK_RECON

Report: postback-reconciliation.

**Status:** DRAFT  
**Slug:** `admin_report_postback_reconciliation`  
...
- `/reports/postback-reconciliation` — Postback vs ledger reconciliation
- API: `GET /api/v1/reports/postback-reconciliation`
- Column contract: Handler row keys (ReportMapEnvelope): campaign_id, click_id, conversion_at, conversion_value_micro, ledger_day_fee_micro, postback_status, reconcile_status
...
| :--- | :--- | :--- |
| GET | GET /api/v1/reports/postback-reconciliation | reportPostbackReconciliation |
| Columns | Handler row keys (ReportMapEnvelope): campaign_id, click_id, conversion_at, conversion_value_micro, ledger_day_fee_micro, postback_status, reconcile_status | handler |
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Report title from catalog or static (postback-reconciliation) | GET /api/v1/reports/catalog or reportTitle() |
...
| content | report_postback_reconciliation | role=grid; column keys from OpenAPI/handler rows[] | rows[] or typed response |
...
| `live` | true |
| Handler | internal/reports/ — `reportPostbackReconciliation` |

### 4.3 Layout and placement (grid contract)


**Non-sortable headers:** campaign_id, click_id, conversion_at, conversion_value_micro, ledger_day_fee_micro, postback_status, reconcile_status
...
```
/api/v1/reports/postback-reconciliation?customer_id={uuid}&from={iso}&to={iso}
```
...
| Path | Role |
| :--- | :--- |
| web/src/ui/reports/report_postback_reconciliation.tsx | ReportDataGrid section |
| web/src/ui/reports/report_postback_reconciliation.module.css | Grid/heatmap CSS module |
| web/src/helpers/reports_api.ts | fetchReport('postback-reconciliation', params) |
| web/src/pages/report_route_pages.tsx | Compose until ui/reports migration |

**Legacy page:** `report_route_pages.tsx PostbackReconReportPage`
...
| :--- | :--- | :--- | :--- |
| 1 | api/openapi/paths/ops_reports.yaml | Confirm `reportPostbackReconciliation` params + response schema | openapi_gate.sh |
| 2 | make openapi-types | Regenerate openapi.d.ts | typecheck |
| 3 | web/src/helpers/reports_api.ts | fetchReport('postback-reconciliation', params) | Compiles |
| 4 | web/src/app_routes.tsx | Register `/reports/postback-reconciliation` (legacy compose until ui/reports lands) | Route resolves |
| 5 | web/src/ui/reports/report_postback_reconciliation.tsx | ReportDataGrid per 4.3 | surface gate |
...
```bash
curl -s '/api/v1/reports/postback-reconciliation?customer_id={uuid}&from={iso}&to={iso}' | jq '.freshness'
```

| Check | Command / procedure | Pass criteria |
| :--- | :--- | :--- |
| No ghost labels | rg 'ghost_' web/src/ui/reports/report_postback_reconciliation | no matches |
...
| Export POST | POST /api/v1/reports/jobs body report_key=postback-reconciliation | 202 + job id |
...
- [ ] Legacy `report_route_pages.tsx PostbackReconReportPage` replaced or delegates to `ui/reports/`
