...
- `/selfserve` — Buyer portfolio landing; campaign grid; bulk pause/resume with confirm
- API: GET /api/v1/selfserve/campaigns; GET /api/v1/dashboards/buyer (portfolio); pause/resume campaign
- Confirm modal: bulk pause, bulk resume, single campaign pause/resume
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Portfolio | static |
| filters | PortfolioFilter | status filter → server refetch | query params |
| toolbar | BulkActionsToolbar | pause/resume selected | pause/resume APIs |
| grid | PortfolioGrid | campaign rows | selfserve campaigns |
| empty | EmptyState | No campaigns | buyer empty copy |
| error | ErrorBlock | Portfolio fetch failure | errors |
...
| :--- | :--- | :--- |
| Primary fetch | GET /api/v1/selfserve/campaigns; GET /api/v1/dashboards/buyer (portfolio); pause/resume campaign | Server owns business rules |
...
| Path | Role |
| :--- | :--- |
| web/src/pages/selfserve_portfolio_page.tsx | Compose |
...
**Legacy page:** `selfserve_portfolio_page.tsx`
...
- [ ] Legacy `selfserve_portfolio_page.tsx` replaced or delegates to `ui/selfserve/`
