# ADMIN_REPORT_MILESTONE_CAMPAIGN_TOGGLE_COHORT

Report: campaign-toggle-cohort.

**Route gap:** register `/reports/campaign-toggle-cohort` in `app_routes.tsx` before `live: true`.

**Status:** DRAFT  
**Slug:** `admin_report_campaign_toggle_cohort`  
...
- `/reports/campaign-toggle-cohort` — Before/after metrics for campaign fraud toggle changes
- API: `GET /api/v1/reports/campaign-toggle-cohort`
- Column contract: Handler row keys (ReportMapEnvelope): window, impressions, rejects, conversions, roi_pct, delta_label, delta_tone
...
| :--- | :--- | :--- |
| GET | GET /api/v1/reports/campaign-toggle-cohort | reportCampaignToggleCohort |
| Columns | Handler row keys (ReportMapEnvelope): window, impressions, rejects, conversions, roi_pct, delta_label, delta_tone | handler |
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Report title from catalog or static (campaign-toggle-cohort) | GET /api/v1/reports/catalog or reportTitle() |
| freshness | FreshnessBadge | freshness.stale, freshness.ch_lag_seconds, freshness_label when present | envelope.freshness |
| filters | ReportFilterPanel | customer_id, from, to, campaign_id, toggle_field, toggle_at, window_hours | URL searchParams → query string |
| toolbar | ReportToolbar | Export → POST /api/v1/reports/jobs; optional saved views | jobs API + /api/v1/views when permitted |
| content | report_campaign_toggle_cohort | before/after window KPI rows | rows[] or typed response |
...
| `live` | false |
| Handler | internal/reports/ — `reportCampaignToggleCohort` |

### 4.3 Layout and placement (grid contract)


**Non-sortable headers:** window, impressions, rejects, conversions, roi_pct, delta_label, delta_tone
...
| campaign_id | campaign_id |  | required uuid |
| toggle_field | toggle_field |  | required enum |
| toggle_at | toggle_at |  | optional RFC3339 |
| window_hours | window_hours | 24 | 1-168 |
...
```
/api/v1/reports/campaign-toggle-cohort?customer_id={uuid}&from={iso}&to={iso}&campaign_id={uuid}&toggle_field=silent_reject_enabled
```
...
| Path | Role |
| :--- | :--- |
| web/src/ui/reports/report_campaign_toggle_cohort.tsx | ToggleCohortGrid section |
| web/src/ui/reports/report_campaign_toggle_cohort.module.css | Grid/heatmap CSS module |
| web/src/helpers/reports_api.ts | fetchReport('campaign-toggle-cohort', params) |
| web/src/pages/report_route_pages.tsx | Compose until ui/reports migration |

**Legacy page:** `report_stub_page.tsx (campaign-toggle-cohort)`
...
| :--- | :--- | :--- | :--- |
| 1 | api/openapi/paths/ops_reports.yaml | Confirm `reportCampaignToggleCohort` params + response schema | openapi_gate.sh |
| 2 | make openapi-types | Regenerate openapi.d.ts | typecheck |
| 3 | web/src/helpers/reports_api.ts | fetchReport('campaign-toggle-cohort', params) | Compiles |
| 4 | web/src/app_routes.tsx | Register gap route `/reports/campaign-toggle-cohort` + nav/catalog entry | Route resolves |
| 5 | web/src/ui/reports/report_campaign_toggle_cohort.tsx | ToggleCohortGrid section | check_ui_surface_gate.sh |
...
```bash
Register `/reports/campaign-toggle-cohort` in app_routes before catalog live:true
curl -s '/api/v1/reports/campaign-toggle-cohort?customer_id={uuid}&from={iso}&to={iso}&campaign_id={uuid}&toggle_field=silent_reject_enabled' | jq '.freshness'
```

| Check | Command / procedure | Pass criteria |
| :--- | :--- | :--- |
| No ghost labels | rg 'ghost_' web/src/ui/reports/report_campaign_toggle_cohort | no matches |
...
- [ ] Legacy `report_stub_page.tsx (campaign-toggle-cohort)` replaced or delegates to `ui/reports/`
