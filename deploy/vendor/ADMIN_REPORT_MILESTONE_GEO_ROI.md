# ADMIN_REPORT_MILESTONE_GEO_ROI

Report: geo-roi.

**Status:** DRAFT  
**Slug:** `admin_report_geo_roi`  
...
- `/reports/geo-roi` — ROI by country
- API: `GET /api/v1/reports/geo-roi`
- Column contract: Handler row keys (ReportMapEnvelope): country, clicks, ivt_rate, spend_micro, roi_pct
...
| :--- | :--- | :--- |
| GET | GET /api/v1/reports/geo-roi | reportGeoRoi |
| Columns | Handler row keys (ReportMapEnvelope): country, clicks, ivt_rate, spend_micro, roi_pct | handler |
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Report title from catalog or static (geo-roi) | GET /api/v1/reports/catalog or reportTitle() |
...
| content | report_geo_roi | role=grid; column keys from OpenAPI/handler rows[] | rows[] or typed response |
...
| Field | Spec |
| :--- | :--- |
| Path | `/reports/geo-roi` |
| `live` | true |
| Handler | internal/reports/ — `reportGeoRoi` |

### 4.3 Layout and placement (grid contract)


**Non-sortable headers:** country, clicks, ivt_rate, spend_micro, roi_pct
...
```
/api/v1/reports/geo-roi?customer_id={uuid}&from={iso}&to={iso}
```
...
| Path | Role |
| :--- | :--- |
| web/src/ui/reports/report_geo_roi.tsx | ReportDataGrid section |
| web/src/ui/reports/report_geo_roi.module.css | Grid/heatmap CSS module |
| web/src/helpers/reports_api.ts | fetchReport('geo-roi', params) |
| web/src/pages/report_route_pages.tsx | Compose until ui/reports migration |

**Legacy page:** `report_route_pages.tsx GeoRoiReportPage`
...
| :--- | :--- | :--- | :--- |
| 1 | api/openapi/paths/ops_reports.yaml | Confirm `reportGeoRoi` params + response schema | openapi_gate.sh |
| 2 | make openapi-types | Regenerate openapi.d.ts | typecheck |
| 3 | web/src/helpers/reports_api.ts | fetchReport('geo-roi', params) | Compiles |
| 4 | web/src/app_routes.tsx | Register `/reports/geo-roi` (legacy compose until ui/reports lands) | Route resolves |
| 5 | web/src/ui/reports/report_geo_roi.tsx | ReportDataGrid per 4.3 | surface gate |
...
```bash
curl -s '/api/v1/reports/geo-roi?customer_id={uuid}&from={iso}&to={iso}' | jq '.freshness'
```

| Check | Command / procedure | Pass criteria |
| :--- | :--- | :--- |
| No ghost labels | rg 'ghost_' web/src/ui/reports/report_geo_roi | no matches |
...
- [ ] Legacy `report_route_pages.tsx GeoRoiReportPage` replaced or delegates to `ui/reports/`
