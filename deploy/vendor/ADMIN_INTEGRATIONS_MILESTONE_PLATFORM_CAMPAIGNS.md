# ADMIN_INTEGRATIONS_MILESTONE_PLATFORM_CAMPAIGNS

Integration: /integrations/platform-campaigns.

**Route gap:** register `/integrations/platform-campaigns` in `app_routes.tsx` before `live: true`.

**Status:** DRAFT  
**Slug:** `admin_integrations_platform_campaigns`  
...
- `/integrations/platform-campaigns` — External ad platform links; pause/resume/budget; manual sync
- APIs: GET /api/v1/platform-campaigns/links; PUT/DELETE .../links/{campaign_id}/{network}; POST .../refresh; POST .../{campaign_id}/pause|resume|budget; POST .../sync-run
- Forms ⊆ OpenAPI; StubBanner on 501
- Confirm modal on destructive/retry/run actions
- RBAC: `campaigns:read; campaigns:write; Enterprise ad platform API license` on write actions
...
| :--- | :--- | :--- |
| Route not registered | gap_route until SPA route + nav ship | OpenAPI ops exist; UI milestone blocked on license + route |
| License gate | Requires Enterprise SKU ad platform campaign API | StubBanner when license JWT lacks entitlement |
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Platform campaign links | static |
| license | LicenseGateBanner | Enterprise SKU required | license status |
| filters | LinkFilterBar | campaign_id, customer_id | URL |
| links | PlatformLinksGrid | network, external_id, sync status | GET links |
| upsert | LinkForm | UpsertPlatformCampaignLinkRequest | PUT links/{campaign_id}/{network} |
| mutations | MutationToolbar | pause, resume, budget, refresh | POST mutation ops |
| sync | SyncRunPanel | manual sync-run | POST sync-run |
...
| Topic | Spec |
| :--- | :--- |
| CSS ownership | `web/src/ui/platform_campaigns/*.module.css` only |
...
| :--- | :--- | :--- | :--- |
| campaign_id | campaign_id |  | filter links |
| customer_id | customer_id |  | filter links |
...
| Path | Role |
| :--- | :--- |
| web/src/pages/platform_campaigns_page.tsx | Compose |
| web/src/ui/platform_campaigns/* | Sections + CSS modules |
| web/src/helpers/platform_campaigns_api.ts | Typed API helpers |
...
| :--- | :--- | :--- | :--- |
| 1 | GET /api/v1/platform-campaigns/links | Confirm OpenAPI ops + x-permissions | openapi_gate.sh |
...
| 4 | web/src/helpers/platform_campaigns_api.ts | Typed API helpers; secrets never logged | Compiles |
| 5 | web/src/ui/platform_campaigns/* | Sections per regions + grid_ascii | check_ui_surface_gate.sh |
| 6 | web/src/pages/platform_campaigns_page.tsx | Compose + URL params | No client filter on items[] |
...
| :--- | :--- | :--- |
| No client sort/filter | rg 'useMemo.*filter\|table_sort' web/src/pages/platform_campaigns_page.tsx web/src/ui/platform_campaigns/ | no matches |
...
Revert `web/src/ui/platform_campaigns/`, helpers, and page compose in one slice; no half migration.
