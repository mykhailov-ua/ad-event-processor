# ADMIN_INTEGRATIONS_MILESTONE_SUPPLY

Integration: /integrations/supply.

**Status:** DRAFT  
**Slug:** `admin_integrations_supply`  
...
- `/integrations/supply` — sellers.json + ads.txt CRUD; live preview; validation report
- APIs: GET/POST/PUT/DELETE /api/v1/supply/sellers*; GET/POST/PUT/DELETE .../ads-txt*; GET .../preview/*; GET .../validation; GET .../export-path
- Forms ⊆ OpenAPI; StubBanner on 501
- Confirm modal on destructive/retry/run actions
- RBAC: `settings:read; settings:write` on write actions
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Supply chain files | static |
| tabs | SupplyTabBar | sellers \| ads \| preview \| validation | URL ?tab= |
| sellers | SellersGrid | CRUD sellers.json rows | supply/sellers* |
| ads | AdsTxtGrid | CRUD ads.txt rows | supply/ads-txt* |
| preview | SupplyPreviewPanel | rendered sellers.json + ads.txt | GET preview/* |
| validation | SupplyValidationPanel | checksums, issues[] | GET .../validation |
| export | ExportPathChip | nginx export path | GET .../export-path |
...
| Topic | Spec |
| :--- | :--- |
| CSS ownership | `web/src/ui/supply/*.module.css` only |
...
| :--- | :--- | :--- | :--- |
| tab | tab | sellers | sellers \| ads \| preview \| validation |
...
| Path | Role |
| :--- | :--- |
| web/src/pages/integrations_supply_page.tsx | Compose |
| web/src/ui/supply/* | Sections + CSS modules |
| web/src/helpers/supply_api.ts | Typed API helpers |
| web/src/types/generated/openapi.d.ts | Generated types |

**Legacy page:** `integrations_supply_page.tsx`
...
| :--- | :--- | :--- | :--- |
| 1 | GET/POST/PUT/DELETE /api/v1/supply/sellers* | Confirm OpenAPI ops + x-permissions | openapi_gate.sh |
...
| 4 | web/src/helpers/supply_api.ts | Typed API helpers; secrets never logged | Compiles |
| 5 | web/src/ui/supply/* | Sections per regions + grid_ascii | check_ui_surface_gate.sh |
| 6 | web/src/pages/integrations_supply_page.tsx | Compose + URL params | No client filter on items[] |
...
| :--- | :--- | :--- |
| No client sort/filter | rg 'useMemo.*filter\|table_sort' web/src/pages/integrations_supply_page.tsx web/src/ui/supply/ | no matches |
...
- [ ] Legacy `integrations_supply_page.tsx` replaced or delegates to `ui/supply/`
- [ ] Verification output pasted in PR

## 9. Rollback

Revert `web/src/ui/supply/`, helpers, and page compose in one slice; no half migration.
