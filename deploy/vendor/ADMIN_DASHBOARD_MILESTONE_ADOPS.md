# ADMIN_DASHBOARD_MILESTONE_ADOPS

Dashboard: Ad ops.

**Status:** DRAFT  
**Slug:** `admin_dashboard_adops`  
...
- Route `/dashboards/adops`
- Delivery/pacing KPIs; campaign rows; worst sources
- API: `GET /api/v1/dashboards/adops` (`dashboardAdops`)
- KPI keys: kpis.spend_micro, kpis.revenue_micro, kpis.profit_micro, kpis.conversions, kpis.roi_pct, kpis.freshness.stale
- FreshnessBadge from freshness DTO only
- RBAC: `campaigns:read` per OpenAPI x-permissions
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Ad ops | static |
| freshness | FreshnessBadge | kpis.freshness | kpis.freshness |
| kpis | KpiGrid | repeat(3, minmax(12rem, 1fr)) | MetricsBlockDTO |
| campaigns | PacingGrid | campaigns[] pacing_drift_pct, overspend_risk | campaigns[] |
| sources | WorstSourcesGrid | worst_sources[] ivt_rate, quality_score | worst_sources[] |
| links | DrillDownLinks | /reports/pacing-drift, /reports/source-quality | static |
...
```
GET /api/v1/dashboards/adops?customer_id={uuid}&from={iso}&to={iso}
```
...
| web/src/ui/dashboard/adops_dashboard.tsx | KPI + sections |
...
| :--- | :--- | :--- | :--- |
| 1 | GET /api/v1/dashboards/adops | Confirm operation_id + handler DTO fields | openapi_gate.sh + handler test |
