# ADMIN_REPORT_MILESTONE_FRAUD_EVIDENCE_PACK

Report: fraud-evidence-pack.

**Route gap:** register `/reports/fraud-evidence-pack` in `app_routes.tsx` before `live: true`.

**Status:** DRAFT  
**Slug:** `admin_report_fraud_evidence_pack`  
...
- `/reports/fraud-evidence-pack` — Signed per-click fraud evidence pack
- API: `GET /api/v1/reports/fraud-evidence-pack`
- Column contract: `ops_reports.yaml#/FraudEvidencePack` row keys: click_id, customer_id, campaign_id, timeline, fraud_events, signals, digest_sha256, signature
...
| :--- | :--- | :--- |
| GET | GET /api/v1/reports/fraud-evidence-pack | reportFraudEvidencePack |
| Columns | `ops_reports.yaml#/FraudEvidencePack` row keys: click_id, customer_id, campaign_id, timeline, fraud_events, signals, digest_sha256, signature | openapi |
| Envelope | rows + freshness (+ next_cursor when paginated) | ReportMapEnvelope or typed response |
| Schema | ops_reports.yaml#/FraudEvidencePack | openapi.d.ts row type |
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Report title from catalog or static (fraud-evidence-pack) | GET /api/v1/reports/catalog or reportTitle() |
| freshness | FreshnessBadge | freshness.stale, freshness.ch_lag_seconds, freshness_label when present | envelope.freshness |
| filters | ReportFilterPanel | customer_id, from, to, compare, campaign_id, limit, offset, cursor, click_id | URL searchParams → query string |
| toolbar | ReportToolbar | Export → POST /api/v1/reports/jobs; optional saved views | jobs API + /api/v1/views when permitted |
| content | report_fraud_evidence_pack | Signed pack object; not paginated rows[] | rows[] or typed response |
...
| `live` | false |
| Handler | internal/reports/ — `reportFraudEvidencePack` |

### 4.3 Layout and placement (grid contract)


**Non-sortable headers:** click_id, customer_id, campaign_id, timeline, fraud_events, signals, digest_sha256, signature
...
```
/api/v1/reports/fraud-evidence-pack?customer_id={uuid}&from={iso}&to={iso}&click_id={click_id}
```
...
| Path | Role |
| :--- | :--- |
| web/src/ui/reports/report_fraud_evidence_pack.tsx | FraudEvidencePackView section |
| web/src/ui/reports/report_fraud_evidence_pack.module.css | Grid/heatmap CSS module |
| web/src/helpers/reports_api.ts | fetchReport('fraud-evidence-pack', params) |
| web/src/pages/report_route_pages.tsx | Compose until ui/reports migration |

**Legacy page:** `report_stub_page.tsx (fraud-evidence-pack)`
...
| :--- | :--- | :--- | :--- |
| 1 | api/openapi/paths/ops_reports.yaml | Confirm `reportFraudEvidencePack` params + response schema | openapi_gate.sh |
| 2 | make openapi-types | Regenerate openapi.d.ts | typecheck |
| 3 | web/src/helpers/reports_api.ts | fetchReport('fraud-evidence-pack', params) | Compiles |
| 4 | web/src/app_routes.tsx | Register gap route `/reports/fraud-evidence-pack` + nav/catalog entry | Route resolves |
| 5 | web/src/ui/reports/report_fraud_evidence_pack.tsx | FraudEvidencePackView section | check_ui_surface_gate.sh |
...
```bash
Register `/reports/fraud-evidence-pack` in app_routes before catalog live:true
curl -s '/api/v1/reports/fraud-evidence-pack?customer_id={uuid}&from={iso}&to={iso}&click_id={click_id}' | jq '.freshness'
```

| Check | Command / procedure | Pass criteria |
| :--- | :--- | :--- |
| No ghost labels | rg 'ghost_' web/src/ui/reports/report_fraud_evidence_pack | no matches |
...
- [ ] Legacy `report_stub_page.tsx (fraud-evidence-pack)` replaced or delegates to `ui/reports/`
