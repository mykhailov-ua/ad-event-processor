# ADMIN_REPORT_MILESTONE_RTB_OVERVIEW

Report: rtb-overview.

**Status:** DRAFT  
**Slug:** `admin_report_rtb_overview`  
...
- `/reports/rtb/overview` — RTB auction overview KPIs
- API: `GET /api/v1/reports/rtb/overview`
- Column contract: Handler row keys (ReportMapEnvelope): deal_id, bids, wins, win_rate, spend_micro
...
| :--- | :--- | :--- |
| GET | GET /api/v1/reports/rtb/overview | reportRtbOverview |
| Columns | Handler row keys (ReportMapEnvelope): deal_id, bids, wins, win_rate, spend_micro | handler |
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Report title from catalog or static (rtb-overview) | GET /api/v1/reports/catalog or reportTitle() |
...
| content | report_rtb_overview | role=grid; column keys from OpenAPI/handler rows[] | rows[] or typed response |
...
| Field | Spec |
| :--- | :--- |
| Path | `/reports/rtb/overview` |
| `live` | true |
| Handler | internal/reports/ — `reportRtbOverview` |

### 4.3 Layout and placement (grid contract)


**Non-sortable headers:** deal_id, bids, wins, win_rate, spend_micro
...
```
/api/v1/reports/rtb/overview?customer_id={uuid}&from={iso}&to={iso}
```
...
| Path | Role |
| :--- | :--- |
| web/src/ui/reports/report_rtb_overview.tsx | ReportDataGrid section |
| web/src/ui/reports/report_rtb_overview.module.css | Grid/heatmap CSS module |
| web/src/helpers/reports_api.ts | fetchReport('rtb-overview', params) |
| web/src/pages/report_route_pages.tsx | Compose until ui/reports migration |

**Legacy page:** `report_route_pages.tsx RtbOverviewReportPage`
...
| :--- | :--- | :--- | :--- |
| 1 | api/openapi/paths/ops_reports.yaml | Confirm `reportRtbOverview` params + response schema | openapi_gate.sh |
| 2 | make openapi-types | Regenerate openapi.d.ts | typecheck |
| 3 | web/src/helpers/reports_api.ts | fetchReport('rtb-overview', params) | Compiles |
| 4 | web/src/app_routes.tsx | Register `/reports/rtb/overview` (legacy compose until ui/reports lands) | Route resolves |
| 5 | web/src/ui/reports/report_rtb_overview.tsx | ReportDataGrid per 4.3 | surface gate |
...
```bash
curl -s '/api/v1/reports/rtb/overview?customer_id={uuid}&from={iso}&to={iso}' | jq '.freshness'
```

| Check | Command / procedure | Pass criteria |
| :--- | :--- | :--- |
| No ghost labels | rg 'ghost_' web/src/ui/reports/report_rtb_overview | no matches |
...
- [ ] Legacy `report_route_pages.tsx RtbOverviewReportPage` replaced or delegates to `ui/reports/`
