# ADMIN_REPORT_MILESTONE_DISCREPANCY

Report: discrepancy-buy-sell.

**Status:** DRAFT  
**Slug:** `admin_report_discrepancy_buy_sell`  
...
- `/reports/discrepancy-buy-sell` — Buy vs sell spend discrepancy
- API: `GET /api/v1/reports/discrepancy-buy-sell`
- Column contract: Handler row keys (ReportMapEnvelope): campaign_id, buy_spend_micro, sell_rev_micro, delta_micro, delta_pct
...
| :--- | :--- | :--- |
| GET | GET /api/v1/reports/discrepancy-buy-sell | reportDiscrepancyBuySell |
| Columns | Handler row keys (ReportMapEnvelope): campaign_id, buy_spend_micro, sell_rev_micro, delta_micro, delta_pct | handler |
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Report title from catalog or static (discrepancy-buy-sell) | GET /api/v1/reports/catalog or reportTitle() |
...
| content | report_discrepancy_buy_sell | role=grid; column keys from OpenAPI/handler rows[] | rows[] or typed response |
...
| `live` | true |
| Handler | internal/reports/ — `reportDiscrepancyBuySell` |

### 4.3 Layout and placement (grid contract)


**Non-sortable headers:** campaign_id, buy_spend_micro, sell_rev_micro, delta_micro, delta_pct
...
```
/api/v1/reports/discrepancy-buy-sell?customer_id={uuid}&from={iso}&to={iso}
```
...
| Path | Role |
| :--- | :--- |
| web/src/ui/reports/report_discrepancy_buy_sell.tsx | ReportDataGrid section |
| web/src/ui/reports/report_discrepancy_buy_sell.module.css | Grid/heatmap CSS module |
| web/src/helpers/reports_api.ts | fetchReport('discrepancy-buy-sell', params) |
| web/src/pages/report_route_pages.tsx | Compose until ui/reports migration |

**Legacy page:** `report_configs.ts discrepancy-buy-sell`
...
| :--- | :--- | :--- | :--- |
| 1 | api/openapi/paths/ops_reports.yaml | Confirm `reportDiscrepancyBuySell` params + response schema | openapi_gate.sh |
| 2 | make openapi-types | Regenerate openapi.d.ts | typecheck |
| 3 | web/src/helpers/reports_api.ts | fetchReport('discrepancy-buy-sell', params) | Compiles |
| 4 | web/src/app_routes.tsx | Register `/reports/discrepancy-buy-sell` (legacy compose until ui/reports lands) | Route resolves |
| 5 | web/src/ui/reports/report_discrepancy_buy_sell.tsx | ReportDataGrid per 4.3 | surface gate |
...
```bash
curl -s '/api/v1/reports/discrepancy-buy-sell?customer_id={uuid}&from={iso}&to={iso}' | jq '.freshness'
```

| Check | Command / procedure | Pass criteria |
| :--- | :--- | :--- |
| No ghost labels | rg 'ghost_' web/src/ui/reports/report_discrepancy_buy_sell | no matches |
...
- [ ] Legacy `report_configs.ts discrepancy-buy-sell` replaced or delegates to `ui/reports/`
