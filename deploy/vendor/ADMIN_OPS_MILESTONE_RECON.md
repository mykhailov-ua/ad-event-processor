# ADMIN_OPS_MILESTONE_RECON

Ops: /ops/recon.

**Status:** DRAFT  
**Slug:** `admin_ops_recon`  
...
- `/ops/recon` — Budget reconciliation run history grid
- API: GET /api/v1/recon/runs
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Reconciliation runs | static |
| filters | ReconFilterBar | date/status filters per OpenAPI | URL params |
| grid | ReconRunsGrid | Run history rows | ReconRun[] |
| footer | PaginationBar | limit/offset | envelope |
| empty | EmptyState | No runs in window | items.length===0 |
...
| :--- | :--- | :--- |
| Primary fetch | GET /api/v1/recon/runs | Server owns business rules |
...
| Path | Role |
| :--- | :--- |
| web/src/pages/ops_recon_page.tsx | Compose only |
...
**Legacy page:** `ops_recon_page.tsx`
...
| 5 | web/src/pages/ops_recon_page.tsx | Thin compose; permission gates | No *.module.css on page |
...
- [ ] Legacy `ops_recon_page.tsx` replaced or delegates to `ui/ops/`
