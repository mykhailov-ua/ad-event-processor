# ADMIN_REPORT_MILESTONE_RTT_SPLIT_TUNNEL

Report: rtt-split-tunnel.

**Route gap:** register `/reports/rtt-split-tunnel` in `app_routes.tsx` before `live: true`.

**Status:** DRAFT  
**Slug:** `admin_report_rtt_split_tunnel`  
...
- `/reports/rtt-split-tunnel` — RTT split-tunnel distribution
- API: `GET /api/v1/reports/rtt-split-tunnel`
- Column contract: Handler row keys (ReportMapEnvelope): campaign_id, country, event_count, split_tunnel_count, split_tunnel_share, share_label, coverage_pct, coverage_label
...
| :--- | :--- | :--- |
| GET | GET /api/v1/reports/rtt-split-tunnel | reportRTTSplitTunnel |
| Columns | Handler row keys (ReportMapEnvelope): campaign_id, country, event_count, split_tunnel_count, split_tunnel_share, share_label, coverage_pct, coverage_label | handler |
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Report title from catalog or static (rtt-split-tunnel) | GET /api/v1/reports/catalog or reportTitle() |
...
| content | report_rtt_split_tunnel | role=grid; column keys from OpenAPI/handler rows[] | rows[] or typed response |
...
| `live` | false |
| Handler | internal/reports/ — `reportRTTSplitTunnel` |

### 4.3 Layout and placement (grid contract)


**Non-sortable headers:** campaign_id, country, event_count, split_tunnel_count, split_tunnel_share, share_label, coverage_pct, coverage_label
...
```
/api/v1/reports/rtt-split-tunnel?customer_id={uuid}&from={iso}&to={iso}
```
...
| Path | Role |
| :--- | :--- |
| web/src/ui/reports/report_rtt_split_tunnel.tsx | ReportDataGrid section |
| web/src/ui/reports/report_rtt_split_tunnel.module.css | Grid/heatmap CSS module |
| web/src/helpers/reports_api.ts | fetchReport('rtt-split-tunnel', params) |
| web/src/pages/report_route_pages.tsx | Compose until ui/reports migration |

**Legacy page:** `report_stub_page.tsx (rtt-split-tunnel)`
...
| :--- | :--- | :--- | :--- |
| 1 | api/openapi/paths/ops_reports.yaml | Confirm `reportRTTSplitTunnel` params + response schema | openapi_gate.sh |
| 2 | make openapi-types | Regenerate openapi.d.ts | typecheck |
| 3 | web/src/helpers/reports_api.ts | fetchReport('rtt-split-tunnel', params) | Compiles |
| 4 | web/src/app_routes.tsx | Register gap route `/reports/rtt-split-tunnel` + nav/catalog entry | Route resolves |
| 5 | web/src/ui/reports/report_rtt_split_tunnel.tsx | ReportDataGrid section | check_ui_surface_gate.sh |
...
```bash
Register `/reports/rtt-split-tunnel` in app_routes before catalog live:true
curl -s '/api/v1/reports/rtt-split-tunnel?customer_id={uuid}&from={iso}&to={iso}' | jq '.freshness'
```

| Check | Command / procedure | Pass criteria |
| :--- | :--- | :--- |
| No ghost labels | rg 'ghost_' web/src/ui/reports/report_rtt_split_tunnel | no matches |
...
- [ ] Legacy `report_stub_page.tsx (rtt-split-tunnel)` replaced or delegates to `ui/reports/`
