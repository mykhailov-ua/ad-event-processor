# ADMIN_REPORT_MILESTONE_TG_BOTS

Report: telegram-bots.

**Status:** DRAFT  
**Slug:** `admin_report_telegram_bots`  
...
- `/reports/telegram/bots` — Telegram bot performance
- API: `GET /api/v1/reports/telegram/bots`
- Column contract: Handler row keys (ReportMapEnvelope): bot_id, bot_username, clicks, conversions, fraud_signals
...
| :--- | :--- | :--- |
| GET | GET /api/v1/reports/telegram/bots | reportTelegramBots |
| Columns | Handler row keys (ReportMapEnvelope): bot_id, bot_username, clicks, conversions, fraud_signals | handler |
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Report title from catalog or static (telegram-bots) | GET /api/v1/reports/catalog or reportTitle() |
...
| content | report_telegram_bots | role=grid; column keys from OpenAPI/handler rows[] | rows[] or typed response |
...
| `live` | false |
| Handler | internal/reports/ — `reportTelegramBots` |

### 4.3 Layout and placement (grid contract)


**Non-sortable headers:** bot_id, bot_username, clicks, conversions, fraud_signals
...
```
/api/v1/reports/telegram/bots?from={iso}&to={iso}&customer_id={uuid}
```
...
| Path | Role |
| :--- | :--- |
| web/src/ui/reports/report_telegram_bots.tsx | ReportDataGrid section |
| web/src/ui/reports/report_telegram_bots.module.css | Grid/heatmap CSS module |
| web/src/helpers/reports_api.ts | fetchReport('telegram-bots', params) |
| web/src/pages/report_telegram_bots_page.tsx | Compose until ui/reports migration |

**Legacy page:** `report_telegram_bots_page.tsx`
...
| :--- | :--- | :--- | :--- |
| 1 | api/openapi/paths/ops_reports.yaml | Confirm `reportTelegramBots` params + response schema | openapi_gate.sh |
| 2 | make openapi-types | Regenerate openapi.d.ts | typecheck |
| 3 | web/src/helpers/reports_api.ts | fetchReport('telegram-bots', params) | Compiles |
| 4 | web/src/app_routes.tsx | Register `/reports/telegram/bots` (legacy compose until ui/reports lands) | Route resolves |
| 5 | web/src/ui/reports/report_telegram_bots.tsx | ReportDataGrid per 4.3 | surface gate |
...
```bash
curl -s '/api/v1/reports/telegram/bots?from={iso}&to={iso}&customer_id={uuid}' | jq '.freshness'
```

| Check | Command / procedure | Pass criteria |
| :--- | :--- | :--- |
| No ghost labels | rg 'ghost_' web/src/ui/reports/report_telegram_bots | no matches |
...
- [ ] Legacy `report_telegram_bots_page.tsx` replaced or delegates to `ui/reports/`
