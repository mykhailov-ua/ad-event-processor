# ADMIN_REPORT_MILESTONE_KEYWORDS

Report: keywords.

**Status:** DRAFT  
**Slug:** `admin_report_keywords`  
...
- `/reports/keywords` — Keyword performance grid
- API: `GET /api/v1/reports/keywords`
- Column contract: `ops_reports.yaml#/KeywordReportRow` row keys: keyword, campaign_id, impressions, clicks, conversions, spend_micro, revenue_micro, profit_micro, roi_pct, cpa_micro, ctr, ivt_rate
...
| :--- | :--- | :--- |
| GET | GET /api/v1/reports/keywords | reportKeywords |
| Columns | `ops_reports.yaml#/KeywordReportRow` row keys: keyword, campaign_id, impressions, clicks, conversions, spend_micro, revenue_micro, profit_micro, roi_pct, cpa_micro, ctr, ivt_rate | openapi |
| Envelope | rows + freshness (+ next_cursor when paginated) | ReportMapEnvelope or typed response |
| Schema | ops_reports.yaml#/KeywordReportRow | openapi.d.ts row type |
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Report title from catalog or static (keywords) | GET /api/v1/reports/catalog or reportTitle() |
...
| content | report_keywords | role=grid; column keys from OpenAPI/handler rows[] | rows[] or typed response |
...
| `live` | true |
| Handler | internal/reports/ — `reportKeywords` |

### 4.3 Layout and placement (grid contract)


**Non-sortable headers:** keyword, campaign_id, impressions, clicks, conversions, spend_micro, revenue_micro, profit_micro, roi_pct, cpa_micro, ctr, ivt_rate
...
```
/api/v1/reports/keywords?customer_id={uuid}&from={iso}&to={iso}
```
...
| Path | Role |
| :--- | :--- |
| web/src/ui/reports/report_keywords.tsx | ReportDataGrid section |
| web/src/ui/reports/report_keywords.module.css | Grid/heatmap CSS module |
| web/src/helpers/reports_api.ts | fetchReport('keywords', params) |
| web/src/pages/report_route_pages.tsx | Compose until ui/reports migration |

**Legacy page:** `report_query_page.tsx (keywords)`
...
| :--- | :--- | :--- | :--- |
| 1 | api/openapi/paths/ops_reports.yaml | Confirm `reportKeywords` params + response schema | openapi_gate.sh |
| 2 | make openapi-types | Regenerate openapi.d.ts | typecheck |
| 3 | web/src/helpers/reports_api.ts | fetchReport('keywords', params) | Compiles |
| 4 | web/src/app_routes.tsx | Register `/reports/keywords` (legacy compose until ui/reports lands) | Route resolves |
| 5 | web/src/ui/reports/report_keywords.tsx | ReportDataGrid per 4.3 | surface gate |
...
```bash
curl -s '/api/v1/reports/keywords?customer_id={uuid}&from={iso}&to={iso}' | jq '.freshness'
```

| Check | Command / procedure | Pass criteria |
| :--- | :--- | :--- |
| No ghost labels | rg 'ghost_' web/src/ui/reports/report_keywords | no matches |
...
| Export POST | POST /api/v1/reports/jobs body report_key=keywords | 202 + job id |
...
- [ ] Legacy `report_query_page.tsx (keywords)` replaced or delegates to `ui/reports/`
