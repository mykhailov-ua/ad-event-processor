# ADMIN_REPORT_MILESTONE_FRAUD_BREAKDOWN

Report: fraud-breakdown.

**Status:** DRAFT  
**Slug:** `admin_report_fraud_breakdown`  
...
- `/reports/fraud-breakdown` — Fraud tier breakdown by reason
- API: `GET /api/v1/reports/fraud-breakdown`
- Column contract: Handler row keys (ReportMapEnvelope): campaign_id, placement_id, fraud_reason, event_count, silent_reject_count, silent_reject_ratio
...
| :--- | :--- | :--- |
| GET | GET /api/v1/reports/fraud-breakdown | reportFraudBreakdown |
| Columns | Handler row keys (ReportMapEnvelope): campaign_id, placement_id, fraud_reason, event_count, silent_reject_count, silent_reject_ratio | handler |
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Report title from catalog or static (fraud-breakdown) | GET /api/v1/reports/catalog or reportTitle() |
...
| content | report_fraud_breakdown | role=grid; column keys from OpenAPI/handler rows[] | rows[] or typed response |
...
| `live` | true |
| Handler | internal/reports/ — `reportFraudBreakdown` |

### 4.3 Layout and placement (grid contract)


**Non-sortable headers:** campaign_id, placement_id, fraud_reason, event_count, silent_reject_count, silent_reject_ratio
...
```
/api/v1/reports/fraud-breakdown?customer_id={uuid}&from={iso}&to={iso}
```
...
| Path | Role |
| :--- | :--- |
| web/src/ui/reports/report_fraud_breakdown.tsx | ReportDataGrid section |
| web/src/ui/reports/report_fraud_breakdown.module.css | Grid/heatmap CSS module |
| web/src/helpers/reports_api.ts | fetchReport('fraud-breakdown', params) |
| web/src/pages/report_route_pages.tsx | Compose until ui/reports migration |

**Legacy page:** `report_route_pages.tsx FraudBreakdownReportPage`
...
| :--- | :--- | :--- | :--- |
| 1 | api/openapi/paths/ops_reports.yaml | Confirm `reportFraudBreakdown` params + response schema | openapi_gate.sh |
| 2 | make openapi-types | Regenerate openapi.d.ts | typecheck |
| 3 | web/src/helpers/reports_api.ts | fetchReport('fraud-breakdown', params) | Compiles |
| 4 | web/src/app_routes.tsx | Register `/reports/fraud-breakdown` (legacy compose until ui/reports lands) | Route resolves |
| 5 | web/src/ui/reports/report_fraud_breakdown.tsx | ReportDataGrid per 4.3 | surface gate |
...
```bash
curl -s '/api/v1/reports/fraud-breakdown?customer_id={uuid}&from={iso}&to={iso}' | jq '.freshness'
```

| Check | Command / procedure | Pass criteria |
| :--- | :--- | :--- |
| No ghost labels | rg 'ghost_' web/src/ui/reports/report_fraud_breakdown | no matches |
...
| Export POST | POST /api/v1/reports/jobs body report_key=fraud-breakdown | 202 + job id |
...
- [ ] Legacy `report_route_pages.tsx FraudBreakdownReportPage` replaced or delegates to `ui/reports/`
