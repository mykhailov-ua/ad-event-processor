# ADMIN_REPORT_MILESTONE_WIRE_SIGNAL_BREAKDOWN

Report: wire-signal-breakdown.

**Route gap:** register `/reports/wire-signal-breakdown` in `app_routes.tsx` before `live: true`.

**Status:** DRAFT  
**Slug:** `admin_report_wire_signal_breakdown`  
...
- `/reports/wire-signal-breakdown` — L7/TLS/H2 wire fraud signal breakdown
- API: `GET /api/v1/reports/wire-signal-breakdown`
- Column contract: `ops_reports.yaml#/WireSignalBreakdownRow` row keys: campaign_id, fraud_reason, fraud_category, fraud_category_label, event_count, silent_reject_count, silent_reject_ratio, signals_degraded
...
| :--- | :--- | :--- |
| GET | GET /api/v1/reports/wire-signal-breakdown | reportWireSignalBreakdown |
| Columns | `ops_reports.yaml#/WireSignalBreakdownRow` row keys: campaign_id, fraud_reason, fraud_category, fraud_category_label, event_count, silent_reject_count, silent_reject_ratio, signals_degraded | openapi |
| Envelope | rows + freshness (+ next_cursor when paginated) | ReportMapEnvelope or typed response |
| Schema | ops_reports.yaml#/WireSignalBreakdownRow | openapi.d.ts row type |
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Report title from catalog or static (wire-signal-breakdown) | GET /api/v1/reports/catalog or reportTitle() |
...
| content | report_wire_signal_breakdown | role=grid; column keys from OpenAPI/handler rows[] | rows[] or typed response |
...
| `live` | false |
| Handler | internal/reports/ — `reportWireSignalBreakdown` |

### 4.3 Layout and placement (grid contract)


**Non-sortable headers:** campaign_id, fraud_reason, fraud_category, fraud_category_label, event_count, silent_reject_count, silent_reject_ratio, signals_degraded
...
```
/api/v1/reports/wire-signal-breakdown?customer_id={uuid}&from={iso}&to={iso}
```
...
| Path | Role |
| :--- | :--- |
| web/src/ui/reports/report_wire_signal_breakdown.tsx | ReportDataGrid section |
| web/src/ui/reports/report_wire_signal_breakdown.module.css | Grid/heatmap CSS module |
| web/src/helpers/reports_api.ts | fetchReport('wire-signal-breakdown', params) |
| web/src/pages/report_route_pages.tsx | Compose until ui/reports migration |

**Legacy page:** `report_stub_page.tsx (wire-signal-breakdown)`
...
| :--- | :--- | :--- | :--- |
| 1 | api/openapi/paths/ops_reports.yaml | Confirm `reportWireSignalBreakdown` params + response schema | openapi_gate.sh |
| 2 | make openapi-types | Regenerate openapi.d.ts | typecheck |
| 3 | web/src/helpers/reports_api.ts | fetchReport('wire-signal-breakdown', params) | Compiles |
| 4 | web/src/app_routes.tsx | Register gap route `/reports/wire-signal-breakdown` + nav/catalog entry | Route resolves |
| 5 | web/src/ui/reports/report_wire_signal_breakdown.tsx | ReportDataGrid section | check_ui_surface_gate.sh |
...
```bash
Register `/reports/wire-signal-breakdown` in app_routes before catalog live:true
curl -s '/api/v1/reports/wire-signal-breakdown?customer_id={uuid}&from={iso}&to={iso}' | jq '.freshness'
```

| Check | Command / procedure | Pass criteria |
| :--- | :--- | :--- |
| No ghost labels | rg 'ghost_' web/src/ui/reports/report_wire_signal_breakdown | no matches |
...
| Export POST | POST /api/v1/reports/jobs body report_key=wire-signal-breakdown | 202 + job id |
...
- [ ] Legacy `report_stub_page.tsx (wire-signal-breakdown)` replaced or delegates to `ui/reports/`
