# ADMIN_REPORT_MILESTONE_DATA_QUALITY

Report: data-quality.

**Status:** DRAFT  
**Slug:** `admin_report_data_quality`  
...
- `/reports/data-quality` — PG vs ClickHouse data quality signals
- API: `GET /api/v1/reports/data-quality`
- Column contract: Handler row keys (ReportMapEnvelope): campaign_id, date, pg_total, ch_total, diff_pct, severity
...
| :--- | :--- | :--- |
| GET | GET /api/v1/reports/data-quality | reportDataQuality |
| Columns | Handler row keys (ReportMapEnvelope): campaign_id, date, pg_total, ch_total, diff_pct, severity | handler |
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Report title from catalog or static (data-quality) | GET /api/v1/reports/catalog or reportTitle() |
...
| content | report_data_quality | role=grid; column keys from OpenAPI/handler rows[] | rows[] or typed response |
...
| `live` | true |
| Handler | internal/reports/ — `reportDataQuality` |

### 4.3 Layout and placement (grid contract)


**Non-sortable headers:** campaign_id, date, pg_total, ch_total, diff_pct, severity
...
```
/api/v1/reports/data-quality?customer_id={uuid}&from={iso}&to={iso}
```
...
| Path | Role |
| :--- | :--- |
| web/src/ui/reports/report_data_quality.tsx | ReportDataGrid section |
| web/src/ui/reports/report_data_quality.module.css | Grid/heatmap CSS module |
| web/src/helpers/reports_api.ts | fetchReport('data-quality', params) |
| web/src/pages/report_route_pages.tsx | Compose until ui/reports migration |

**Legacy page:** `report_route_pages.tsx DataQualityReportPage`
...
| :--- | :--- | :--- | :--- |
| 1 | api/openapi/paths/ops_reports.yaml | Confirm `reportDataQuality` params + response schema | openapi_gate.sh |
| 2 | make openapi-types | Regenerate openapi.d.ts | typecheck |
| 3 | web/src/helpers/reports_api.ts | fetchReport('data-quality', params) | Compiles |
| 4 | web/src/app_routes.tsx | Register `/reports/data-quality` (legacy compose until ui/reports lands) | Route resolves |
| 5 | web/src/ui/reports/report_data_quality.tsx | ReportDataGrid per 4.3 | surface gate |
...
```bash
curl -s '/api/v1/reports/data-quality?customer_id={uuid}&from={iso}&to={iso}' | jq '.freshness'
```

| Check | Command / procedure | Pass criteria |
| :--- | :--- | :--- |
| No ghost labels | rg 'ghost_' web/src/ui/reports/report_data_quality | no matches |
...
- [ ] Legacy `report_route_pages.tsx DataQualityReportPage` replaced or delegates to `ui/reports/`
