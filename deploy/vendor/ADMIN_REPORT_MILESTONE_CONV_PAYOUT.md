# ADMIN_REPORT_MILESTONE_CONV_PAYOUT

Report: conversion-type-payout.

**Status:** DRAFT  
**Slug:** `admin_report_conversion_type_payout`  
...
- `/reports/conversion-type-payout` — Payout grouped by conversion goal_name
- API: `GET /api/v1/reports/conversion-type-payout`
- Column contract: `ops_reports.yaml#/ConversionTypePayoutRow` row keys: campaign_id, goal_name, conversions, payout_micro
...
| :--- | :--- | :--- |
| GET | GET /api/v1/reports/conversion-type-payout | reportConversionTypePayout |
| Columns | `ops_reports.yaml#/ConversionTypePayoutRow` row keys: campaign_id, goal_name, conversions, payout_micro | openapi |
| Envelope | rows + freshness (+ next_cursor when paginated) | ReportMapEnvelope or typed response |
| Schema | ops_reports.yaml#/ConversionTypePayoutRow | openapi.d.ts row type |
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Report title from catalog or static (conversion-type-payout) | GET /api/v1/reports/catalog or reportTitle() |
...
| content | report_conversion_type_payout | role=grid; column keys from OpenAPI/handler rows[] | rows[] or typed response |
...
| `live` | true |
| Handler | internal/reports/ — `reportConversionTypePayout` |

### 4.3 Layout and placement (grid contract)


**Non-sortable headers:** campaign_id, goal_name, conversions, payout_micro
...
```
/api/v1/reports/conversion-type-payout?customer_id={uuid}&from={iso}&to={iso}
```
...
| Path | Role |
| :--- | :--- |
| web/src/ui/reports/report_conversion_type_payout.tsx | ReportDataGrid section |
| web/src/ui/reports/report_conversion_type_payout.module.css | Grid/heatmap CSS module |
| web/src/helpers/reports_api.ts | fetchReport('conversion-type-payout', params) |
| web/src/pages/report_route_pages.tsx | Compose until ui/reports migration |

**Legacy page:** `report_route_pages.tsx ConversionTypePayoutReportPage`
...
| :--- | :--- | :--- | :--- |
| 1 | api/openapi/paths/ops_reports.yaml | Confirm `reportConversionTypePayout` params + response schema | openapi_gate.sh |
| 2 | make openapi-types | Regenerate openapi.d.ts | typecheck |
| 3 | web/src/helpers/reports_api.ts | fetchReport('conversion-type-payout', params) | Compiles |
| 4 | web/src/app_routes.tsx | Register `/reports/conversion-type-payout` (legacy compose until ui/reports lands) | Route resolves |
| 5 | web/src/ui/reports/report_conversion_type_payout.tsx | ReportDataGrid per 4.3 | surface gate |
...
```bash
curl -s '/api/v1/reports/conversion-type-payout?customer_id={uuid}&from={iso}&to={iso}' | jq '.freshness'
```

| Check | Command / procedure | Pass criteria |
| :--- | :--- | :--- |
| No ghost labels | rg 'ghost_' web/src/ui/reports/report_conversion_type_payout | no matches |
...
- [ ] Legacy `report_route_pages.tsx ConversionTypePayoutReportPage` replaced or delegates to `ui/reports/`
