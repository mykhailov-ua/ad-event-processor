# ADMIN_INTEGRATIONS_MILESTONE_MARGIN_GUARD

Integration: /integrations/margin-guard.

**Status:** DRAFT  
**Slug:** `admin_integrations_margin_guard`  
...
- `/integrations/margin-guard` — Policy editor per campaign; activity log; breach scan (legacy client fanout)
- APIs: GET/POST /api/v1/margin-guard/policies; GET .../activity; POST .../overrides; GET /api/v1/campaigns/{id}/margin
...
| :--- | :--- | :--- |
| Customer breach scan API | Legacy scanMarginBreaches N+1: campaigns list + per-campaign /margin | Milestone may keep fanout temporarily; document cold-path violation; prefer future list endpoint |
| Policy PATCH/DELETE | OpenAPI has GET/POST policies only | No edit/delete UI until ops exist |
...
| :--- | :--- | :--- | :--- |
| chrome | PageChrome | Margin guard | static |
| scope | CustomerCampaignPicker | customer_id + campaign_id | URL params |
| scan | BreachScanPanel | list campaigns with margin_breach | legacy scanMarginBreaches |
| policies | PolicyGrid | threshold policies | GET/POST .../policies?campaign_id= |
| activity | ActivityGrid | guard actions log | GET .../activity?campaign_id= |
| overrides | OverrideActions | clear placement override | POST .../overrides |
| detail_link | CampaignMarginLink | -> campaign margin tab | GET campaigns/{id}/margin |
...
| Topic | Spec |
| :--- | :--- |
| CSS ownership | `web/src/ui/margin_guard/*.module.css` only |
...
| :--- | :--- | :--- | :--- |
| customer_id | customer_id |  | operator scope for scan |
| campaign_id | campaign_id |  | policy/activity detail |
...
| Path | Role |
| :--- | :--- |
| web/src/pages/integrations_margin_guard_page.tsx | Compose |
| web/src/ui/margin_guard/* | Sections + CSS modules |
| web/src/helpers/margin_guard_api.ts | Typed API helpers |
| web/src/types/generated/openapi.d.ts | Generated types |

**Legacy page:** `integrations_margin_guard_page.tsx`
...
| :--- | :--- | :--- | :--- |
| 1 | GET/POST /api/v1/margin-guard/policies | Confirm OpenAPI ops + x-permissions | openapi_gate.sh |
...
| 4 | web/src/helpers/margin_guard_api.ts | Typed API helpers; secrets never logged | Compiles |
| 5 | web/src/ui/margin_guard/* | Sections per regions + grid_ascii | check_ui_surface_gate.sh |
| 6 | web/src/pages/integrations_margin_guard_page.tsx | Compose + URL params | No client filter on items[] |
...
| :--- | :--- | :--- |
| No client sort/filter | rg 'useMemo.*filter\|table_sort' web/src/pages/integrations_margin_guard_page.tsx web/src/ui/margin_guard/ | no matches |
...
- [ ] Legacy `integrations_margin_guard_page.tsx` replaced or delegates to `ui/margin_guard/`
- [ ] Verification output pasted in PR

## 9. Rollback

Revert `web/src/ui/margin_guard/`, helpers, and page compose in one slice; no half migration.
