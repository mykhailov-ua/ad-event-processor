...
|----------------|--------|------------------------|
| `AuthServiceClient` / `AuthServiceServer` | Monolith in-process via `identity.AuthAPI`; `Login`/`APIKey` domain types | `identity/{handler,handler_core,handler_grpc,auth_convert,apikey_types,apikey_convert,serve,grpc_api,resolve_api}.go`; `controlplane/auth_client.go` |
