...
10. Rename files per naming rules. — done for billing/payment/identity/notifier/controlplane cold path (`handler→api`, `worker_*`, `client_*`); adminapi ops/reports/selfserve consolidated; controlplane `service_*` merged by stem (`service_rtb`, `service_node`, `service_shard`, fraud/pacing/customers/campaigns/delivery)
...
Controlplane file consolidation: `operation_lease.go`, `outbox.go`, `recon.go`, `api.go`, `node_scorer.go`, `ops.go`/`ops_readers.go`, `service_platform.go`, `quota.go`, `shard_control.go`, `service_campaign.go`, `workers.go`; adminapi reports/billing/wire merged; notifier `provider.go`; identity `api.go`.
Renames: `fraud_modeling/` → `fraudtrain/`; `mgmt_pg_gate` → `postgres_gate` (`PostgresGate`); TCP/UDP `*_MGMT_*` config → `TCPControl*`/`UDPControl*` (env `TCP_CONTROL_*` / `UDP_CONTROL_*` with legacy fallback).
