#!/usr/bin/env bash
# tarpit_test.sh: offline Lua tests + optional live edge smoke (GAP-CMP-01).
set -euo pipefail
...
if [[ "${EDGE_TARPIT_ENABLED:-}" == "1" || "${EDGE_TARPIT_ENABLED:-}" == "true" ]]; then
	if awk -v b="$before" -v a="$after" 'BEGIN { exit !(a > b) }'; then
		echo "chaos_proof fault=edge_tarpit_triggered tarpit_before=${before} tarpit_after=${after} baseline_ok=true"
	else
