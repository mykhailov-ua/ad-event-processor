...
case "$CMD" in
  start)
    ...
    bash "$SCRIPTS/test/bpf/resolve_targets.sh" "$TARGETS_JSON" "${AD_EVENT_PROCESSOR_BPF_TARGETS:-tracker,nginx,redis,processor}"
  ...
  stop)
    ...
    if [[ -f "$LOG_FILE" ]] && grep -q "memlock rlimit" "$LOG_FILE" 2> /dev/null; then
      log "WARN: collector failed memlock - run: sudo bash scripts/dev/stack/bpf_session.sh start"
    fi
    ...
    if aed_go_run ./cmd/load-report bpf "$OUT_DIR"; then
      :
    else
