...
if [[ "$(id -u)" -eq 0 && "${AD_EVENT_PROCESSOR_LOAD_MALFORMED_ROOT_OK:-0}" != "1" ]]; then
  die "do not run as root/sudo - bpf_probe_session calls sudo internally; root breaks 'docker compose -f' (use: AD_EVENT_PROCESSOR_BPF_PROBE=1 bash scripts/test/load/malformed.sh smoke)"
fi
...
if [[ "$PREPARE" == "1" ]]; then
  if [[ "$CONSTRAINED" == "1" ]]; then
    ...
    bash "$SCRIPTS/test/load/prepare_constrained_stack.sh" 2>&1 | tee -a "$OUT/compose.log"
  else
  ...
  if [[ "$CONSTRAINED" == "1" ]]; then
    ...
    bash "$SCRIPTS/test/load/seed_limits.sh" || log "WARN: seed limits failed"
