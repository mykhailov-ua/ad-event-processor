...
bash "$SCRIPTS/test/reconcile_ingestion_migrations.sh"
bash "$SCRIPTS/test/load/verify_schema.sh"
...
go run ./cmd/admin db seed-load-test-stack-sql --count 100 | "${COMPOSE[@]}" exec -T db psql -h localhost -p "$DB_PORT" -U ad_event_processor_user -d ad_event_processor
...
bash "$SCRIPTS/test/load/seed_limits.sh"
