...
if [ ! -f "$ENV_FILE" ]; then
  ...
  sed -i 's/your_redis_password_here/sentinel_fault_test/' "$ENV_FILE" 2> /dev/null \
...
echo "test_sentinel_failover: post-failover verify (marker, budget, load stats, fault_proof)..."
