...
if [[ "$PREPARE" == "1" ]]; then
  ...
  bash "$SCRIPTS/test/load/prepare_constrained_stack.sh" 2>&1 | tee "$OUT/compose.log"
fi
...
if [[ "${AD_EVENT_PROCESSOR_BPF_PROBE:-0}" == "1" ]]; then
  bash "$SCRIPTS/test/bpf/probe_session.sh" start "$OUT" || log "WARN: BPF start failed (need sudo + bpf-dev)"
