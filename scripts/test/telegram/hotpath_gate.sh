...
go test -short -count=1 -run 'TgClick|ParseTgClick|DecodeTgBid|Check_zeroAlloc_localQuantaFullSkip' ./internal/ingest/...
...
if [[ -n "$SESSION_DIR" && -d "$SESSION_DIR" ]]; then
  ...
  PREPARE="${PREPARE:-0}" bash "$SCRIPTS/test/telegram/hotpath_soak.sh"
