...
if ! go test ./internal/edge/bpf/ -run='^TestEdgeSealed_ValidLicenseLoadsCollection$' -count=1 -v 2>&1 | tee -a "$LOG"; then
	die "sealed collection load failed (see $LOG)"
fi

log "smoke: MCK path unit (TestEdgeSealed_MCKMatchesLicenseFilePath)"
go test ./internal/edge/bpf/ -run='^TestEdgeSealed_MCKMatchesLicenseFilePath$' -count=1 >>"$LOG" 2>&1

export SEALED_BPF_XDP_SMOKE=1
log "attach lab on ${IFACE} (TestEdgeSealed_XDPAttachMatchesBaseline)"
go test ./internal/edge/bpf/ -run='^TestEdgeSealed_XDPAttachMatchesBaseline$' -count=1 -v 2>&1 | tee -a "$LOG"
