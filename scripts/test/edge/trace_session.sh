...
CGO_ENABLED=0 aed_go_build -tags ad_event_processor_bpf_trace -o bin/tracker ./cmd/tracker
...
bash "$SCRIPTS/dev/stack/bpf_session.sh" start "$OUT_DIR"
...
DEPLOY_COMPOSE_TRACKER=0 bash "$SCRIPTS/test/edge/live_smoke.sh" || die "pixel_live_smoke failed"
...
bash "$SCRIPTS/dev/stack/bpf_session.sh" stop "$OUT_DIR"

if command -v go > /dev/null 2>&1; then
  log "generating bpf report"
  bash "$SCRIPTS/dev/stack/bpf_session.sh" report "$OUT_DIR" || true
fi
