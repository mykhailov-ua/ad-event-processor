...
run_case "redteam7_openrtb_seed_coupling" \
  go test ./internal/ingest/ -run 'OpenRTBLicenseAllowed_seedCoupling' -count=1
...
if [[ "$(uname -s)" == "Linux" ]]; then
  ...
  run_case "redteam8_sealed_processor_ch_policy" \
    go test ./internal/ingest/ -run 'ProcessorCHIngestPolicy' -count=1
...
if [[ "$(uname -s)" == "Linux" ]] && command -v gdb > /dev/null 2>&1; then
  run_case "redteam9_gdb_guard_smoke" \
    env LICENSE_GDB_SMOKE=1 bash scripts/test/license/gdb_guard_smoke.sh
else
