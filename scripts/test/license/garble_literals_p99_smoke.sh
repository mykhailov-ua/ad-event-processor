...
if ! curl -sf --max-time 3 "${PROMETHEUS_URL%/}/-/ready" > /dev/null 2>&1; then
  log "skip (prometheus not ready at $PROMETHEUS_URL - run: bash scripts/test/load/prepare_constrained_stack.sh)"
  exit 0
fi

cid="$("${COMPOSE[@]}" ps -q tracker-0 2> /dev/null || true)"
if [[ -z "$cid" ]]; then
  log "skip (tracker-0 not running - run: bash scripts/test/load/prepare_constrained_stack.sh)"
  exit 0
fi
...
run_load() {
  ...
  DURATION="$DURATION" PREPARE=0 CONSTRAINED=1 bash scripts/test/load/malformed.sh smoke 2>&1 | tee "$logfile"
  sleep 5
}
