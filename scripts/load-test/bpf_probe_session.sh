#!/usr/bin/env bash
# Start/stop BPF probe session parallel to k6 load tests.
# Usage:
#   bpf_probe_session.sh start <out_dir>
#   bpf_probe_session.sh stop  <out_dir> [collector_pid]
set -euo pipefail
...
start)
	...
	bash "$SCRIPTS/load-test/bpf_resolve_targets.sh" "$TARGETS_JSON" "${ESPX_BPF_TARGETS:-tracker,nginx,redis,processor}"
...
stop)
	...
	# Collector may have been re-parented under sudo; stop by pid file name match.
	...
	bash "$SCRIPTS/load-test/analyze_bpf.sh" "$OUT_DIR" || true
	;;
*)
