#!/usr/bin/env bash
# Resolve host PIDs for BPF target_pids map (docker compose and/or native processes).
#
# Usage: bpf_resolve_targets.sh <targets.json> [tracker,nginx,redis,processor]
#
# Env:
#   ESPX_BPF_SAMPLE_RATE       sample rate written into targets.json
#   ESPX_BPF_NATIVE=1          also resolve local processes via pgrep
#   ESPX_BPF_EXTRA_TARGETS     comma list pid:role:name (role 1=tracker 2=nginx 3=redis 4=loadgen 5=processor)
#   ESPX_BPF_NATIVE_PATTERNS   override native pgrep patterns: role:pattern,role:pattern
set -euo pipefail
