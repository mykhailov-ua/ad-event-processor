#!/usr/bin/env bash
# Seed Redis + edge settings for load tests: high campaign RL, no false perimeter caps.
set -euo pipefail
...
# Blacklist version stamp so edge phase1 does not 503 on stale sync during dev load.
now="$(date +%s)"
