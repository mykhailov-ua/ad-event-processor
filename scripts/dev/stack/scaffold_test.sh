...
Examples:
  bash scripts/dev/stack/scaffold_test.sh internal/notify
  bash scripts/dev/stack/scaffold_test.sh notify --scenario enqueue
...
HOT_PATH=0
case "$PKG_PATH" in
  internal/ingest | internal/filter | internal/track | internal/stream | internal/domain | internal/rtb | cmd/tracker | pkg/broker) HOT_PATH=1 ;;
esac
...
echo "  3. bash scripts/ci/static/integration_test_slop.sh"
