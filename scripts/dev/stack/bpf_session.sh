...
case "$CMD" in
  start)
    ...
    bash "$SCRIPTS/test/bpf/probe_session.sh" start "$OUT"
    ...
    log "load: PREPARE=1 AD_EVENT_PROCESSOR_BPF_PROBE=1 bash scripts/test/load/malformed.sh business"
    log "stop: bash scripts/dev/stack/bpf_session.sh stop"
    ;;
  stop)
    OUT="$(resolve_out_dir "${2:-}")"
    PID=""
    ...
    bash "$SCRIPTS/test/bpf/probe_session.sh" stop "$OUT" "$PID"
  ...
  status)
    if [[ -f "$CURRENT_PATH_FILE" ]]; then
      ...
      log "no active session (run: sudo bash scripts/dev/stack/bpf_session.sh start)"
    fi
    ;;
  report)
    OUT="$(resolve_out_dir "${2:-}")"
    source "$SCRIPTS/lib/go.sh"
    if ! aed_go_run ./cmd/load-report bpf "$OUT"; then
