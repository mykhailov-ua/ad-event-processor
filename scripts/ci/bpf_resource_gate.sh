#!/usr/bin/env bash
# eBPF load-test resource gate: compose smoke + bpf-collector + load-report bpf-gate.
set -euo pipefail
...
if [[ "$STRICT" != "true" ]]; then
  ...
  log "hint: BPF_GATE_STRICT=true bash scripts/ci/bpf_resource_gate.sh"
  exit 0
fi
...
if ! bash "$SCRIPTS/test/bpf_requirements.sh" >> "$GATE_LOG" 2>&1; then
...
if [[ "$MODE" == "smoke" ]]; then
  ...
  CONSTRAINED=1 ESPX_BPF_PROBE=1 \
