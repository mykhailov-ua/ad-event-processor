...
run_luacheck() {
  ...
  if ((${#paths[@]} == 0)); then
    echo "lint_lua_gate: no Lua directories present" >&2
    exit 1
  fi
  echo "lint_lua_gate: luacheck (${paths[*]})..."
  if command -v luacheck > /dev/null 2>&1; then
    luacheck "${paths[@]}" --config .luacheckrc
    return 0
  fi
  if command -v docker > /dev/null 2>&1; then
    docker run --rm -v "$ROOT:/work" -w /work "$LUA_LINT_IMAGE" \
      sh -c "apk add --no-cache luacheck >/dev/null 2>&1 && luacheck $(lua_path_args) --config .luacheckrc"
    return 0
  fi
