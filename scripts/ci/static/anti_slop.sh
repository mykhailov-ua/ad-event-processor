...
if rg -n '^func BenchmarkUnifiedFilter_Check\(b \*testing\.B\)' internal/ingest internal/filter --glob '*_test.go' 2> /dev/null; then
  fail "rename BenchmarkUnifiedFilter_Check to BenchmarkUnifiedFilter_Check_mock (mock Redis harness)"
fi

if rg -n 'testing\.Testing\(\)' internal/controlplane internal/ingest internal/filter internal/track internal/stream internal/payment pkg/ \
  --glob '*.go' --glob '!*_test.go' 2> /dev/null; then
  fail "testing.Testing() in production code"
fi

if rg -n 'os\.Getenv\("CI"\)' internal/controlplane internal/ingest internal/filter internal/track internal/stream internal/payment pkg/ \
  --glob '*.go' --glob '!*_test.go' 2> /dev/null; then
...
done < <(find internal/controlplane internal/payment internal/ingest internal/filter internal/track internal/stream -mindepth 1 -maxdepth 1 -type d 2> /dev/null)
...
if rg -n 'nolint:errcheck' internal/controlplane internal/ingest internal/filter internal/track internal/stream internal/payment pkg/ \
  ...
  rg -n 'nolint:errcheck' internal/controlplane internal/ingest internal/filter internal/track internal/stream internal/payment pkg/ \
