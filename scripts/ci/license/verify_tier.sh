...
run_gate entangle-1a bash scripts/ci/static/mck_info_label.sh
...
if on_linux; then
  ...
  run_gate entangle-4b go test ./internal/ingest/ -run ResolveUnifiedFilterLua -count=1
  run_gate entangle-4c go test ./internal/ingest/ -run ProcessorCHIngestPolicy -count=1
...
if command -v openssl > /dev/null 2>&1 && openssl kdf -help > /dev/null 2>&1; then
  run_gate entangle-6 bash scripts/ci/license/differential.sh
else
  skip_gate entangle-6 "openssl kdf unavailable"
fi
if [[ "${SEALED_BPF_XDP_SMOKE:-0}" == "1" ]]; then
  run_gate entangle-7 bash scripts/test/edge/sealed_bpf_xdp_smoke.sh
else
  skip_gate entangle-7 "set SEALED_BPF_XDP_SMOKE=1 as root for XDP attach lab"
fi
if [[ "${GARBLE_LITERALS_P99_SMOKE:-0}" == "1" ]]; then
  run_gate garble-p99 bash scripts/test/license/garble_literals_p99_smoke.sh
else
...
run_gate release-4 bash scripts/ci/license/fuzz_nightly.sh
run_gate release-5 bash scripts/test/license/red_team_extended.sh
if [[ "${LICENSE_VERIFY_RELEASE_QA:-0}" == "1" ]]; then
  run_gate release-6 bash scripts/test/release/qa_smoke.sh
else
  skip_gate release-6 "set LICENSE_VERIFY_RELEASE_QA=1 for fuzz+garble release QA smoke"
fi

log "Hot-path alloc"
run_gate license-alloc bash scripts/ci/license/alloc.sh
