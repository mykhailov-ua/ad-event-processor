...
# Usage: dev_stack.sh {infra|full|sentinel|multi-region|crypto|down|status|build|bpf|probe}
set -euo pipefail
...
build)
	...
	bash "$SCRIPTS/local-dev/bpf_setup.sh" || echo "dev_stack: WARN bpf_setup failed (optional dev tooling)" >&2
	;;
bpf)
...
probe)
	...
	start | stop | status | report)
		sudo bash "$SCRIPTS/local-dev/bpf_session.sh" "$sub" "${3:-}"
		;;
	*)
