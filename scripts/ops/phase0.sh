...
if [[ -x "$SCRIPTS/edge/sysctl.sh" ]]; then
	run_check "sysctl" bash "$SCRIPTS/edge/sysctl.sh" verify || {
...
if [[ -x "$SCRIPTS/edge/nic_tune.sh" ]]; then
	run_check "NIC RX/IRQ" bash "$SCRIPTS/edge/nic_tune.sh" verify || {
		[[ "$STRICT" == "1" ]] || warn "NIC tuning not applied (run: sudo bash scripts/edge/nic_tune.sh apply)"
	}
else
