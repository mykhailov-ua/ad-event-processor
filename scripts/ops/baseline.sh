...
fetch_metrics() {
  ...
  EDGE_PERIMETER_RPS="$(prom_query_scalar 'sum(rate(ad_event_processor_edge_perimeter_pass_total[5m]))' || echo "")"
...
write_snapshot() {
  ...
  {
    echo "# edge minimal baseline snapshot (not a 24h soak)"
    ...
    echo "edge_perimeter_rps=${EDGE_PERIMETER_RPS:-na}"
