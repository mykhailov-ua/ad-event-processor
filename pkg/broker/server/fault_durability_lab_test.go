...
func skipFaultLab(t *testing.T) {
	...
	if os.Getenv("BROKER_FAULT_LAB") != "1" && os.Getenv("CI") == "" {
		t.Skip("durability lab tests run with BROKER_FAULT_LAB=1 or in CI; use scripts/fault/broker_fault_lab.sh")
	}
}
...
func TestFault_SlowFsync_ProduceLatencyRises(t *testing.T) {
	skipFaultLab(t)
	...
	dir, err := os.MkdirTemp("", "fault-slow-fsync-*")
	if err != nil {
		t.Fatal(err)
	}
	...
	t.Logf("fault_proof fault=slow_fsync sync_delay_ms=%d mean_produce_ms=%.2f produce_ok=true",
		syncDelay.Milliseconds(), avg.Seconds()*1000)
}

func TestFault_PageCachePressure_ProduceLatencyRises(t *testing.T) {
	skipFaultLab(t)
	if !faultLabOnLinux() || !stressNgAvailable() {
		t.Skip("requires Linux and stress-ng")
	}

	dir, err := os.MkdirTemp("", "fault-pagecache-*")
	if err != nil {
		t.Fatal(err)
	}
	defer os.RemoveAll(dir)
	...
	t.Logf("fault_proof fault=page_cache_thrashing baseline_us=%d under_stress_us=%d ratio=%.2f stress_ng=true",
		baseline.Microseconds(), underStress.Microseconds(), ratio)
}

func TestFault_CPUThrottle_ReplicationCatchesUp(t *testing.T) {
	skipFaultLab(t)
	if !faultLabOnLinux() || !cpulimitAvailable() {
	...
	leaderDir, err := os.MkdirTemp("", "fault-cpu-leader-*")
	if err != nil {
		t.Fatal(err)
	}
	...
	probeCoord, err := NewCoordinator("fault-cpu-probe", leaderAddr, redisURL, nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	followerProc := startBrokerProcess(t, bin, followerDir, "fault-cpu-follower", redisURL, followerAddr)
	...
	t.Logf("fault_proof fault=cpu_throttle_replication cpulimit_pct=20 msgs=%d lag_observed=%v caught_up=true",
		msgCount, sawLag)
}

func TestFault_RedisOutage_CoordinationRecovers(t *testing.T) {
	skipFaultLab(t)
	...
	proxy, proxyListen := startFaultTCPProxy(t, hostPort, 0, 0, 0)
	defer proxy.close()
	...
	t.Logf("fault_proof fault=redis_brief_outage block_seconds=6 produce_recovered=true")
}

func TestFault_RedisSentinelFailover_ProduceContinues(t *testing.T) {
	skipFaultLab(t)
	if os.Getenv("BROKER_FAULT_SENTINEL") != "1" {
		t.Skip("requires BROKER_FAULT_SENTINEL=1 and deploy/broker-lab sentinel stack")
	}
	...
	dir, err := os.MkdirTemp("", "fault-sentinel-*")
	if err != nil {
		t.Fatal(err)
	}
	defer os.RemoveAll(dir)
	...
	coord, err := NewCoordinator("fault-sentinel-broker", s.Addr(), redisURL, s)
	if err != nil {
		t.Fatal(err)
	}
	...
	masterContainer := os.Getenv("BROKER_FAULT_SENTINEL_STOP_CONTAINER")
	if masterContainer != "" {
	...
	t.Logf("fault_proof fault=redis_sentinel_failover master_stopped=%s produce_after=true", masterContainer)
}
