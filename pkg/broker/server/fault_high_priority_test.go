...
func TestFault_NetworkPartition_StaleLeaderRejected(t *testing.T) {
	...
	t.Logf("fault_proof fault=redis_network_partition stale_local_accept=%v follower_msgs=21 authoritative=true", staleAccepted)
}

func TestFault_LeaseExpiry_FrozenLeaderRejected(t *testing.T) {
	skipFaultIntegration(t)
	...
	leaderDir, err := os.MkdirTemp("", "fault-freeze-leader-*")
	if err != nil {
		t.Fatal(err)
	}
	...
	probeCoord, err := NewCoordinatorWithConfig("fault-freeze-probe", leaderAddr, redisURL, nil, fastCoordConfig())
	if err != nil {
		t.Fatal(err)
	}
	requireEventually(t, func() bool {
	...
	followerCoord, err := NewCoordinatorWithConfig("fault-freeze-follower", follower.Addr(), redisURL, follower, fastCoordConfig())
	if err != nil {
		t.Fatal(err)
	}
	...
	t.Logf("fault_proof fault=lease_expiry_sigstop stale_rejected=%v failover_produce=true", staleRejected)
}

func TestFault_ClientRedirect_AfterFailover(t *testing.T) {
	skipFaultIntegration(t)
	...
	t.Logf("fault_proof fault=client_redis_failover stale_addr=true redirect_produce=true offset=%d", off)
}

func TestFault_ConsumerResume_AfterLeaderKill(t *testing.T) {
	skipFaultIntegration(t)
	...
	off, commitErr := commitCli.CommittedOffset(topic, 0, "fault-resume-group")
	_ = commitCli.Close()
	if commitErr != nil {
	...
	t.Logf("fault_proof fault=consumer_resume_leader_kill seen=%d committed_ok=true", preKill+postKill)
}
